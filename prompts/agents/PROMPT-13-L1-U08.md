...
- **Level:** L1 (A1)
- **Unit number:** 8
- **Position in cached unit list:** index 7 (zero-indexed)
- **Student Work Protection:** ACTIVE

Pull the unit ID from your cached list:
```
unit_id = cached_L1_units[8-1].id
unit_title = cached_L1_units[8-1].title_en
```
...
- Log: `[PROTECTION] L1-U8: <N> student records found`
...
```
feat(L1-U8): rewrite reading passages + questions to meet A1 targets
...
Refs: PROMPT 13 L1 Unit 8
```

## After Successful Commit + Push Verification
Read the next file: **`PROMPT-13-L1-U09.md`**

## Failure Handling
- If Phase A returns 0 passages → log "L1-U8: NO PASSAGES FOUND", proceed to next file
...
- Do NOT proceed to PROMPT-13-L1-U09.md without successful commit + push verification
