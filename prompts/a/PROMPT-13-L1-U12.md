...
- **Level:** L1 (A1)
- **Unit number:** 12
- **Position in cached unit list:** index 11 (zero-indexed)
- **Student Work Protection:** ACTIVE

Pull the unit ID from your cached list:
```
unit_id = cached_L1_units[12-1].id
unit_title = cached_L1_units[12-1].title_en
```
...
- Log: `[PROTECTION] L1-U12: <N> student records found`
...
```
feat(L1-U12): rewrite reading passages + questions to meet A1 targets
...
Refs: PROMPT 13 L1 Unit 12
```

## After Successful Commit + Push Verification
Read the next file: **`PROMPT-13-L1-99-FINALIZE.md`**

## Failure Handling
- If Phase A returns 0 passages → log "L1-U12: NO PASSAGES FOUND", proceed to next file
...
- Do NOT proceed to PROMPT-13-L1-99-FINALIZE.md without successful commit + push verification
