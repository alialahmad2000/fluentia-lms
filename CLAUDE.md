...
### 2026-07-10 — PROVISION مصعب: L2/private account + 5 university-course A2 vocab tracks («مفردات مقرّراتي»)
- What: Provisioned private student مصعب جمال العمري (mosab05113@gmail.com) end-to-end on prod. **Layer 1 (backbone):** dedicated private group «مصعب العمري — فردي» (code MOSAB, level 2, trainer=Ali, cap 1) + auth user + profiles (must_change_password=true) + students row (academic_level=2, package=private, track=foundation, gender=male, study_mode=group) — mirrors the 2 existing L2/private students. 12 generic L2 units resolve (is_published is NOT the unit gate — red herring). **Layer 2:** generated 5×18 = 90 globally-unique A2 course words (ENT325/SCM341/HCM345/NPF323/MGT303) into vocab_cards (the LIVE unified store; source='uni:<CODE>' → 5 collections; curriculum_vocabulary_id=null; idempotent on student_id+word_normalized). Feeds his SRS/reviews. Verified as the STUDENT role (RLS): 5 collections resolve; cross-student isolation confirmed (mock student sees 0).
- Frontend: NEW /student/course-vocab «مفردات مقرّراتي» — premium RTL surface grouping his course-tagged vocab_cards into 5 labeled collections (per-course jewel accents, counts, mastery, word pronunciation via pronounceWord). Gated nav item (sections + drawer) via visibleWhen 'course-vocab-count' (Sidebar + MobileDrawer count providers) → ONLY students with course vocab see it. premium-ui-critic pass 7.5→ (AA contrast, 44px targets, hierarchy, reduced-motion, hover, empty-state medallion). Design-loop screenshot + LIVE mock-student verify: route + empty state render, item correctly hidden, existing nav intact, 0 new console errors.
- Deploy: ported onto origin/main content (local tree far-diverged, 292 behind) + committed via Git Data API base_tree (preserves all parallel work) → commit 0aee8b5. Vercel SUCCESS; app.fluentia.academy serves new build (index-Dm1SOBZH.js contains course-vocab).
- Files: NEW src/pages/student/CourseVocabulary.jsx, scripts/provision-mosab-alamri.cjs, scripts/verify-mosab-student-rls.cjs; modified src/App.jsx, src/config/navigation.js, src/components/layout/{Sidebar,MobileDrawer}.jsx. DB: 1 group, 1 auth user, 1 profile, 1 students row, 90 vocab_cards. No global-curriculum change (§7). Audio = existing word-pronunciation util (catalog MP3 → TTS fallback), no ElevenLabs spend.
- Next (Ali): send welcome email (§9); mosab changes temp password on first login. Out of scope now: per-course reading/speaking/writing + deeper vocab (§7).
