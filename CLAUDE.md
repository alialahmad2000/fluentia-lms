...
### 2026-07-10 — مصعب: full custom BUSINESS-MAJOR curriculum (uses_custom_curriculum, owner units)
- What: Per Ali's choice to go beyond vocab, built مصعب his OWN business-major English curriculum. Set students.uses_custom_curriculum=true + created 5 owner_student_id units (one per course: ENT 325 / SCM 341 / HCM 345 / NPF 323 / MGT 303) on /student/curriculum. Each unit = A2 English content SET in a business/major context (business is the VEHICLE — we teach English, not business), masculine Arabic. Per unit: 1 reading (113–140w, target words *marked*) + 7 comprehension MCQs + 12 target vocab + 1 grammar point (present simple / prepositions of movement / frequency adverbs / past simple regular / can-ability) with an auto-graded 'choose' exercise + 1 writing task + 1 speaking task. He now sees ONLY his 5 units (the generic L2 spine is replaced — Ali's explicit trade).
- Data-only: the uses_custom_curriculum + owner-unit path is already deployed (_useCurriculumData.js: custom students get owner_student_id units by custom_sort; CurriculumBrowser auto-redirects to /level/{academic_level}). No frontend change. RLS verified AS the student: 5 units + all content resolve (cc_unit_visible/cc_reading_visible are owner-aware); mock student sees 0 units/readings (isolation). Mirrors the live curriculum schema exactly.
- Files: NEW scripts/provision-mosab-curriculum.cjs + scripts/verify-mosab-curriculum-rls.cjs. DB: 5 curriculum_units (owner) + 5 readings + 35 comprehension_questions + 60 curriculum_vocabulary + 5 grammar + 5 grammar_exercises + 5 writing + 5 speaking; students.uses_custom_curriculum=true. No global-curriculum change.
- Note: his «مفردات مقرّراتي» course-vocab surface (vocab_cards source=uni:*) + SRS deck remain, complementary. Revert = uses_custom_curriculum=false (owner units hide, generic L2 spine returns; owner rows preserved).
