...
### 2026-06-11 — EVIDENCE-BASED PERF PASS + permanent regression armor (zero behavior change)
- What: measured-first optimization pass. **Unit-page chunk 737→57 kB** (gz 141→17): `ActivityStation.jsx` had `import * as LucideIcons` which bundled the ENTIRE icon library — replaced with a static 8-icon map (same fallback). **Entry chunk 512→432 kB / initial JS 1,030→944 kB (gz ≈320→280)**: trainer-only `FloatingToolbar`+`TimerBadge` now lazy+role-gated in LayoutShell (rendered null for students anyway); Sidebar's vocab due-badge dynamic-imports `services/vocab` (was dragging ts-fsrs into entry); canvas-confetti lazy+idle-warmed in `lib/celebrations.js`; 3 render-blocking trainer/preview-only font links in index.html switched to async preload (Tajawal untouched).
- Protected surfaces (save/submit, auth, listening pipeline, push/SW, DB) untouched. GPU/blur findings documented in `PERF-AUDIT-FINDINGS.md` as report-only (visual change needs Ali's call). Runtime was measured healthy: no heap growth across 10 navs, SPA nav avg 210ms chromium / 70ms webkit (localhost).
- **Safety net:** `tests/perf-safety/` Playwright suite (chromium+webkit, `npm run test:perf-safety`) encodes the protected behaviors — partial-answer autosave/restore-never-complete, submit guard, explicit-submit-only, 0-units guard, 8-route nav stability, listening audio mounts. Ran green after EVERY individual fix. Uses mock-test-a1 against a local `vite preview` (PERF_BASE_URL to override).
- **Regression armor:** `npm run build:check` → `scripts/check-perf-budget.mjs` FAILS the build if entry >510 kB, initial JS >1100 kB, or any chunk >600 kB (the lucide failure mode). `.github/workflows/perf-guard.yml` runs the budget on every push/PR + the safety suite when `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` repo secrets exist (NOT yet configured — add them to activate the CI behavior suite). Raising budgets = deliberate decision, document in `PERF-AUDIT-FINDINGS.md`.
- Files: `src/pages/student/curriculum/unit-v3/components/ActivityStation.jsx`, `src/components/layout/LayoutShell.jsx`, `src/components/layout/Sidebar.jsx`, `src/lib/celebrations.js`, `index.html`, `tests/perf-safety/*`, `scripts/check-perf-budget.mjs`, `.github/workflows/perf-guard.yml`, `PERF-AUDIT-FINDINGS.md`, `package.json`.
- Status: complete; all numbers in `PERF-AUDIT-FINDINGS.md` (before/after table).
...
- **Frontend:** `RangePicker` → 7 presets (اليوم/٣ أيام/أسبوع/١٤ يوم/شهر/٩٠ يوم/الكل = days 1/3/7/14/30/90/3650, URL-persisted via shared `VALID_DAYS`). `useAdminReports.js` `ensureTodayRollup()` (memoized 5 min, failure-tolerant) runs before report fetches. Usage tab opens with **"الوقت المستثمَر حسب القسم"**: per-section minutes bars (skill-colored), % of total, completions + student counts. Footnotes updated (data includes today).
- Verified live: 30d section_time = reading 374د/12 · writing 252د · grammar 219د…; all-time = 3,774 دقيقة / 77 يوم / 13 طالب; days=1 works post-refresh.
- Files: `supabase/migrations/20260611100000_admin_reports_hub_v2.sql` (NEW), `src/pages/admin/reports/{useAdminReports.js,reportKit.jsx,AdminReportsHub.jsx,StudentReportDetail.jsx}`, `tabs/{UsageTab,PulseTab,StudentsTab}.jsx`.
- ⚠️ NOTE (concurrent sessions): a parallel push (guestbook/impersonation) based CLAUDE.md on a pre-hub blob and dropped the 2026-06-10 REPORTS HUB entry — restored below alongside this one. When pushing CLAUDE.md via the Trees API, ALWAYS re-fetch the latest blob right before committing.
...
### 2026-06-10 — REPORTS HUB (مركز التقارير): total rebuild of /admin/reports on REAL usage data + legacy archived
- Owner: the old reports page read DEAD tables (payments=1 row, submissions=0, attendance=0, churn_predictions=0 → "إيرادات 600/واجبات 0"). He asked to archive it and build a deeply insightful hub: "who uses the platform, when, for what" — explicitly for paying-student retention.
- **Server layer (migration `20260610120000_admin_reports_hub.sql`, applied):** 6 SECURITY DEFINER report RPCs, each gated `IF NOT is_staff()` as line 1 (admin+agent; negative-tested with a student JWT → `forbidden`), STABLE, `search_path=public`, REVOKE PUBLIC/anon: `admin_report_pulse` (totals+prev-window deltas from `student_daily_activity`, daily series, live today/active-now from `user_sessions`, WAU/MAU, hour×weekday heatmap from `analytics_events`, top events) · `admin_report_students` (THE retention heart: per-student identity+live signals+window aggregates+first/second-half trend+14d sparkline, one call) · `admin_report_usage` (feature ranking via UNION across scp/srs_review_logs/game_sessions/spelling_lab/library/speaking/chat/saved_words + devices/browsers + normalized top pages) · `admin_report_learning` (weekly avg score per skill, score distribution, hardest units ≥5 attempts, vocab growth cumulative, SRS ratings, speaking volume) · `admin_report_health` (client_error_log trend+signatures, section_crash, audio_event_log stall/error rate, open quality flags, AI cost by day/type/model/student from `ai_usage`) · `admin_report_student_detail` (gated wrapper around existing `get_student_activity_report` + admin extras: sessions/devices/errors/AI cost/games/recordings). Riyadh-TZ day bucketing everywhere; test accounts excluded via `profiles.is_test_account`. rls-security-auditor: PASS (0 blocking; advisory: the PRE-EXISTING `get_student_activity_report` is anon-callable — unchanged here, flagged for a future token-gated refactor).
- **Frontend (`src/pages/admin/reports/`):** `AdminReportsHub` (tab+range in URL: ?tab=&days=14/30/90) with 5 tabs — النبض (live hero: active-now pulse + today partial, 8 KPI cards w/ period deltas, daily Composed chart, study-hours heatmap) · الطلاب (desktop dense table / mobile cards, risk badge ALWAYS visible w/ tap-to-toggle reasons, sparkline, trend arrow, expiry chip, CSV export w/ BOM, sort presets, row → deep-dive) · الاستخدام · التعلّم · الصحة والتكلفة; plus `/admin/reports/student/:studentId` deep-dive (canonical report + sessions/devices/errors/AI cost/recordings/words/timeline). Risk model client-side in `reportKit.jsx` (`computeRisk`: high=5d inactive/never; watch=3d/streak-lost/declining-halves; reasons always shown) — thresholds tunable without migrations.
- **Design-loop (2 premium-ui-critic iterations, real admin session vs prod data):** 5.5→7+/10; fixed 12px-floor sweep, 26px KPI numbers (label above), 44px hero live counter, mobile card layout (risk verdict never off-canvas), 44px touch targets, lucide instead of emoji, icon empty-states, **bidi scramble on Arabic-unit numbers (`dir="ltr"`→`dir="auto"` — "س 13 د 20" now renders "13 س 20 د")**. 0 console errors both widths.
- **Archive (hide-don't-delete):** old `AdminReports.jsx` UNTOUCHED on disk, now at `/admin/reports-legacy` (churn+scheduling tabs included), linked from the hub header; nav unchanged (التقارير already points to /admin/reports). `/admin/analytics` raw event viewer linked too.
- **Verify:** RPCs live-tested as admin (cross-check vs `get_student_activity_report` exact match), student → forbidden; esbuild bundle-resolution green; admin smokes extended (hub renders/deep-dive/legacy — env-gated on SMOKE_ADMIN_*). Admin session for testing minted via service-key `generate_link(magiclink)` → `/auth/v1/verify` (no password needed — the documented admin password is still stale).
- Files (new): `supabase/migrations/20260610120000_admin_reports_hub.sql`, `src/pages/admin/reports/{AdminReportsHub,StudentReportDetail,reportKit}.jsx`, `src/pages/admin/reports/useAdminReports.js`, `src/pages/admin/reports/tabs/{PulseTab,StudentsTab,UsageTab,LearningTab,HealthTab}.jsx`. Modified: `src/App.jsx` (3 routes), `tests/smoke/admin-paths.spec.mjs` (+3 specs), `CLAUDE.md`.
...
### 2026-06-10 — CLASS COORDINATOR (منسّقة الأكاديمية): new role + full class-scheduling system
- What: Hajar (هاجر الدسوقي) moves from WhatsApp-only coordination into the platform. New 4th staff role **`coordinator`** records the class timetable — fixed WEEKLY templates + ONE-OFF sessions, individual AND group — and students + teachers see times/links in-app with push notifications. Decided with Ali: dedicated role (not `agent`), both recurrence models, she owns group schedules too, all three automations (create/change/cancel pushes, pre-class reminders, wa.me share button).
- **DB (migration `20260610120000_class_coordinator.sql`, APPLIED to prod + enum `coordinator` added separately):** `class_schedules` (weekly template: type individual|group, student_id/group_id, trainer_id, day_of_week, start_time Riyadh, duration, meeting_link, status active|paused|ended) + `class_sessions` (dated occurrences; `orig_start_at` unique-per-template slot so reschedules are never resurrected by the materializer; ad-hoc rows have schedule_id NULL). **Deliberately NOT cs_bookings** — that table has a GLOBAL no-overlap EXCLUDE (one CS calendar); classes need parallel teachers, so the EXCLUDEs here are scoped per-trainer + per-student (status='scheduled' only). RLS: SELECT-only for involved parties (student own/own-group, trainer own, coordinator/admin all via `is_coordinator_staff()`); ZERO client write policies — all writes via SECURITY DEFINER `coord_*` RPCs (create/update schedule, create/cancel/reschedule/update session, pickers, week views, conflict preview) + `trainer_my_sessions` + `student_upcoming_sessions`. Notifications: `class_notify()` → pg_net → `send-push-notification` (in-app `class_reminder` rows + web push, anon-key pattern like group_message_push). Crons: `class-sessions-materialize` (00:20 Riyadh daily, 14 days ahead, p_days clamped ≤30) + `class-session-reminders` (*/5 min sweep: 60-min + 15-min pushes, reminded_60/15 flags).
- **Security (rls-security-auditor findings FIXED in prod + file):** materializer was anon-reachable via the `auth.uid() IS NULL` cron bypass → `revoke from public, anon` on it AND every coord_*/trainer/student RPC (defense-in-depth; all also gated internally); p_days clamped; `coord_assert()` made VOLATILE. Verified `has_function_privilege('anon', …) = false` across the surface.
- **Frontend:** `/coordinator` workspace (`CoordinatorWorkspace` shell + tabs) — `CoordinatorWeek` (week grid, Arabic dates via Intl ar-u-nu-latn, today highlight, per-day ghost «+ إضافة حصة» that opens the modal prefilled, session cards w/ copy-link + wa.me share + reschedule + cancel via in-app `ConfirmModal`), `SchedulesList` (templates w/ تعديل/إيقاف مؤقت/استئناف/إنهاء, neutral ghost actions), `ClassModal` (segmented فردية/جماعية + أسبوعية/مرة واحدة, searchable student picker w/ gradient avatars, group→prefills its teacher, live conflict preview via `coord_check_conflicts`, framer-motion entrance, success state = WhatsApp share `wa.me/?text=` + «حصة أخرى»). Role wiring: `user_role` value in authStore (`isCoordinator`/`useIsCoordinator`), App.jsx routes `allowedRoles=['coordinator','admin']`, RoleRedirect + LoginPage cases, `COORDINATOR_NAV` + Sidebar ROLE_DASHBOARDS. **Student:** `NextClassWidget` rewritten — fetches real `student_upcoming_sessions` (per-session link, live «الحصة جارية الآن» state, correct Arabic plurals بعد ٣ أيام, next-2 list, `hideTitle` under Spotlight's labeled section) with full legacy groups.schedule fallback. **Teacher:** new `/trainer/schedule` («جدولي» in TEACHER_NAV; mobile bar drops curriculum to stay at 5) — week list w/ student/group names + state-aware دخول button (solid when <30 min away).
- **Verify:** e2e vs prod with real JWTs 14/14 PASS (pickers, student/trainer blocked from coord RPCs incl. no-profiles-row case, weekly create → 2 occurrences Riyadh-correct, student sees session+link, reschedule/cancel, one-off, reminder sweep fired, overlap rejected 23P01); in-app notification rows verified for BOTH parties across all 5 event types then cleaned up; `npm run build` green; design-loop (screenshots + premium-ui-critic 5.5–7/10 → fixes applied: Arabic dates everywhere, 12px type floor, plurals, neutral action rows, 40-44px targets, hover lift, ghost empty-day CTAs, modal entrance motion, ConfirmModal replaces window.confirm) + arabic-copy-reviewer fixes (neutral imperatives, غياب not لم يحضر, «وصل الإشعار للجميع», DD-MM not ISO in pushes).
- **Accounts:** هاجر الدسوقي `hagareldesoky86@gmail.com` role=coordinator (must_change_password=true — see handoff). Throwaway zz-coord-test deleted post-verify. NOTE: her /coordinator workspace activates when this branch merges to main; the DB layer is already live.
- Files (new): `supabase/migrations/20260610120000_class_coordinator.sql`, `src/lib/coordinator/scheduling.js`, `src/pages/coordinator/{CoordinatorWorkspace,CoordinatorWeek,SchedulesList,ClassModal,ConfirmModal}.jsx`, `src/pages/teacher/schedule/TeacherSchedule.jsx`. Modified: `src/App.jsx`, `src/stores/authStore.js`, `src/config/navigation.js`, `src/config/teacherNavigation.js`, `src/components/layout/Sidebar.jsx`, `src/pages/public/LoginPage.jsx`, `src/pages/student/dashboard/widgets/NextClassWidget.jsx`, `src/pages/student/dashboards/SpotlightDashboard.jsx`.
- DB: 2 tables + 2 EXCLUDE constraints + 5 indexes + 19 functions/RPCs + 6 RLS policies + 2 cron jobs + 1 enum value. Edge fns: none (reuses send-push-notification). Status: DB LIVE + verified; frontend on branch `class-coordinator` → PR for Ali.

### 2026-06-10 — INDIVIDUAL (1-on-1) STUDENT ACCOUNT TYPE + profession-tailored track (pilot: Marketing Manager)
- What: a NEW account type for individual-course students (Ali: "I don't give them the platform because I don't want them on the normal account"). `students.study_mode` ('group' default | 'individual') + `students.specialization_id` → individual students get a completely different experience: an "executive briefing" home (obsidian + ONE brass accent — the approved Majlis direction, scoped `.iv-root` in `individual.css`), a minimal profession-first nav (`INDIVIDUAL_NAV` via `getNavForUser(role, studentData)` in LayoutShell — Home/مساري المهني/Library/Chat/Profile; NO group surfaces), and **المسار المهني** at `/student/track` — sequential missions, each = brief → vocab dossier (10 terms) → power phrases → **AI voice ROLEPLAY in character** → writing deliverable. Group students 100% unaffected (default 'group', dashboard switch is a single `study_mode` check in StudentDashboard.jsx).
- **Content (pilot):** `specializations` ('marketing_manager') + 10 authored bilingual `specialization_modules` (intro→value prop→team meeting→brief→results/metrics→content strategy→agency negotiation→emails→insights→CEO pitch). Seed: `scripts/seed-marketing-track.cjs` (idempotent upsert; DB Arabic is GENDER-NEUTRAL per project rule). New personas later = new seed rows, zero code.
- **Roleplay engine:** extended `speaking-conversation-turn/-grade` (additive `module_id` path; curriculum path byte-identical): turn fn loads scenario from `specialization_modules.roleplay`, AI plays a CHARACTER (Daniel the BD director, the skeptical client, the CEO…) via `buildRoleplaySystemPrompt`; **stage-direction strip** (prompt rule + `stripStageDirections()`) so TTS never reads "*smiles*" aloud — caught live in smoke. Grade fn for module convos calls `track_mark_roleplay()` and does NOT touch speaking_recordings/student_curriculum_progress. `speaking_conversations.unit_id` now nullable + `module_id` added.
- **Security (rls-security-auditor pass, all findings fixed in `20260610130000_individual_track_hardening.sql`):** progress writes are SERVER-AUTHORITATIVE — clients call `track_complete_stage()` (SECURITY DEFINER, whitelists brief/vocab/phrases/writing, sanitizes extra, atomic jsonb merge → no race with the grade fn's `track_mark_roleplay()` which is service_role-only); direct INSERT/UPDATE policies dropped (forge-proof, verified live with the demo JWT: forge=0 rows, roleplay-via-RPC=rejected); `trg_guard_student_account` blocks non-admins changing study_mode/specialization_id (verified live); partial unique index respects soft-delete; trainer SELECT policy added; sc_student_update got explicit WITH CHECK.
- **Admin:** AddStudentModal → نوع الحساب (مجموعات/فردي ١:١) + التخصص المهني picker; individual → group_id null. Demo account: `indiv-demo-marketing@fluentia.academy` / `Fluentia2025!` (is_test_account, male — exercises the male tone path; progress soft-reset after testing so it demos fresh).
- **Reviewed:** premium-ui-critic (10 fixes applied: mobile mission-card stacking, hero ring anchored in panel + 0% spark «نبدأ اليوم», locked rows readable + «بعد المهمة N» chips, brief 2-col 62ch, type floor ≥12px, 44px touch targets, RTL logical props, interactive hovers, compact dashboard index) + arabic-copy-reviewer (gendered strings → useG()/neutral; 4 seeded strings fixed + re-seeded; KPI plural agreement) + rls auditor (above). esbuild + `npm run build` green; headless E2E as the demo student: login → exec dashboard → track → module stages render, RPC + guards verified, roleplay starts in character with TTS audio. Known pre-existing noise: the `submissions.voice_url` 400 (documented previously, unrelated).
- Files (new): `supabase/migrations/20260610120000_individual_track.sql` + `20260610130000_individual_track_hardening.sql` (both APPLIED live), `src/pages/student/individual/{IndividualDashboard,TrackHome,ModulePage}.jsx` + `individual.css`, `src/hooks/useIndividualTrack.js`, `scripts/seed-marketing-track.cjs`, `scripts/seed-individual-demo.cjs`, `scripts/indiv/_capture.mjs`. Modified: `src/config/navigation.js` (INDIVIDUAL_NAV + getNavForUser; STUDENT_NAV untouched — hidden-9 intact), `src/components/layout/LayoutShell.jsx`, `src/pages/student/StudentDashboard.jsx`, `src/App.jsx` (+2 routes), `src/components/curriculum/speaking/ConversationMode.jsx` (optional moduleId), `src/pages/admin/AdminStudents.jsx`, `supabase/functions/speaking-conversation-{turn,grade}/index.ts` (deployed turn v5 / grade v6).
- DB: 3 new tables + 2 students columns + 2 speaking_conversations changes + 2 RPCs + 1 trigger + RLS. Status: complete; deploying to main.
- Next personas: run a sibling of `seed-marketing-track.cjs` (e.g. CEO) + add the specialization row — the UI/nav/roleplay all key off `specialization_id`, no code changes needed.
