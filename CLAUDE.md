...
### 2026-07-11 — مصعب: FardiHero mission home + bespoke premium cover-art for his 5 units
- What: Made مصعب's whole account revolve around his major. (1) Set students.custom_mission_ar → activates the DEFAULT dashboard's **FardiHero** (isFardi = uses_custom_curriculum && custom_mission_ar): a mission-driven hero lit by his current unit's cover art + custom-curriculum progress. His home is now his English-for-business mission. (2) Designed a bespoke, cohesive **"Obsidian Atelier"** cover-art system for his 5 custom units — obsidian base + per-course jewel accent glow (gold/sky/emerald/rose/violet) + real lucide glyph (rocket/truck/heart-pulse/heart-handshake/briefcase) inside concentric accent arcs + gold hairline frame w/ corner brackets + code chip + Playfair title + dot-grid/grain. Rendered in HTML/CSS via Playwright chromium (1536×864 PNG), uploaded to curriculum-images/units/mosab-*.png, wired to curriculum_units.cover_image_url. Drives his FardiHero home background + unit cards. NO FAL needed.
- Why HTML/CSS not FLUX: FAL is LOCKED (exhausted balance — needs Ali to top up fal.ai/dashboard/billing). The designed covers are premium + need no top-up; if Ali wants photoreal covers later, top up FAL and regenerate.
- Files: NEW scripts/generate-mosab-covers.mjs (reusable — renders lucide via react-dom/server + Playwright). DB: students.custom_mission_ar set; 5 curriculum_units.cover_image_url set; 5 PNGs in storage. Data-only, no deploy (FardiHero + cover rendering already live).
- Honest status: his account is now fully major-oriented (mission home + custom curriculum + covers + course vocab + SRS). The CURRICULUM is still a START (5 units × 1 reading) — a "full system" that really moves his English needs DEPTH next: listening per unit, a 2nd reading per unit, more units, broader grammar. FAL-blocked shared surfaces (Everyday English/library/speaking-hub) stay global. See [[project-fluentia-mosab-course-vocab]].
