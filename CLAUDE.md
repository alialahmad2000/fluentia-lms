...
### 2026-06-09 — ROOT-CAUSE FIX: reading + listening answers silently stopped saving for ALL students (since June 6)
- Student فاطمة خواجي (fa.khawaji@gmail.com) reported reading answers don't save. Investigation (instrumented production-build reproduction + DB verification) found a SEVERE, widespread regression: BOTH reading and listening stopped persisting ANY progress for EVERY student on **2026-06-06** — while writing/grammar/vocabulary/speaking kept working.
- **Root cause:** the teacher-account ship `d4cd061` (went live 06-06 when the previously-broken prod build resumed deploying) added `if (readOnly) return` guards to the reading/listening SAVE handlers, but declared `const { readOnly } = useCurriculumPreview()` only in the PARENT components (`ReadingTab`/`ListeningTab`) — NOT in the child components that own the handlers: `ReadingContent` + `PassageDisplay` (ReadingTab) and `ListeningExercises` (ListeningTab). So `readOnly` was undefined where the guard ran → **`ReferenceError: readOnly is not defined`** thrown inside a fire-and-forget autosave callback → swallowed (no visible crash) → the answer registered on screen but the DB write never executed. Same class as the April "profile is not defined" WritingTab crash. The other 5 skill tabs scoped readOnly correctly, which is why ONLY reading+listening broke.
- **Fix (minimal/surgical):** declare `const { readOnly } = useCurriculumPreview()` INSIDE each child component that uses it — `ReadingContent` + `PassageDisplay` (ReadingTab.jsx), `ListeningExercises` (ListeningTab.jsx). The now-redundant parent declarations are left in place (harmless).
- **Verified end-to-end on a production-identical build** (`vite build` + `vite preview`, real injected session, `window.__SAVE_DBG` trace that survives the console-drop): BEFORE → `promise-REJECTED: "readOnly is not defined"`, zero DB write; AFTER → autosave POST fires and a real `in_progress` reading row lands in `student_curriculum_progress` — re-confirmed on the CLEAN no-instrumentation build. esbuild --packages=external + full Vite build green on both files. (Reading reproduced directly; listening is the identical bug fixed identically.)
- Shipped to `main` via GitHub Trees API; fired the admin force-refresh switch so every device pulls the fix on next open. Files: `src/pages/student/curriculum/tabs/ReadingTab.jsx`, `src/pages/student/curriculum/tabs/ListeningTab.jsx`.

### 2026-06-09 — FORCE-REFRESH SWITCH: admin-triggered "wipe cache + re-download" for all devices
- For the "we fixed it but their phone still runs the OLD code" tickets (stale PWA/service-worker cache): an on-demand lever to make every student's device clear its cache + pull the latest build. The app already AUTO-heals on a version mismatch (`main.jsx` self-heal clears caches + unregisters SWs + reloads), but there was no MANUAL override.
- **DB (migration `20260609020000`, applied via Mgmt API):** `app_config.force_refresh_at` (epoch) + public `get_app_force_refresh()` RPC (anon+authenticated, read at boot) + admin-only `bump_force_refresh()` RPC (sets it to now()).
- **Client (`main.jsx`):** new boot IIFE `applyForcedRefresh()` — fetches the switch (public RPC, no-store), and if it's newer than this device's durable `localStorage.fluentia_force_refresh_applied` marker, clears all caches + unregisters all SWs + reloads. Applies a given epoch at most once (marker set BEFORE reload → cannot loop). Independent of the version-mismatch self-heal.
- **Admin UI:** "إجبار تحديث الأجهزة" button on /admin/bug-reports (next to تحديث) → `bump_force_refresh()` → every device force-refreshes on its next open.
- Chicken-and-egg handled: a client on an OLDER build first updates via the existing version-mismatch self-heal (already clears+reloads), landing on the new build that contains `applyForcedRefresh` for all future forced refreshes.
- Verify: `get_app_force_refresh()` returns 0 (idle); esbuild --packages=external clean on both files; full Vite build green. Shipped to main via Trees API. Files: `src/main.jsx`, `src/pages/admin/AdminBugReports.jsx`, `supabase/migrations/20260609020000_force_refresh_switch.sql`.

### 2026-06-09 — STUDENT BUG TICKETS: audio "plays then stops" race + submit-button dead-end
- Triaged the open student bug tickets (/admin/bug-reports). Proved first that audio FILES + infra are healthy (uniform mono, HTTP-206 range, audio/mpeg; SW already excludes media) — so NOT a content problem — then found two real, root-caused CODE bugs and fixed them.
- **BUG 1 — audio "plays a fraction of a second then stops"** (الهنوف on Mac: full reading-article audio, "all articles"; لمياء on iPhone: listening). Root cause: the blob-source loaders in BOTH `ListeningPlayer.jsx` and `useAudioEngine.js` (reading player) had a CLOBBER RACE — the background `fetch()`→blob could resolve in the tiny window AFTER the gesture's `play()` but BEFORE `audio.paused` flips false; the old guard `(!audio.paused || currentTime>0)` was still true, so it set `audio.src = blob` + `load()`, RESETTING the element and aborting the just-started playback. Fix: also bail while a play() is PENDING — `ListeningPlayer` reuses the existing `isStartingRef`; `useAudioEngine` adds a `committedRef` (set true in play/toggle/retry, reset on source change). No regression: the blob still attaches pre-tap; it just never clobbers a committed playback, and raw streaming already works (SW excludes media).
- **BUG 2 — "my answer won't submit / ما ينرفق"** (ليان, Reading + Listening). The "تسليم الإجابات" button was DISABLED until every question was answered, with no hint WHICH was missing → looked broken/dead. Now it's always tappable: if questions remain it scrolls to + highlights the first unanswered one and toasts how many are left ("باقٍ N أسئلة بدون إجابة"); it only submits when all are answered. Each question wrapped with a scroll-target id (`listen-q-${i}` / `read-q-${id}`). Applied to `ListeningTab.jsx` + `ReadingTab.jsx`. (Also: the AI auto-responder had misread her as an audio problem — corrected in the ticket.)
- Replied to all 4 open tickets (ليان ×2, الهنوف, لمياء) via the two-way ticket system. Verify: esbuild --packages=external resolution clean on all 4 files; full Vite build green. Shipped direct to main via GitHub Trees API. لمياء's iPhone case may still need a device-side check (silent switch / Bluetooth) — flagged to Dr. Ali.

### 2026-06-08 — SPELLING LAB (مختبر الإملاء): fix "same 10 words" repeat + timed-recall mode + progression map + activity/strength reports
- Owner (session "BUG 2"): the Spelling Lab re-served the SAME 10 words on "جلسة جديدة"; the student felt lost (no progression map); wanted a TIMED flashcard mode (see word → it disappears → write from memory → peek again), a "what have I done since the beginning" report, and a weakest/middle/strongest report.
- **Root cause of the repeat (proven):** `SpellingSession.jsx` summary `onAgain` reset the counters but REPLAYED the in-memory `words` array — it never re-called `spelling_lab_select_session`; the loader only ran on `[mode]`. The server selector itself was fine (it excludes already-seen words). Fix = a `reloadKey` epoch that re-runs the loader on "جلسة جديدة", hardened with a server-side anti-repeat guard.
- **Migration `20260608180000_spelling_lab_reports.sql` (applied via Mgmt API; additive only, NO schema changes):**
  - `spelling_lab_select_session` upgraded to `(p_mode, p_size, p_exclude uuid[])` — excludes the just-finished set + prefers least-recently-seen in the absolute fallback, so back-to-back sessions never overlap. DROPped the old 2-arg signature first to avoid a PGRST203 overload (the dm_list_contacts lesson). Verified overlap=0; the live 2-named-arg call still resolves via the default.
  - `spelling_lab_select_weak(p_size)` — focused weakest-words drill (accuracy < 0.85, not mastered).
  - `spelling_lab_overview()` — level / mastered / due / remaining-at-level / to-next-level / current_streak_days for the home map.
  - `spelling_lab_activity_report()` — lifetime totals + a 30-day daily series + recent words + current/best streak (Riyadh days).
  - `spelling_lab_strength_report()` — per-word accuracy buckets (weakest/middle/strongest). All SECURITY DEFINER, auth.uid()-scoped, GRANT authenticated. Validated under simulated student JWT claims.
- **WS3 timed recall (REPLACES the old see-mode, owner-confirmed):** `see_retype` now shows the full study card with a countdown ring (default 5s, student-adjustable ٣/٥/٧ ث, persisted to `localStorage fluentia:spellingPeekSecs:<studentId>`) → auto-hides → write from memory → "انظري مرة أخرى" peek re-reveals then re-hides. "اكتبها الآن" skips the wait. Home mode-2 copy → "تذكّر الكلمة / ادرسيها ثوانٍ، ثم تختفي لتكتبيها من الذاكرة".
- **New screens (Constellation vc-* / VocabShell reuse):** `SpellingProgressMap.jsx` (home journey card — level→next-level bar + words remaining + CTA), `SpellingActivityReport.jsx` (lifetime tiles + lightweight 30-day strip + recent words), `SpellingStrengthReport.jsx` (3 buckets + "تدرّبي على كلماتكِ الضعيفة" → launches a weak-words session via source='weak'). Home reworked: map + two drill modes + "تقاريري" section.
- **Verify:** anti-repeat overlap=0 (mock A1 student); all 5 RPCs validated under student claims; full Vite/Rollup build green; esbuild --bundle --packages=external resolution clean on all 5 files. Shipped direct to `main` via GitHub Trees API (working branch is ~108 behind; prod = main). Files: migration + `src/pages/student/SpellingLab.jsx` + `src/pages/student/spelling-lab/{SpellingSession,SpellingProgressMap,SpellingActivityReport,SpellingStrengthReport}.jsx`.

### 2026-06-06 — TEACHER ACCOUNT TOTAL REBUILD (ground-up "مساحة المدرّب") — shipped to production
- The old trainer surface (half-built v2 cockpit + ~25 `.legacy`/`.deprecated` files + abandoned Nabih/interventions/XP/payroll) was replaced by a brand-new teacher app under the SAME `/trainer/*` URLs (role stays `trainer`). Four locked decisions: curriculum control = **delivery not content**; **NO teacher impersonation** (admin-only) — instead the teacher walks the curriculum **as a student** + sees **every student's actual answers** + a **follow-daily-work dashboard by level/group**; **total UI rebuild**; roster = **manage own roster** (notes/flags/move between own groups). No push features.
- **Shell:** `src/layouts/TeacherLayout.jsx` + `teacher.css` (scoped `.teacher-app`, self-contained premium dark RTL), `src/components/teacher/shell/*`, `src/config/teacherNavigation.js` (5 items: الرئيسية/طلابي/الأعمال والتقييم/المنهج/الإعدادات).
- **Pages (`src/pages/teacher/**`):** TeacherHome (follow dashboard via `student_daily_activity`), students/StudentsList, students/StudentProfile (skills + unit_progress + activity + notes/flag + move-to-group), students/StudentAnswers (every section's question+answer+correctness+AI/trainer feedback), work/WorkReview (grading queue), TeacherSettings, curriculum/TeacherCurriculumPreview (mounts STUDENT renderer read-only, all levels). Hooks in `src/hooks/teacher/*`; answer renderer `src/components/teacher/answers/AnswerItem.jsx`.
- **Reuse:** RLS already lets a trainer read all answer tables; existing RPCs `get_trainer_grading_queue`/`approve_submission`/`request_submission_redo`, `get_student_360_overview`/`add_trainer_note`, `get_student_activity_report`, scoped `students`/`speaking_recordings`. Net-new backend = **1 RPC** `trainer_move_student` (migration `20260606120000`, SECURITY DEFINER, own-groups-only) — applied live.
- **Read-only guard:** added an inert `if (readOnly) return` at the persist entry of the Reading/Listening/Speaking/Writing/Pronunciation tabs (+ `readOnly` in CurriculumPreviewContext) so a teacher walking the curriculum can't write a student's progress. (Vocab tabs intentionally not guarded in this prod port — main is ahead of the source branch on the vocab restructure; left as a follow-up.)
- **Routing:** `/trainer/*` now mounts `TeacherLayout` + the new pages; old singular `/trainer/student/:id[...]` links repointed; every retired URL → `Navigate` to the nearest new home. Old trainer files left on disk, unimported.
- **Deploy:** this work was built on the diverged `reading-glossary-and-bug-reports` branch (also tagged `teacher-rebuild`), then surgically **ported onto production `main` via an isolated worktree** (only the teacher files + the 5 non-vocab tab guards + App.jsx route edits — NOT the branch's 18 unshipped student commits, which stay on the dev branch). Build verified green before push.
- **Verify as teacher:** admin → impersonate a trainer (e.g. د. محمد → "المجموعة 2", 11 students) lands in the new teacher app with real data (524 progress rows / 452 answered / 45 recordings / 245 daily-activity / 80 pending-grading).
- **Deferred:** hard unit/skill locks (`useSetGroupFocus` backend ready), attendance + class-announce, vocab-tab read-only guard.

### 2026-06-06 — FLUENTIA LIBRARY (مكتبة طلاقة): novels feature MVP — live for admin review
- What: New flagship feature — a library of original bilingual graded novels with the signature "veil-lift" reading interaction. Built per `docs/FLUENTIA-LIBRARY-SPEC.md`. Visible to admin in the sidebar ("المكتبة" under العمليات) at `/library` for design review before student rollout.
- DB (applied earlier): `supabase/migrations/20260606130000_library_schema.sql` — 8 `library_*` tables (books/chapters/paragraphs/sentence_pairs/reading_progress/chapter_completions/saved_vocab/achievements) + 3 SECURITY DEFINER helpers (library_student_level / library_is_staff / library_chapter_visible) + RLS. Staff (admin/trainer) read everything; students gated by academic_level.
- Content seeded: "The Silent Tide / المدّ الصامت" (A1–A2 mystery), Chapter 1, **82 fully-aligned bilingual sentence pairs** (original fiction + translation). Plus 3 cover-only "shell" books to demonstrate the three browse rooms.
- Frontend (NEW `src/features/library/`): `library.css` (the "Midnight Reading Room" — warm lamplit reader vs the cool app, Cormorant Garamond English / Tajawal Arabic whisper, typographic per-theme covers, veil-lift animation); hooks/useLibrary.js; components/{BookCover, SentenceReveal}.jsx; pages/{LibraryHome (three rooms: مكتبتي / اقرأ الفصل الأول / قريباً), LibraryBook, LibraryReader (Reveal veil-lift + Assist mode)}.jsx. Reads via authenticated Supabase client (edge functions deferred). All hooks-at-top, esbuild parse-clean (8/8).
- Modified: `src/App.jsx` (3 lazyRetry imports + 3 shared routes), `src/config/navigation.js` (admin "المكتبة" → /library). Seed tooling: `scripts/library/{build_seed_sql.py, the-silent-tide.json}`.
- Status: MVP shipped to main for Ali's in-LMS design review. Pending after approval: chapters 2+ of The Silent Tide, the remaining novels, vocab SRS, streaks/seals persistence, and audio narration.

### 2026-06-03 — BUG REPORT: guided "where + what" picker (help students point us at the exact section)
- Owner ask: when a student reports a problem, guide them to specify EXACTLY where it is (cascading: pick the sidebar area → then the sub-part, e.g. المنهج → القراءة) + what kind, so triage is fast. Plus "imagine what else to add."
- **v1 is frontend-only (fully previewable, zero production risk): NO DB migration, NO edge-function change.** The structured selections ride in BOTH (a) a short Arabic header prepended to `description` (so the section + type show instantly in the existing staff in-app notification + admin email, which are built from `description`), and (b) `device_info.context` (machine-readable for the admin page's chips + filter). The edge function already persists `description` + `device_info` verbatim, so nothing backend changed. If approved, phase 2 can promote these to real `bug_reports` columns for analytics/scale.
- **New `src/lib/bugReportTaxonomy.js`** (shared by the student sheet + admin so labels never drift): AREAS mirror the live student sidebar (المنهج/المفردات/مراجعة المفردات/الكلمات الصعبة/مختبر الإملاء/الاختبار التجريبي/IELTS/المحادثة/لوحة الشرف/الرئيسية/حسابي/أخرى), المنهج has sub-sections (القراءة/الاستماع/القواعد/المفردات/الكتابة/التحدّث/النطق/الفيديو/الاختبار القصير) and المفردات has (الرحلة/البطاقات/الاختبار). PROBLEM_TYPES (🔊 صوت / ⬛ شاشة فارغة / 🔘 زر / ❌ محتوى خاطئ / 📝 ترجمة خاطئة / 🐢 بطء / 🎨 شكل / 💡 اقتراح / ❓ آخر) + 3-level SEVERITY. `detectArea(pathname)` longest-prefix match.
- **`BugReportModal.jsx` rewritten** into a guided sheet: WHERE (area chips, **auto-detected from the current page & pre-selected** so the common path is one confirm tap) → sub-section chips (when the area has them) → WHAT (problem-type chips) → optional severity → optional details textarea → optional screenshot (paste/file kept). Submit is permissive (any of: type / text / screenshot). Gender-aware feminine Arabic copy (default-female userbase).
- **`bugReport.js`**: builds the description header ("📍 المنهج › القراءة — 🔊 الصوت لا يعمل") + enriches `device_info` with `appVersion` (best-effort `/version.json` fetch — feeds the "suspect a stale cached build first" rule), `standalone` (PWA vs browser), `online`, `screen`, `swControlled`, plus the structured `context` object.
- **`AdminBugReports.jsx`**: renders the context as colored chips (area›section / problem type / severity) above each report, adds a client-side problem-type filter row (only types present in the loaded set), and enriches the device line (viewport · platform · vN · PWA/متصفح · offline).
- **Verify:** esbuild JSX transform clean on all 4 files; authoritative build = the Vercel preview this branch triggers; decisive check = owner on device. Note: chips/filters read `device_info.context`, so pre-existing reports (without it) simply show no chips — backward compatible.
- Files: `src/lib/bugReportTaxonomy.js` (NEW), `src/components/bug-report/BugReportModal.jsx`, `src/lib/bugReport.js`, `src/pages/admin/AdminBugReports.jsx`, `CLAUDE.md`. DB: none. Edge: none.
- Status: On branch `feat/guided-bug-report` (isolated `/tmp` worktree off origin/main) → Vercel **preview** only, NOT merged to main.

### 2026-06-03 — VOCAB JOURNEY STOP: portal the full-screen session to <body> ("pressed Start, nothing happened")
- Owner report (impersonating سارة, with screenshot): pressing **ابدئي** on a constellation stop on المفردات felt unresponsive. The session DID open — it's the dark "annual / كلمة جديدة / اعرفي المعنى" panel — but it appeared **low in the page, below the fold**, with the sidebar + the 4-card choice grid still fully visible above it. So it read as "not responding."
- **Root cause:** `JourneyStop.jsx` is a full-screen modal styled `fixed inset-0 z-[80]`, but it was rendered **deep inside the scrolling content column** (sibling of `VocabShell`, inside `<main>`), not portaled. A `fixed` element only pins to the viewport when no ancestor establishes a containing block; the screenshot proves one does at runtime (a true viewport overlay would cover the z-30 sidebar — but the sidebar stays visible), so the `inset-0` was resolving against an ancestor box and the flex-centered card landed below the visible area. There was also no scroll-into-view on open.
- **Fix (1 file, `src/components/vocab-cosmos/journey/JourneyStop.jsx`):** wrap the returned modal in `createPortal(…, document.body)` so it's a direct child of `<body>` — immune to ANY ancestor transform/filter/backdrop-filter/contain — and reliably covers the viewport every open. Added a mount `useEffect` that sets `document.body.style.overflow = 'hidden'` while open (restores prior value on close) so the page behind can't be left mid-scroll. No logic / data / DB / props changed — purely where it mounts in the DOM. Mirrors the existing portaled-primitive pattern (`Popover`/`BottomSheet`).
- **Verify:** esbuild JSX transform clean (syntax valid). Authoritative build = the Vercel preview this branch triggers; decisive check = owner on device (press ابدئي → session jumps front-and-center). Note: exact trapping ancestor not pinned statically (clean LayoutShell chain) — DevTools could name it, but the portal fix resolves it regardless of which ancestor it is.
- Files: `src/components/vocab-cosmos/journey/JourneyStop.jsx`, `CLAUDE.md`. DB: none. Edge: none.
- Status: On branch `fix/vocab-journey-stop-modal` (isolated `/tmp` worktree off origin/main, parallel session's tree untouched) → Vercel **preview** only, NOT merged to main.

### 2026-06-03 — VOCAB "PATH OF LIGHT" → HUB: fix the empty-space bug + restore student choice
- Owner feedback on the new المفردات journey (with screenshot): (1) a **giant empty dark area** at the top + empty space below, (2) "don't force them to only have one choice (play only the journey words) — give them the other choices we already had, and never remove their progress," (3) keep it a NEW structure + design, more attractive.
- **Root cause of the void (confirmed against CSS):** `.vc-nebula` is a 60vw / up-to-620px block with NO `position` of its own — it only becomes `absolute` via the `.vocab-cosmos .vc-field > *` rule. I'd reused `vc-nebula` as a decorative bloom *inside* two `vc-card`s (the sky-summary + `NextStopCard`), so each rendered as a ~620px in-flow block with the real content shoved to the bottom → the giant empty card. **Fix:** added `style={{ position:'absolute' }}` to both nebula divs → cards collapse to real content height (sky-summary now 155px, was ~620px). Verified via headless measurement.
- **Restored choice (the real ask):** rebuilt المفردات from "only the journey" into a **hub** (`src/pages/student/vocabulary/VocabJourney.jsx`, wrapped children in `space-y-4`): compact "سماؤك" progress hero → **رحلة النور · المسار الموصى به** (the journey trail + "ابدئي" next-stop, now framed as ONE recommended way) → **طرق أخرى للتعلّم — اختاري ما يناسبك**: a new `VocabChoiceGrid.jsx` 2×2 of the modes they always had — تصفّح وتدرّب (→/student/flashcards), مراجعة اليوم (→/student/srs, +due badge), الكلمات الصعبة (→/student/hard-words), مختبر الإملاء (→/student/spelling-lab). The choice grid renders **unconditionally** (even on journey load/error) so the journey is never the only option. **Zero progress touched** — every mode reads/writes the same unified `vocab_cards` store; nothing deleted, no DB change.
- **Verify:** `npm run build` green. Headless (mock-test-a1, laptop 1366×900): first card 155px (void gone), 4 choice cards + the ابدئي CTA render, **0 console errors**; full-page screenshot confirms the page now fills top-to-bottom and is attractive (`/tmp/vocab-hub-laptop.png`).
- Files (new): `src/components/vocab-cosmos/journey/VocabChoiceGrid.jsx`, `scripts/audits/vocab-restructure/shot-jfix.cjs`. Files (modified): `src/pages/student/vocabulary/VocabJourney.jsx`, `src/components/vocab-cosmos/journey/NextStopCard.jsx`, `public/version.json`, `CLAUDE.md`.
- DB: none. Edge Functions: none. Status: Complete + build-green + screenshot-verified → merging to `main` (ref-only) → production.
...
- **DB (additive, applied live via Management API):** `20260603170000_vocab_journey.sql` — two SECURITY-DEFINER RPCs over `vocab_cards`:
  - `vocab_get_journey(student)` → the trail. Two-tier model: a **region** = a themed curriculum unit (`curriculum_units.theme_ar`); within it, **constellations** = ~12-word stops (curriculum_vocabulary chunked by tier/sort order). Returns regions (with per-region mastered/total + status complete/current/in_progress/available) + the current stop + headline stats (words_known, due_count). **Windowed to the student's working band** (`level-1 .. level+1`) so advanced students don't start at L0 basics. Fast (~1s for a 1,313-word student via a single LEFT JOIN on mastered words, not a per-word correlated subquery).
  - `vocab_get_stop(student, unit_id, constellation_index)` → the composed micro-session word list: the constellation's new/learning words + up to 4 folded-in due reviews (from anywhere) + up to 2 struggling "repair" words, each enriched (def/example/audio/ipa) with the student's existing card.
- **Service:** `src/services/vocab.ts` gains `getJourney` + `getStop` (thin RPC wrappers). The stop's writes reuse the EXISTING proven `addCard` (vocab_add_card) + `applyRating` (FSRS update on own card + best-effort review log) — no new write path.
- **Frontend (Constellation design system reused):** `src/hooks/useVocabJourney.js`; `src/pages/student/vocabulary/VocabJourney.jsx` (the Path home: sky summary [stars known / lit constellations / due] → `JourneyTrail` [horizontal RTL waypoint trail, current pulses gold/indigo] → `NextStopCard` [the focal "ابدئي" CTA, always one clear next step]); `src/components/vocab-cosmos/journey/{JourneyTrail,NextStopCard,JourneyStop}.jsx`. **`JourneyStop`** is the heart — a full-screen flowing micro-session that builds a deduped beat queue (discover/recall), auto-plays word audio on discover, reveal-then-rate (discover = صعبة/عرفتها; recall = نسيت/صعبة/جيدة/سهلة → `applyRating`), no-regex example highlighter (avoids the old Flashcard crash), and a "كوكبة مضيئة" completion that refetches the journey → the next stop appears.
- **Navigation:** the sidebar **المفردات** item (all 3 nav configs + mobile bar) now opens the Path (`/student/vocab-journey`); the old flashcard browse stays reachable at `/student/flashcards`. The other 3 vocab items (مراجعة/srs, hard-words, spelling-lab) remain as focused tools on the same unified store.
- **Verify:** `npm run build` green (×3). DB: journey verified on an empty student (mock-test-a1 → starts at "الحياة اليومية" stop 1, 12 new words) AND a data-rich one (الهنوف, 1,313 mastered, L3 → journey focuses L2–L4, current "الدماغ والذاكرة" with 12 new + 4 folded reviews). **Playwright (dev) smoke:** journey renders all elements with **0 console errors / no error boundary**; getJourney+getStop fire 200; the stop opens, reveals, advances. The rate→write path (`vocab_add_card` + own-card FSRS `UPDATE`) **proven under mock-test-a1's real RLS** via a modal-free transaction (`selectable=1 updated=1`, rolled back). (Headless couldn't click the rate button *through* the onboarding modal — a test artifact; real students are past onboarding.) The 2 stray console 400s in the smoke are pre-existing app-wide `xp_transactions`/`submissions` HEAD queries, unrelated to vocab.
- Files (new): `supabase/migrations/20260603170000_vocab_journey.sql`, `src/hooks/useVocabJourney.js`, `src/pages/student/vocabulary/VocabJourney.jsx`, `src/components/vocab-cosmos/journey/{JourneyTrail,NextStopCard,JourneyStop}.jsx`, `scripts/audits/vocab-restructure/{test-journey,journey-smoke,diag}.cjs`. Files (modified): `src/services/vocab.ts`, `src/App.jsx`, `src/config/navigation.js`, `public/version.json`, `CLAUDE.md`.
- DB: 2 new RPCs (additive). Edge Functions: none. Status: Complete + build-green + verified → merging to `main` (ref-only) → Vercel production.
