...
### 2026-07-19 — YUSRA KHOJAH: full custom B2 course + «Insight» theme (Malak/Sara treatment)
- What: Provisioned new PRIVATE student يسرا أحمد خوجة (B2/level 4, business analyst + technical consultant, 2 classes/week) with the full custom-curriculum treatment on /student — her own themed world + complete course, mirroring Malak/Sara.
- Account: auth+profiles+students (uses_custom_curriculum=true, uses_pro_desk=false, theme_key='insight', academic_level=4, gender=female, must_change_password, temp pw Fluentia2025!) + private group «يسرا خوجة — فردي» (code YUSRA). student id 9e387f1c-ea4c-42e6-bcf4-7a9c78529d15, group 90ba5aa6-b8d3-4f8e-bb82-2a6385f9117a. custom_price=3000 PLACEHOLDER (confirm her 2/wk price).
- Course: 10 owner_student_id curriculum_units «تحليل الأعمال والاستشارات» (Discovery Call → Solution Walkthrough), each with ALL 6 skills — reading + 7 comprehension + 12 vocab + grammar (10 single-item exercises, 7 choose + 3 fill) + listening (7Q) + speaking + writing — all is_published. Authored by 10 parallel agents vs scripts/yusra-content/SPEC.md → hard-validated (scripts/insert-yusra-course.cjs) → arabic-copy-reviewer SHIP-CLEAN (fully feminine) + rls-security-auditor SAFE (owner scoping proven live). Generic L4 (12 units) untouched. Fixed 1 who/whom grading ambiguity (U5 grammar).
- Theme: new theme_key='insight' (indigo→magenta BA/analyst identity, Scene «مشهد التحليل»). Additive edits ported onto main via Git Data API (commit c362211f): design-tokens.css (--accent-insight* + html[data-track=insight]), scene.css (.scene-root insight block), LayoutShell CUSTOM_TRACKS += insight, sceneConfig SCENE_LABEL. Verified live in prod CSS bundle.
- Covers: 10 free CSS/Playwright "Insight studio" covers (curriculum-images/units/yusra-U*.png, wired cover_image_url). Audio: all 10 listening clips voiced FREE via edge-tts multi-speaker (curriculum-audio/listening/yusra/<id>.mp3). Welcome email sent (Resend 9b6ee497).
- Files (scripts/, NOT committed — diverged tree): provision-yusra-khojah.cjs, insert-yusra-course.cjs, deploy-yusra-theme.cjs, generate-yusra-covers.mjs, voice-yusra-listening.mjs, send-yusra-welcome.cjs, yusra-content/*.
- Status: LIVE. Pending Ali: confirm custom_price for the 2/wk plan + session scheduling.


### 2026-07-18 — IELTS READING = full 3-passage authentic tests (all 43 articles + 14 tests)
- What: Rebuilt the IELTS reading section (for الهنوف) to mirror the real Academic Reading paper — 14 full tests, each 3 passages of rising difficulty (band 5-6 → 6-7 → 7-8), 40 questions, 60 minutes, one combined band + Arabic per-answer review. Every one of the 43 articles brought to authentic Cambridge standard.
- Content: 43-passage author→adversarial-verify workflow (86 agents, 0 errors). 21 old/short/object-shaped drafts got full 820-920w prose rebuilds; the 22 already-strong kept prose but were re-questioned. Standardized per-position type mix + counts (P1=13/P2=13/P3=14) with Arabic answer_key explanations. Deterministic gate scripts/ielts/load-and-assemble.mjs (band-9-on-answers, verbatim gap answers, matching-letters-in-range, no stem-echo, exact counts, strong-prose preservation); then scripts/ielts/renumber-continuous.mjs → continuous 1-40 per test. All 43 published. scripts/ielts/verify-grade.mjs: all 14 tests grade 40/40 → band 9 via the exact app path.
- DB: NEW ielts_reading_tests table (test_number, ordered passage_ids[3] easy→hard, total_questions 40, total_time_minutes 60; RLS mirrors ielts_reading_passages). 14 tests assembled (1 band_5_6 spare). Content is DB-live.
- Frontend: rewrote src/pages/student/ielts-atelier/Reading.jsx single-passage-library → TEST-first (14 cards → 3-passage session: passage tabs + multi-group palette + one 60-min timer, mirrors Mock/segments/MockReading.jsx → combined band + per-passage breakdown + Arabic review). New hooks in src/hooks/ielts/useReadingLab.js: useReadingTests/usePassageMeta/useSubmitReadingTest (preserved upstream useCompletedReadingSessions on deploy). Route unchanged (/reading).
- Verify: full Vite build green; shipped to main via Git Data API (187385e2 → f126d6f2 → d3d7c7a4); prod alias serving. Live-verified as ielts-demo (Playwright): 14 cards render, session runs (continuous 1-40 palette, 60-min timer), drove a full correct submission → 40/40 (per-passage 13/13+13/13+14/14) → results + Arabic review; test rows cleaned up. arabic-copy-reviewer clean (fixed Western→Arabic-Indic digits on tabs نص ١/٢/٣ + meta chips ٤٠ سؤالاً · ٦٠ دقيقة).
- Status: Complete + LIVE + prod-verified. Note: single-passage reading practice is now removed (tests ARE the reading experience); the Atelier Mock still grabs first-3-published unordered (untouched).

### 2026-07-17 — «تمارين مخصّصة» → warm "worksheet" surface (Ali's artifact aesthetic) + task-as-worksheet
...
### 2026-07-17 (pm) — «تمارين مخصّصة» REFRAMED: trainer-assigned task board (NO AI generation)
- What: Ali clarified the concept — he does NOT want AI to generate exercises on this page. It's a **per-student task board** where HE (via Claude Code) hand-authors specific interactive quizzes and assigns them to a student; the student just does them (auto-graded, XP). Decisions he confirmed: format = interactive auto-graded quizzes; scope = every student (per-student boards).
- **Page reframe (`StudentExercises.jsx`):** removed the AI **forge card** + `generateMutation` (analyze-error-patterns/generate-targeted-exercises) + invokeWithRetry/RefreshCw/Sparkles imports. Hero reworded «مهامّ من مدرّبك / Your Assigned Tasks» — «مهامّ اختارها لك مدرّبك… ونتيجتك تُحسب فورًا» (gender-aware). Section «تمارينك المستهدَفة»→«مهامّك» (ClipboardList). Empty state → «لم يُسنِد لك مدرّبك مهامّ بعد» + optional «تدريب إضافي» (the general library kept as fallback practice). Stat console now assigned-only. The immersive violet Intelligence-Lab world + premium cards + ExerciseView engine (MCQ/fill/reorder grading + XP) all preserved — only the AI mechanic left.
- **Backbone = existing `targeted_exercises` table** (per-student, content JSONB questions, auto-graded, XP) — already the right per-student store; nav item `custom-exercises` is `visibleWhen:'targeted-exercises-count'` so a student sees «تمارين مخصّصة» only once they have assigned tasks. pattern_id stays NULL for manual tasks.
- **Authoring pipeline (NEW `scripts/assign-tasks.cjs`):** reads a spec JSON `{student_id|student_ids, replace_pending?, tasks:[{title,skill,difficulty,instructions,content:{type,questions:[{id,question,options?,correct_answer|accepted_answers,explanation}]}}]}` and inserts them as `status='pending'` via the Management API (token from .mcp.json). `--dry` prints SQL. WORKFLOW: Ali describes tasks in detail → Claude Code encodes the spec → run the script → they appear on that student's board. Verified end-to-end (inserted 2 demo tasks for the mock test student, screenshotted the board rendering «مهامّك» with the two cards, then deleted them — mock account left clean, 0 pending).
- Files: `src/pages/student/StudentExercises.jsx`, NEW `scripts/assign-tasks.cjs`. DB/Edge: none (uses existing table). Status: complete — shipped direct-to-main via Trees API, Vercel verified. NEXT: awaiting Ali's task specs to author ظافر's board.

### 2026-07-17 — TWO immersive page redesigns (ظافر) + nav hides («إنجليزي يومي» / «بناء الجُمل»)
- What: Ali asked to redesign «تمارين مخصّصة» AND «مسار الأعمال» from A-to-Z (his rule, now standing: NO page may have a flat/black/plain background — every surface gets an immersive world), plus two nav removals.
- **«تمارين مخصّصة» (StudentExercises.jsx) — full rebuild as an "Intelligence Lab":** immersive violet/indigo world (aurora + faint target-rings + precision grid + motes + grain + scrims, self-contained explicit-rgba so theme-proof), editorial Aref-Ruqaa gradient hero + «Personalized Training», a centerpiece AI **forge card** (concentric-target motif + glowing violet CTA that runs analyze-error-patterns → generate-targeted-exercises), a 4-tile stat console (Arabic-Indic digits), skill-filter chips, and premium per-skill-colored exercise cards (resting accent bar + medallion + tags). Completed rows get score badges. ExerciseView shell wrapped in the same world. ALL data/mutations/general-exercises/localStorage logic preserved. Gender-aware via useG (ظافر=male): also fixed pre-existing masculine-hardcoded runner strings (اختبر/ابدأ/اكتب/تحقق → useG) + neutralized a masc tail + made the score line skill-agnostic + «القرامر»→«القواعد». NEW `studentExercises.css`.
- **«مسار الأعمال» home (BizTrackHome.jsx + bizTrack.css) — immersive amber world:** the whole page now lives inside a fixed amber living-world (warm base + amber aurora incl. a viewport-center bloom so scrolled content never bottoms out to black + blueprint grid + embers + grain + soft vignette). `.bt-page` pins light text tokens (theme-proof) + softens the hero base so the world glows around it; stage/lesson stations + pillars become warm translucent amber glass that lifts off the world; gauge dial warmed; sub-12px labels bumped. Hero/gauge/spine structure kept. pointer:coarse + reduced-motion gated (Android-flicker lesson).
- **Nav (navigation.js `getNavForUser`):** «إنجليزي يومي» (everyday-english) HIDDEN from every student (route+tables kept — hide-don't-delete). «بناء الجُمل» (sentence-builder) is now NADIAH-ONLY — new `gateSentenceBuilder()` strips it for everyone except نادية القحطاني (id 0aba3164…), impersonation-safe. Both routes stay direct-URL reachable.
- design-loop: 2 build→Playwright(chromium+webkit as mock student, /bizpreview temp-unguarded for the biz shot — reverted)→premium-ui-critic→fix iterations. Critic 7.5/6 → both fixed (center-weighted world blooms + softened scrims killed the flat-black-on-scroll; biz cards → warm glass; forge motif brightened; type floors). arabic-copy-reviewer pass applied. Verified NO flat-black region remains on either page.
- Files: `src/pages/student/StudentExercises.jsx` (rewrite), NEW `src/pages/student/studentExercises.css`, `src/pages/student/biz-track/BizTrackHome.jsx`, `src/pages/student/biz-track/bizTrack.css`, `src/config/navigation.js`. DB/Edge: none.
- Status: complete — shipped direct-to-main via Trees API (built each file from CURRENT origin/main + my edits; shared-tree/parallel-session safe), Vercel verified.
...
- **Grading = rule-based only.** Prod `answerValidator` verified 17/17 against the spec spot-checks (doesn't↔does not contraction tier, curly apostrophes, case/space collapse, trailing ./? stripped, 's/isn't/is not variants, wrong answers fail). NO validator patch needed, NO AI calls.
- **Glue (B2/B3):** `StudentExercises.jsx` — learn⇄test stage toggle, `LearnSection` renderer (forms grid, formation tables, rules, signal words, worked-example cards — tokens only), per-question check mode gated on `content.check_mode='per_question'` (existing exercises untouched), seed-context card + item dividers, LTR inputs for `type:'rewrite'`, score-templated encouragement (neutral Arabic, no API). Nav: «تمارين مخصّصة» item (Target icon) gated on `visibleWhen:'targeted-exercises-count'` in `navigation.js` + count queries in `Sidebar.jsx`/`MobileDrawer.jsx` (mirrors the course-vocab-count house pattern) — appears ONLY for students with targeted_exercises rows (today: ظافر alone).
- Verified under real sessions: ظافر reads exactly 1 task/36 questions; mock student sees 0 rows + 0 cross-reads. Curriculum tables untouched; inserts only.
- Files: NEW `scripts/seed-dhafer-four-tenses.cjs`, `scripts/_verify-four-tenses.cjs`; modified `src/pages/student/StudentExercises.jsx`, `src/config/navigation.js`, `src/components/layout/Sidebar.jsx`, `src/components/layout/MobileDrawer.jsx` (all ported onto origin/main via Trees API).
- DB: 1 row in `targeted_exercises` (id `cc35b725-ffc2-4f4c-8e42-9d31d1a7e83e`). No schema changes. Edge Functions: none.
- Status: complete — data live, frontend shipped direct-to-main.

### 2026-07-16 — CURRICULUM LEVEL PAGE: immersive «رحلة المستوى» redesign (LevelUnits)
- What: Ali asked to make the curriculum level page (المنهج → level view) more attractive/premium — background, hero text, everything. Rebuilt `LevelUnits.jsx` as a SELF-CONTAINED immersive world in the Living-Atlas language: full-viewport living backdrop painted from the student's NEXT unit cover art (falls back to any unit cover → level cover → aurora room) — static blurred far depth + visible Ken-Burns near layer + level-color soft-light wash + breathing gold/cyan bloom + drifting motes + film grain + legibility scrims that never cut to flat black. Editorial hero: Aref Ruqaa ivory→gold gradient title, «رحلة المستوى» eyebrow + CEFR chip, glass stat chips, gold sheen-sweep CTA (gender-aware «ابدأ/ابدئي من حيث توقفت») + «الوحدة التالية: …» hint, gradient-stroke glowing progress ring w/ dark legibility disc, outline CEFR watermark. Brass chapter medallions (Amiri Arabic-Indic numerals + per-chapter ٠/٤ progress pill + gold hairline rules). Cinematic unit cards: graded covers (`saturate(.88) brightness(.96)` + warm tint) so neon/daylight art sits inside the nocturnal world, gold-outline ghost numerals, «وحدتك التالية» badge on the student's next unit, RIGHT-anchored glowing progress bars (RTL), hover = lift + gold border (cover-only zoom, no card scale).
- Hardening: explicit rgba colors throughout + `--cinematic-*` tokens PINNED on `.lvx-root` so shared primitives (StatusChip/EmptyState) can never leak another theme into the page; reduced-motion + `html.a11y-reduce-motion` freeze all drift; `@media (pointer: coarse)` drops the blurred far layer/grain/motes (Android-flicker lesson); cover `onError` → level-color gradient fallback; 44px touch targets; 12px type floor; custom-curriculum students (owner units) + trainer/admin preview routes reuse the same component unchanged (contracts `_useCurriculumData`/`_premiumPrimitives` untouched).
- design-loop: 3 build→Playwright-screenshot→critic iterations (chromium + webkit vs preview build as mock-test-a1). premium-ui-critic 8/10 → ALL ranked fixes applied (mobile ring 104px beside its label, token pinning, cover grading, RTL rule gradients `to left`, badge-side RTL fix, unified gold+cyan accents). arabic-copy-reviewer pass fixed 2 masculine imperatives (افتحي/ابدئي via useG). GOTCHA for future sessions: a "translucent band" visible ONLY in dpr-2 headless screenshots of this page is a CAPTURE artifact of the animated near layer (page pristine at dpr1 + with reducedMotion; `elementsFromPoint` shows no element painting it — same Ken-Burns class as the live Atlas home). Never "fix" the page for it.
- Files: `src/pages/student/curriculum/LevelUnits.jsx` (rewrite), NEW `src/pages/student/curriculum/levelUnits.css`. DB: none. Edge Functions: none.
- Status: complete — shipped direct-to-main via Trees API (shared-tree rule; parallel session's uncommitted paper-theme WIP in the local tree left untouched), Vercel deploy verified.

### 2026-07-16 — NEW PRIVATE STUDENT ظافر آل قهيدان + BUSINESS TRACK «مسار الأعمال» (gated, LIVE)
- What: Provisioned new PRIVATE student **ظافر مفلح محمد آل قهيدان** (47, male, al-quhidan@hotmail.com, auth id `f1ebe336-fe3f-428f-957e-051458c516f5`) — runs an automotive business (cars & maintenance + finance + business growth) alongside his job; wants general English + his-field English. Account: package='private' (3 حصص/أسبوع), private group «ظافر آل قهيدان — فردي» (code DHAFER, level 1 provisional, trainer د. علي), gender=male, academic_level=1 PROVISIONAL + onboarding_completed=false → he takes the in-app **placement test** on first login → Ali approves via /admin/placement-queue «اعتماد المستوى» (⚠️ then also `update groups set level=<N> where code='DHAFER'`). temp pw Fluentia2025! + must_change_password. 4 course-vocab collections (60 words) → «مفردات مقرّراتي»: uni:AUTO-CARS/AUTO-SHOP/BIZ-FIN/BIZ-GROW.
- **BUSINESS TRACK «مسار الأعمال»** — second gated, level-independent field track, cloned from the proven tech-track architecture (NOT shared tables — zero risk to أطياف's «مسار التقنية»): gate `students.uses_biz_track` (TRUE for ظافر only; grant: `update students set uses_biz_track=true where id=…`), route `/biz` + `/biz/:lessonSlug` behind `BizTrackGuard` (flag or staff), nav item «مسار الأعمال» (Briefcase) injected after «الرئيسية» via `injectBizTrack` in getNavForUser.
- DB: migration `20260716090000_biz_track.sql` (applied) — `biz_track_stages`/`biz_track_lessons` (content, authenticated read) + `biz_track_progress` (RPC-only writes via SECURITY DEFINER `biz_track_complete_lesson`, best-score keeping). **Content: 10 stages / 30 lessons** (A1→B2: أساسيات التواصل → السيارة → الورشة → خدمة العملاء → الموردون → المال والفواتير → المالية → التسويق والنمو → إدارة الفريق → إنجليزية الأعمال الاحترافية), authored by 10 parallel agents per `scripts/biz-track-content/SPEC.md` (masculine 2nd-person Arabic, riyal prices, businessman-respect tone), inserted via idempotent `scripts/insert-biz-track.cjs`.
- Verified: `scripts/_verify-biz-track.cjs` PASSED under ظافر's real session (reads 10/30, RPC completes + keeps best score, direct INSERT blocked, cross-student reads 0, test progress cleaned). Audio = device Web-Speech (same as tech track). Content passed arabic-copy-reviewer (2 name errors fixed: «القحطاني»→«آل قهيدان» stage-10, «زافر»/Zafer→«ظافر»/Dhafer stage-4).
- **SECURITY (rls-security-auditor MED finding, FIXED platform-wide):** the `students_update` RLS policy + full-column UPDATE grant let ANY student PATCH his own entitlement/subscription columns (`uses_biz_track`/`uses_tech_track`/`uses_pro_desk`/`uses_custom_curriculum`/`uses_ielts_home`/`uses_speaking_track`, `extra_curriculum_levels`, `access_expires_at` ← self-extend subscription!, `academic_level` ← self-promote, `package`, `custom_price`, `status`, `deleted_at`, …). Migration `20260716100000_students_entitlement_guard.sql` (applied) extends the existing `guard_student_account_columns()` trigger (house pattern) — Tier A admin-only entitlement/billing/lifecycle columns, Tier B staff-only (`group_id`, `assigned_trainer_id`); service paths (auth.uid() NULL) pass. Both track RPCs also hardened: require `deleted_at IS NULL` + the track gate flag; EXECUTE revoked from public/anon. LIVE-VERIFIED: student self-grant/self-extend/self-promote all blocked, benign self-updates (leaderboard/onboarding) work, admin client-side `academic_level` write (placement-queue path) works (`scripts/_verify-entitlement-guard.cjs` + `_verify-guard-admin-path.cjs`).
- Files (new): `src/components/biz-track/BizTrackGuard.jsx`, `src/pages/student/biz-track/{BizTrackHome,BizLessonPage,BizPassage,BizQuiz}.jsx` + `useBizTrack.js` + `bizTrack.css`, `supabase/migrations/20260716090000_biz_track.sql`, `scripts/{provision-dhafer-alquhidan,insert-biz-track,_verify-biz-track}.cjs`, `scripts/biz-track-content/` (SPEC + 10 stage JSONs). Modified (ported onto origin/main): `src/App.jsx` (BizTrackGuard + /biz routes), `src/config/navigation.js` (BIZ_TRACK_ITEM + injectBizTrack), `src/pages/student/CourseVocabulary.jsx` (4 ظافر catalog entries). Also preserved the volatile tech-track SPEC+JSONs into `scripts/tech-track-content/`.
- Status: DB + content LIVE; frontend shipped direct-to-main via Trees API. Credentials are Ali's to send.

### 2026-07-15 — MALAK "SCENE" UNIT (gated redesign) + content gap closed (listening ×10 + writing ×7)
- Ali asked to rethink the unit from a 6-card checklist into something better. NEW gated "Scene" unit overview for custom-curriculum students: a unit is ONE marketing scene lived front-to-back — outcome/promise header + the six skills as ordered BEATS on a copper path (read the brief → the words → the grammar move → hear the other side/listening → PERFORM = speaking capstone → follow-up/writing). Capstone stays dormant until its prerequisites are done then ignites; the live beat is the brightest card («أنتِ هنا · تابعي من هنا»). Reuses the existing skill tabs via onSelect(key) — no new tab machinery, no change to the completion denominator.
- GATING: ONE branch in UnitContent.jsx on studentData?.uses_custom_curriculum === true. Generic students are byte-identical (the false arm is the prior SmartNextStepCTA + MissionGrid JSX). Theme-adaptive via --sc-* tokens: warm obsidian+copper under html[data-track="studio"] (Malak), accent-derived neutral otherwise. NO color-mix (iOS<16.4 safe — userbase is iPhones). Design-looped to premium (premium-ui-critic 7 → refined: veiled capstone, live beat focal, threaded spine, Arabic-Indic numerals). Build green.
- CONTENT GAP CLOSED (DB, live): every one of Malak's 10 units now has all 6 skills — added 7 writing tasks (the missing units) + 10 listening "hear-the-other-side" voicemail scenes (transcript + before_listen + 7 non-obvious questions each, model answers on writing). Authored by a 4-agent fleet, validated, inserted via scripts/insert-and-voice-malak.cjs + scripts/malak-content/*.json. Fixes the dead-end where listening(0)/writing(3) blocked units from ever reaching 100%. Listening audio 7/10 voiced via ElevenLabs (single voice, gender-picked); units 8-10 pending a tiny ElevenLabs top-up (`node scripts/insert-and-voice-malak.cjs --voice-only` finishes them).
- Welcome email sent to Malak (send-email edge fn, Resend id 20977aed…; temp password verified before sending).
- Files: NEW src/pages/student/curriculum/unit-v2/scene/{SceneOverview.jsx, sceneConfig.js, scene.css}; src/pages/student/curriculum/UnitContent.jsx (import + isCustom + gated branch). Ported onto origin/main via Git Data API.

### 2026-07-15 — MALAK GRAMMAR OVERHAUL: 100 exercises (all 5 types) + deep structured lessons + grammar map/reference
- Ali flagged her grammar as thin ("only one question"), unstructured, obvious answers. Root cause: ExerciseCard renders only items[0], and her grammar was 1 fill_blank row/unit; the generic bar is ~10 exercises across ~5 types.
- CONTENT (LIVE in DB, no deploy needed): each of her 10 grammar topics now has 10 exercises AS SEPARATE ROWS (3 fill_blank + 2 choose + 2 transform + 2 error_correction + 1 reorder = 100 total), grounded in HER marketing scenes, non-obvious answers, correct-option positions varied. Authored by a 5-agent fleet, structurally validated (validate.cjs), independently proofread (0 wrong keys; 2 item-validity defects patched). Deep explanations rebuilt with the platform's real section types: explanation (rule + when-to-use, feminine Najdi) + examples (highlighted, from her own passage) + common_mistakes (dedicated ✗→✓ card). Scripts: scripts/insert-malak-grammar.cjs + scripts/malak-grammar/{u*.json, explanations.cjs, validate.cjs}.
- REFERENCE/MAP (frontend, this deploy): NEW GrammarMapPanel.jsx at the top of GrammarTab — her whole grammar arc (done ✓ / current ● «أنتِ هنا» / upcoming) from her owner units' topics + completed-grammar progress; collapsible, «التالي: …» hint. Gated to uses_custom_curriculum students → ZERO effect on generic students' grammar page.
- Files: src/components/grammar/GrammarMapPanel.jsx (NEW), src/pages/student/curriculum/tabs/GrammarTab.jsx (import + render). Ported onto origin/main via Git Data API. Readings audit + personalized daily plan still to come.

### 2026-07-15 — FREE TTS: Kokoro replaces ElevenLabs for batch voicing + Malak fully re-voiced
- What: Ali heard about free TTS "better than ElevenLabs" and wants to STOP paying + rely on a free engine inside the project. Researched the 2026 landscape (WebSearch): Chatterbox-Turbo genuinely beats ElevenLabs in blind tests BUT needs a GPU/Python stack we can't run in Node/Deno scripts; **Kokoro-82M (Apache-2.0)** runs 100% locally in Node via **kokoro-js** (Transformers.js/ONNX, CPU, q8 ~86MB, no key, unlimited, free) — the one we can own/rely on. Verified it runs on this Mac (Node 26 arm64): 28 voices incl. British-female (bf_emma/isabella/alice/lily) + American-female (af_heart/bella/…); ~12–24s/passage on CPU.
- **Project integration:** NEW `scripts/lib/kokoro-tts.mjs` — reusable free-TTS module: `synthMono(text, voice) → { mono 44.1k mp3 buffer (Safari-safe -ac 1), durationMs }`, `getKokoro()` (cached model), `listVoices()`. This is our ElevenLabs replacement for ALL batch voicing going forward. NOT added to package.json (avoids onnxruntime-node postinstall on Vercel + it's not imported by src/ so never bundled); scripts document `npm i kokoro-js`.
- **Malak fully voiced (free):** `scripts/voice-malak-kokoro.mjs --voice bf_emma --force` re-voiced her ENTIRE account — all 10 unit passages + 90 vocab — with Kokoro **Emma (British)**, overwriting the ElevenLabs files at the same storage paths (URLs stable), voice_id→`kokoro:bf_emma`. (One transient upload "fetch failed" on U1 passage → re-run fixed it.) Verified: 10/10 passages + 90/90 vocab = Kokoro, reachable HEAD 200, ElevenLabs fully replaced, consistent voice. Data prod-live.
- Published a voice-picker Artifact (4 voices reading a marketing passage, playable) for Ali to A/B vs ElevenLabs + pick — swapping voice = one `--voice` arg + re-run.
- Files: scripts/lib/kokoro-tts.mjs (NEW), scripts/voice-malak-kokoro.mjs (NEW). DB: Malak's 10 reading_passage_audio + 10 passage_audio_url + 90 vocab audio_url repointed to Kokoro (prod-live). Malak's account now 100% complete (cinematic visuals + fully voiced) — ready for the welcome email.
- Status: complete. NEXT (project-wide, when Ali wants): migrate other batch voice scripts (grammar/reading/listening/Sara/Mosab) + the runtime edge TTS off ElevenLabs/OpenAI onto Kokoro (edge runtime needs a hosted Kokoro endpoint — Deno can't run the model). See [[reference-elevenlabs-retired]].

### 2026-07-14 — «مسار نادية للتحدث»: personal speaking hub built from HER own recordings (per-student)
- What: Ali — focus on نادية القحطاني's SPEAKING individually. Studied her 5 graded speaking_recordings (Apr–May, scores 5.4–6.6): vocab is her STRENGTH (up to 7.5), grammar is her ceiling (5–6.5), and her errors repeat in 6 exact families (verb forms, articles/countability, prepositions, present-perfect, pronoun reference, tense consistency) + word precision. Built her a bespoke hub around HER real data so she's surprised the platform changed for her.
- New per-student gate: students.uses_speaking_track boolean default false (migration 20260714120000, applied to prod). نادية granted true. Reusable for any future student (author their own focus set from their recordings). Route /student/speaking-track (SpeakingTrack.jsx) + nav «مسار التحدث» (Mic) gated by requiresSpeakingTrack in Sidebar.jsx filter + a route-level guard (redirects non-granted → /student). Content: src/data/speaking/nadiahSpeakingTrack.js (her diagnosis + real SCORE_TREND + 7 FOCUS_AREAS, every drill is a mistake SHE actually made).
- Hub: warm «أهلاً بعودتكِ يا نادية» hero + her diagnosis + her score-trend chart + 6 focus cards → fix-it drills (her broken sentence → correct, self-checked via checkAnswer from sentenceBuilder.js, hearable model via Web-Speech, «ليش؟» rule). 100% CREDITLESS (no runtime Claude). Female Arabic, dark cinematic (consistent with «بناء الجُمَل»). Daily-record + live AI coaching staged «قريباً» (needs Claude key — [[reference-anthropic-credits-exhausted]]).
- Design-looped via standalone harness (premium; 0 fatal errors). esbuild resolve-check green.
- NOTE: unrelated local WIP — the «نور» light theme re-skin is NOT deployed (kept local). This deploy re-derived App.jsx/navigation.js/Sidebar.jsx from CURRENT main so نور WIP never shipped.
- Files: src/pages/student/SpeakingTrack.jsx, src/data/speaking/nadiahSpeakingTrack.js, supabase/migrations/20260714120000_student_speaking_track.sql, src/App.jsx, src/config/navigation.js, src/components/layout/Sidebar.jsx.
- Status: complete — live for نادية.

### 2026-07-15 — MALAK: FAL back → full CINEMATIC visuals (best look, in & out)
- What: FAL top-up now works on the .env key (was 403 locked on 2026-07-13). Ali: "use it, best possible outcome, in and out." Regenerated Malak's whole visual layer with **fal-ai/flux-pro/v1.1** (premium cinematic): **10 unit covers** (warm copper/amber/bronze marketing scenes — glowing growth charts, brass negotiation scale, e-commerce launch w/ gold confetti, spotlit pitch stage) + **10 reading before-read images** (clean cinematic still-lifes, no text artifacts). Covers OVERWRITE the 2026-07-13 HTML/CSS Studio covers (kept as fallback in storage) — cinematic is dramatically richer AND powers her immersive world: the Living-Atlas home + LevelUnits/UnitContent CinematicBg bleed the cover, so her home went from dark/minimal → a rich warm living world painted from U1's cover.
- Style locked to her Studio identity (warm palette, golden rim light, dark obsidian, no people/faces/text). Minor garbled screen-text on a few covers is invisible in practice (blurred as background + overlaid by the card title). New storage paths `units/malak-U*-cine.png` + `readings/malak-U*-<id8>-cine.png`; cover_image_url / before_read_image_url repointed.
- Verify: 10/10 covers + 10/10 reading images cinematic + reachable (HEAD 200). Screenshotted her real rendered world (temp must_change_password flip → restored true ✅): curriculum cards, immersive home, unit page all premium. Data written straight to prod → LIVE now, no deploy.
- Files: scripts/generate-malak-visuals.mjs (rewritten: flux-pro/v1.1, cinematic warm prompts, --preview/--force). DB: 10 cover_image_url + 10 before_read_image_url repointed to cinematic (prod-live). Updated the review Artifact.
- Status: account is now top-tier in & out — ready to send the welcome email. ONLY remaining item: units 8–10 audio (ElevenLabs char budget still ~exhausted; `scripts/generate-malak-audio.cjs` finishes it after an EL top-up). Not blocking (she starts at U1, voiced). See [[project-fluentia-student-journey]].

### 2026-07-13 — MALAK: complete her account visuals + audio (bespoke Studio covers + early-unit voicing) before the welcome email
- What: Ali topped up FAL + asked to enhance Malak Al-Kendi's account (id 28a83f30…) inside & outside the curriculum, then complete it to send her welcome email. Study: her functional account was done (10 published marketing units, all content, theme_key='studio', mission → StudioHero) but had 0 unit covers, 0 reading audio, 0 vocab audio.
- **FAL STILL LOCKED (report to Ali):** the .env FAL_API_KEY returns 403 «User is locked. Reason: Exhausted balance» — his top-up did NOT apply to this key/account. So cinematic FAL images (reading before_read_image_url) are NOT generated. `scripts/generate-malak-visuals.mjs` is ready (flux/schnell, curriculum-images bucket, warm-studio prompts) — one run finishes 10 reading images once FAL works.
- **Covers — DELIVERED (no FAL):** built her 10 unit covers the bespoke Sara/Mosab way — HTML/CSS "Obsidian-Atelier" retuned to her copper/amber **Studio** identity (warm palette + one marketing glyph per unit: Presentation/Megaphone/BarChart/Handshake/PenTool/LayoutGrid/Scale/Mail/Cart/Lightbulb), Playfair unit names, «قيادة التسويق · بالإنجليزية» kicker. Playwright→PNG→curriculum-images/units/malak-U*.png→cover_image_url. Covers power her unit cards AND the immersive home/curriculum backgrounds (they bleed the unit cover). `scripts/generate-malak-covers.mjs` (idempotent). Design-loop verified: her curriculum + home render premium & cohesive.
- **Audio — PARTIAL (budget-bound):** ElevenLabs had only 7,237/10,000 chars left. Budget-aware whole-unit-atomic voicing (Alice, mono -ac 1, upsert reading_passage_audio + mirror passage_audio_url + vocab audio_url) → **units 1–7 fully voiced (7 passages + 63 vocab)**; units 8–10 DEFERRED (need ~2,600 more chars → ElevenLabs top-up, then `scripts/generate-malak-audio.cjs` finishes them; it's idempotent). No OpenAI key in .env for a fallback.
- Verify: all assets reachable (cover/passage/vocab HEAD 200). Screenshotted her real world via a temporary must_change_password flip (restored to true in a guaranteed finally — email not sent, zero exposure). Data written straight to prod DB via service key → covers+audio LIVE now, no deploy needed.
- Files (NEW, committed for the top-up follow-ups): scripts/generate-malak-covers.mjs, scripts/generate-malak-audio.cjs, scripts/generate-malak-visuals.mjs. DB: 10 cover_image_url + 7 reading_passage_audio + 7 passage_audio_url + 63 vocab audio_url (all prod-live).
- Status: account ready to send the email NOW (covers + content + units 1–7 audio = strong first impression; she starts at U1). PENDING top-ups: (1) FAL key → 10 reading images, (2) ElevenLabs → units 8–10 audio. See [[project-fluentia-student-journey]], [[reference-elevenlabs-retired]].
...
- 5 self-checking drill types (all local, no AI): expand-a-word→full sentence, complete-the-frame, arrange word-chips (auto-checked), grow-a-fragment, fix-a-shaky-sentence. Each reveal shows a verdict WITH a «جملة كاملة ✓» full-sentence flag (the skill being trained), a hearable model (Web-Speech TTS, creditless), alternatives, and a «ليش؟» rule. Recap shows «نسبة الجُمَل الكاملة» (full-sentence rate) to watch climb over sessions. Female Arabic, RTL, dark cinematic, brand طلاقة.
- Design-loop: rendered the real component via a standalone Vite harness (no auth) → premium-ui-critic → fixes. esbuild resolve-check green; 0 fatal console errors desktop+mobile.
- NOTE: this is the creditless half of the speaking plan. The incoming-call ROLEPLAY + live AI full-sentence coaching (the other half Ali picked) needs the Anthropic account funded — it degrades to canned fallbacks while at $0 (same as Sara's Desk). Staged to light up on top-up.
- Files: src/pages/student/SentenceBuilder.jsx, src/data/speaking/sentenceBuilder.js, src/App.jsx (route), src/config/navigation.js (nav).
- Status: complete — creditless trainer live for group students.

### 2026-07-13 — MULTI-LEVEL CURRICULUM ACCESS: revisit the previous level alongside current (نادية → A2 + B1)
- What: Ali's ask — let a student live in TWO curricula at once (her current level AND the immediately-previous level, to shore up fundamentals) without hiding or losing current-level progress. Novel; built as a PRECISE per-student allowlist, NOT the blunt can_access_lower_levels (which opens ALL lower levels). First recipient: نادية القحطاني (academic_level 3 = B1) granted revisit access to A2 (level 2) → sees B1 «مستواك الحالي» (hero) + A2 «للمراجعة والتقوية» (revisit), everything else locked/hidden. Everyone else: ZERO change (empty allowlist = identical old code path).
- DB: new additive column students.extra_curriculum_levels int[] NOT NULL DEFAULT '{}' (migration 20260713120000, applied to prod via Mgmt API). Level-access is 100% FRONTEND (no RLS/level gate exists — any student can already read any unit + write own progress; the unit-page guard only blocks HIGHER levels), so this only widens which levels the curriculum UI surfaces. Nadiah granted '{2}'. To grant another student: update students set extra_curriculum_levels='{N}' where id=...
- Frontend (2 files, both based on current main — git diff origin/main showed only my edits): (1) _useCurriculumData.js per-level guard now also allows a level listed in extra_curriculum_levels (she can open A2's units). (2) CurriculumBrowser.jsx: an extra-grant student no longer auto-jumps past the grid; the grid shows ONLY current + granted extras as accessible (current level leads as the HERO, extras follow as «للمراجعة والتقوية» revisit cards with HONEST progress — not faked "completed"); "next on the path" locked section shows only levels genuinely AHEAD (below-and-not-granted levels hidden, never mislabelled "next"). Behaviour preserved for staff-preview (canSeeAllLevels), teacher-preview (can_access_lower_levels) + normal students (auto-jump untouched).
- Design-loop: rendered as a test student in Nadiah's exact scenario (B1+A2) → premium-ui-critic; fixed 2 loop findings (revisit card sat above the current-level hero → reordered current-first; Pre-A1/A1 wrongly shown under "next on the path" → hidden). esbuild resolve-check green; 0 fatal console errors desktop+mobile.
- Files: src/pages/student/curriculum/CurriculumBrowser.jsx, src/pages/student/curriculum/_useCurriculumData.js, supabase/migrations/20260713120000_student_extra_curriculum_levels.sql.
- Status: complete — DB column live on prod, Nadiah granted, frontend shipped to main via Trees API.

### 2026-07-13 — UNIT PAGE: fix mislabeled "record yourself" → premium «تسجيل الحصة» (class recording) entry card
- What: Ali: the unit's recording section is NOT self-recording — it's the RECORDED CLASS (staff upload the recorded live class per unit; students watch it). The `?activity=recording` slot already IS the class-recording feature — `RecordingTab` returns `<UnitRecordingsSection>` (premium player: Part A/B, thumbnails, progress, "لا توجد تسجيلات لهذه الوحدة بعد" / "اطلب التسجيل") for students and a Google-Drive upload form for staff; backed by `class_recordings.unit_id/part` (migrations 071/073, per-unit). The ONLY bug: the ENTRY CARD in `UnitContent.jsx` (from the 2026-06-20 assessment fix) was mislabeled **«سجّل أداءك / تسجيل صوتي اختياري»** w/ a Mic icon — reads as "record yourself." Ali flagged this before.
- Fix: replaced that inline card with NEW `src/components/curriculum/ClassRecordingEntryCard.jsx` — a premium gold-cinematic card: play/film crest, title **«تسجيل الحصة»**, availability-aware neutral subtitle (uses `useUnitRecordings(unitId)`, shared cache key — no extra fetch: available → «الجزء أ · الجزء ب — التسجيل المصوّر لحصتك» + gold «● متوفر» chip; none → «سيظهر هنا تسجيل الحصة عند رفعه» + muted «قريباً»), gold top hairline, hover-lift. Gender-neutral copy. Removed unused `Mic` import. Grepped whole src — zero remaining «سجّل أداءك»/self-record framing (destination player already used correct class-recording language).
- Files: src/components/curriculum/ClassRecordingEntryCard.jsx (NEW), src/pages/student/curriculum/UnitContent.jsx (swap card + import). DB/edge: none (upload path already exists — staff RecordingTab form + AdminRecordings). `npm run build` green; design-loop verified both states.
- Status: complete — shipped to main.

### 2026-07-12 — PROVISION: Malak Al-Kendi — custom marketing-leader track (10 units) + Studio identity + per-student theme_key
- Who: ملاك باحشوان الكندي (malakalkendi2@gmail.com), 27, female, B1 (level 3), الفردي/private 3,000 SAR, uses_custom_curriculum, trainer = د. علي. Temp pw fluentia2026 (must_change_password=true). id `28a83f30-9474-4869-8f08-f63dc40c767d`. study_mode='group' (mirrors Sara so she gets the full curriculum nav), no group_id.
- Curriculum «الإنجليزية لقائدة التسويق»: 10 owner-scoped PUBLISHED units built ONLY from her four work domains (marketing strategy · campaign mgmt · creative production · influencer relations). Each unit pairs a real scenario with ONE embedded B1 grammar target taught inside its OWN passage (future forms / obligation modals / past-vs-present-perfect / 1st&2nd conditionals / comparatives+softening / present-cont-vs-simple / question forms+polite disagreement / linking words+formal register / passive / purpose&persuasion). Per unit: reading (2-para realistic artifact, *starred* vocab) + 5 comp Qs + 9 vocab + grammar-in-context (+fill-blank exercise) + speaking roleplay; writing on units 3/8/10. Feminine Najdi teaching voice; brand-word guard (no معهد/دورة/مذهل/مميز/استثنائية/الأفضل); audio_url NULL (follow-up). Mirrors Sara's ROW STRUCTURE only, none of her content. (50 comp Qs / 90 vocab / 10 grammar+ex / 10 speaking / 3 writing.) Scripts: provision-malak-alkendi.cjs, malak-curriculum.cjs, insert-malak-content.cjs, verify-malak.cjs.
- DB (already LIVE in prod): auth + profiles + students rows; NEW nullable `students.theme_key` (migration add_students_theme_key) → Sara + Mosab backfilled 'fardi' (pins their purple), Malak 'studio'. Zero other students touched (verified 0 leak).
- Studio visual identity (NEW, scoped, zero leak): warm copper/amber, distinct from fardi purple + default sky at a glance. `design-tokens.css` --accent-studio* + `html[data-track="studio"]` remap (both --accent-sky* and --ds-* systems). `LayoutShell` now sets data-track from studentData.theme_key (fallback: uses_custom_curriculum→'fardi' so Sara/Mosab byte-for-byte unchanged; never touches the 'desk' track). `SpotlightDashboard` gained `StudioHero` (its OWN component, NOT FardiHero): mission headline + her 10 units as a CAMPAIGN PIPELINE of stages with current-stage label + breathing pulse, next private session, XP demoted to a meta row; `atlasHome.css` .studio-* classes. Render is byte-identical for every non-studio student (only theme_key='studio' hits the new branches). premium-ui-critic 8.5→premium (informational pipeline + pulse fixes applied).
- Verify (14/14, anon client under real RLS): sign-in OK, must_change_password true, exactly 10 published units 1..10, Unit 1 children all render (passage/5Q/9 vocab/grammar+content_ar/exercise/speaking 7 phrases), owner-isolation RLS (Malak can't see Sara's units), Sara 8 units + fardi unchanged. Ported onto origin/main via Git Data API (local branch is 301 behind main). Welcome email drafted for Ali to send MANUALLY (in the MASTER-PROVISION-MALAK prompt) — NOT sent by me.

### 2026-07-12 — CURRICULUM: ambient life on the immersive background (level + unit pages)
- What: Ali asked to extend the enhancement "everywhere including the curriculum." Audit finding: the curriculum shell is ALREADY fully immersive/premium — CurriculumBrowser auto-jumps to LevelUnits, and both LevelUnits AND UnitContent (`<CinematicBg>`) already render the level/unit `cover_image_url` blurred full-bleed behind the content (blur 40px + vignette + film grain + gold/cyan hairlines + Playfair/Amiri type + ProgressRing + color-coded mission cards). So NO rebuild — that would be churn/regression risk on a beloved surface. The one thing it lacked vs the Atlas home = ambient LIFE (its cinematic bg was fully static).
- Added restrained ambient motion to the shared cinematic background so the WHOLE curriculum breathes like the home: a very slow 46s Ken-Burns drift on the blurred cover (`.curr-kenburns`) + a gentle 15s breathing warm-gold bloom (`.curr-breathe`, radial rgba gold 0.10 screen-blend at top). Transform/opacity only, reduced-motion gated (per the restrained-LMS motion rule). Presentation-only — zero query/logic/structure change; every activity/progress/gate untouched.
- Files: src/pages/student/curriculum/_premiumPrimitives.jsx (CinematicBg → unit page), src/pages/student/curriculum/LevelUnits.jsx (inline bg → level page), src/styles/animations.css (NEW `curr-kenburns`/`curr-breathe` keyframes, globally imported). DB/edge: none. `npm run build` green; static render verified intact (level + unit).
- Status: complete — shipped to main. Note for Ali: the rest of the curriculum (reading/listening/vocab/writing/grammar skill tabs) is already premium from prior passes; if a SPECIFIC skill surface feels weak, point at it for a targeted deep pass rather than a blind restyle.

### 2026-07-12 — STUDENT DESIGN: نادي التحدّث + دفتر عباراتي rebuilt into the gold-cinematic world (design-loop, critic-passed)
- What: Ali asked to enhance the design of the standard-curriculum student account. Screenshot audit (mock-test-a1, mobile+desktop) found MOST student surfaces already premium (Atlas home, رحلتي, cosmos vocab, violet SRS, gold spelling-lab, teal everyday-english, Library, chat) — the two genuinely-legacy "cold sky" holdouts were **نادي التحدّث** (a lone icon on flat black) and **دفتر عباراتي** (one flat blue card). Rebuilt both into the same warm gold-on-obsidian world as [[profileJourney]]/atlasHome.
- **نادي التحدّث (StudentSpeakingHubs.jsx + NEW speaking-hub/speakingHub.css `.shx-*`):** atmosphere beam+drifting blob (reduced-motion+coarse gated), cinematic hero with a live-pulse crest, a 3-step "club ritual" (شاهدي·دوّني·انضمّي, gendered via useG), a warm INVITATION empty state (the common view — pulsing gold orb + trimmed copy + hint), and cinematic 16:9 session cards (gradient scrim + frosted-glass play button + watched/live corner stamp + countdown/notes chips). Fixed a real bug: hero title i18n was «نادي المحادثة» colliding with the separate «المحادثة» chat item → set student.speakingHub.list.title = «نادي التحدّث» (matches nav).
- **دفتر عباراتي (StudentPhrasebook.jsx + NEW phrasebook/phrasebook.css `.pbx-*`):** gold hero with live stat pills (count + voiced-count, Arabic plural «٣ عبارات»), gold-spark eyebrow, collectible "phrase-gem" cards (colored category spine, struck-through قلتِ ↔ bold native الأصح, 44px gold trainer-voice play button that pulses while playing, sparkle note). Re-warmed the category palette (gold/rose/teal/amber) to KILL the cold sky-blue/violet so the page reads as one gold world.
- Design-loop: built → screenshot (390+1440, empty+populated via REST intercept) → premium-ui-critic (empty 4.5–6.5 → applied all fixes: desktop max-width, single live indicator, frosted play, warm palette, 44px targets, no-scale hover, ≥12px text, crest→inline-start, stronger atmosphere) → re-screenshot verified. Motion transform/opacity only, iOS-safe rgba. `npm run build` green; both CSS code-split per page (9.1/8.4 kB).
- Files: src/pages/student/StudentSpeakingHubs.jsx, src/pages/student/speaking-hub/speakingHub.css (NEW), src/pages/student/StudentPhrasebook.jsx, src/pages/student/phrasebook/phrasebook.css (NEW), src/i18n/ar.json (1 title). DB: none. Edge: none.
- Status: complete — shipped to main. Next per Ali: extend enhancements to more surfaces incl. the curriculum.

### 2026-07-11 — مصعب: FULL 12-unit business-major curriculum (depth build via 12 parallel agents)
- What: Expanded مصعب's custom curriculum from 5 units (1 reading each) into a real **12-unit A2→B1 business-major English course** with a coherent grammar arc. Added 7 NEW units (Marketing / Customer Service / Money & Budgeting / Teamwork / Workplace Communication / Presentations / Career Growth) + a **2nd reading (B) + a listening** to each of the original 5. Authored by **12 parallel general-purpose agents** against a strict JSON spec (scripts/mosab-content/SPEC.md); inserted deterministically with per-file validation (correct_answer ∈ choices, listening index bounds, question/vocab counts). Business is the VEHICLE (we teach English), A2 level, masculine Arabic throughout.
- Totals: **24 readings · 168 comprehension MCQs · 247 curriculum_vocabulary · 12 grammar points (+36 'choose' exercises) · 12 listening activities (ALL voiced via ElevenLabs, ~3.3k chars) · 12 writing · 12 speaking · 12 Obsidian-Atelier covers**. Grammar arc: present simple → prepositions → frequency → past simple → can → present continuous → past irregular → much/many → comparatives → will/going to → should/have to → present perfect.
- Data-only (custom-curriculum + listening-player + cover rendering already deployed; no frontend change). Verified (verify-mosab-full.cjs): content completeness per unit, student-role RLS resolution (Mosab sees 12 units, U10 full content resolves), listening audio public HEAD 200. QA: 0 feminine-Arabic slips, 0 brand transliteration, content spot-check clean.
- Files: scripts/mosab-content/ (SPEC.md + unit-06..12.json + supp-01..05.json), scripts/insert-mosab-content.cjs, scripts/generate-mosab-listening-audio.cjs, scripts/verify-mosab-full.cjs, scripts/generate-mosab-covers.mjs (extended to 12 courses). DB: +7 curriculum_units + 24 readings + 168 questions + 247 vocab + 12 grammar/36 exercises + 12 listening(audio) + 12 writing + 12 speaking; 12 covers in storage/curriculum-images; 12 mp3s in storage/curriculum-audio. ElevenLabs remaining ~4.8k chars. See [[project-fluentia-mosab-course-vocab]].

### 2026-07-06 — STUDENT JOURNEY «رحلتي»: account surface rebuilt as the identity flagship + student nav IA reorganized (design-loop, 2 critic passes → PREMIUM 8.5)
- What: Ali asked for a breakthrough pass on the student accounts/design/organization. Study phase ran a full screenshot audit of every student surface (mock-test-a1, 10 pages × desktop+mobile): the app is 3 design eras — Atlas/Library/chat are cinematic and approved, SRS/spelling-lab are gold-era, but /student/profile (حسابي) was the OLDEST surface (cold sky, 7 cramped SubTabs, dead "الترتيب —" stat from hidden leaderboards) and التعلّم nav was an 11-item flat wall. This pass rebuilt the account into the student's cinematic identity surface and reorganized the IA.
- **NAV IA (STUDENT_NAV, sections + drawerSections mirrored):** التعلّم wall → رحلة التعلّم (الرئيسية/المنهج/إنجليزي يومي/IELTS) · التدريب اليومي (المراجعة اليومية/الكلمات الصعبة/المفردات/مختبر الإملاء/دفتر عباراتي) · عالم طلاقة (المكتبة/نادي التحدث/المحادثة) · الاختبارات · حسابي (label رحلتي وملفي). Renames: مراجعة المفردات اليومية → المراجعة اليومية (was truncating in the 264px rail), نادي المحادثة → نادي التحدث (was confusable with المحادثة sitting adjacent). mobileBar untouched (owner-locked 5 slots). All routes/gating (requiresPackage/badges/visibleWhen) preserved.
- **رحلتي (/student/profile), new .sj-* system in src/pages/student/profile/profileJourney.css:** warm gold atmosphere (transform-only drift, coarse-pointer + reduced-motion gated); identity hero = gold-framed avatar w/ 46px-hit camera + upload/delete sheet (logic MOVED from the old body — handlers verbatim), name/email/CEFR·package·joined chips, animated SVG XP ring toward the next gamification level, story gems (gold XP · streak — 0 becomes the gendered CTA «ابدئي سلسلتك اليوم» · group rank when available · CEFR; no-rank order puts streak last so the mobile full-width slot lands on it); sliding gold tab pills (framer layoutId, 7 chapters reordered to tell a story). ملفي body under gold spark eyebrows: بيانات الحساب (2-col: display name + username, gold links) / التفضيلات (Immersion + NEW NotificationDisclosure — the 9-accordion NotificationSettings wall now lives inside ONE collapsed sj-card) / الأمان (password card, trigger «تحديث الآن») / سجل الرحلة (achievements w/ EMPTY-STATE design: 0-earned students see 6 tiles + full-color «أقرب إنجاز لك ✦» + عرض الكل toggle instead of a 20-tile locked graveyard; gold XP log). Scoped --accent-sky→gold remap so reused in-body toggles keep the world. [data-theme="minimal"] light overrides shipped AND pixel-verified.
- Design-loop: 2 premium-ui-critic rounds (7/10 → 11 fixes → 8.5/10 PREMIUM, no regressions) + a residuals pass (2-col account card, gem order, tab fill, light-theme shot). Verified desktop 1440 + mobile 390 + drawer + light theme, 0 new console errors.
- **Follow-up roadmap (from the audit, next passes):** شهاداتي/الأفاتار/الفواتير/دعوة صديق sub-tabs still legacy-cold inside the new shell; المفردات (flashcards) page + نادي التحدث empty state are the weakest remaining sky-era student surfaces; AppearanceContent theme cards could go gold-framed.
- Files: src/config/navigation.js (STUDENT_NAV IA), src/pages/student/StudentProfile.jsx (shell + IdentityHero + reorganized ملفي), src/pages/student/profile/profileJourney.css (NEW). DB: none. Edge: none.
- Status: complete — shipped direct-to-main via Trees API, deploy-verify follows.


### 2026-07-06 — BRAND: official Fluentia logo across the platform + real PWA app icon
- What: Integrated the official brand logo (from Ali's brand PDF, extracted vector-accurate via poppler) into the LMS. NEW assets in `public/brand/`: `fluentia-mark.svg` (blue feather-F, tight viewBox), `icon-512/192.png` + `apple-touch-icon.png` (full-bleed navy tile + official white F composed at 1024px with maskable-safe padding, bilinear-sampled from the brand tile — replaces the artifacted single-file logo-icon-dark.png used for every size). `public/logo-icon-dark.png` overwritten with the clean 512 render (boot screen + any legacy refs upgrade for free).
- Placements: student Sidebar brand medallion (was a placeholder gradient «ط» square → now the real mark on a glass medallion), TeacherSidebar brand (dot → mark), LoginPage lockup (mark above the FLUENTIA ACADEMY eyebrow), index.html favicons (SVG + 192 png + 180 apple-touch), vite.config.js PWA manifest icons → /brand/* (192, 512, 512-maskable) — THIS is the home-screen icon students get when installing the PWA.
- Brand rules honored: mark sits BESIDE the طلاقة wordmark text (never replaces Arabic); navy tile only for icon contexts. Same asset set as the marketing site (fluentia-site public/brand — masters documented there).
- Verified: local build green, manifest emits /brand icons, login + sidebar render the mark with 0 new console errors (mock-test-a1 login; the submissions.voice_url 400 is the known pre-existing drift).
- Files: index.html, vite.config.js, src/components/layout/Sidebar.jsx, src/components/teacher/shell/TeacherSidebar.jsx, src/pages/public/LoginPage.jsx, public/brand/* (4 NEW), public/logo-icon-dark.png (replaced)
- DB: none. Edge Functions: none.
- Status: complete — shipped direct-to-main via Trees API (ported onto main's file content, all anchors matched).


### 2026-07-05 — ADMIN PEOPLE HUB: /admin/users rebuilt premium (design-loop, 2 critic passes) + groups-page silent-breakage fix
- What: Ali asked to "organize + enhance the students pages" after the Operations Bridge dashboard/sidebar. Full presentation rebuild of the 4-tab hub (الطلاب / المجموعات / المدربون / تحديث البيانات) on the adx system + NEW `src/pages/admin/adminPeople.css` (.adp-* companions: hub tabs w/ live counts + sliding gold pill, status-strip chips that ARE the filter, toolbar, premium table, glass modals w/ sticky gradient footers + gold accent-color checkboxes, group/trainer cards, mobile roster cards). ALL queries/mutations preserved; data changes are additive only.
- **Roster (الطلاب):** one fetch + client-side filtering (instant), status chips w/ counts as the filter (status column auto-hides when a filter is active), hide-test-accounts toggle (default ON — tab count matches via profiles!inner is_test_account=false), NEW columns آخر دخول (relative, Arabic dual/plural correct, churn = rose not amber) + avatar (neutral glass — no sky/violet in the bridge), name click → /admin/student/:id/progress, NEW per-row تقرير النشاط action → /admin/student/:id/report (was unreachable from this page), gold discipline = XP is the single gold datum per row. <640px renders a real card list (adp-mcard), not a squeezed table; 640-900px table gets scroll-edge fade + thin scrollbar; 44px targets on coarse pointers.
- **BUG FIX (silent, pre-existing): /admin/groups showed 0 groups in prod.** groups.trainer_id FK was repointed to the `trainers` extension table (coordinator/HR work), so the legacy bare `profiles:trainer_id` embed 400s (42703 "column trainers_1.full_name does not exist") and the page's error-swallowing `const { data }` rendered an empty grid. Fixed via `trainer:trainers!groups_trainer_id_fkey(profiles(...))`; the assignment dropdown now sources from `trainers` (is_active) so it can never offer an id the FK rejects. NOTE: AdminSmartScheduling.jsx still has the same broken embed (likely superseded by /coordinator — flagged, untouched).
- **Groups tab:** N+1 per-group count queries → 2 parallel queries; cards get level-tinted hairline, occupancy adx-meter with REAL max_students thresholds (was hardcoded 8/10 vs max 7), active-first sort, معطلة neutral (was red alarm), Arabic day names (الأحد…), mt-auto footers. **Trainers tab:** N+1 → 3 parallel queries; load line "N مجموعات · M طلاب" (correct Arabic agreement incl. مجموعتان/طالبان), admins under a separate الإدارة adx-eyebrow, test accounts dimmed + تجريبي chip + sorted last; edit modal group rows gold-tinted when selected. **Emails tab:** adx restyle; test accounts dimmed + chipped + sunk to bottom (protects against editing a demo email by accident).
- Design-loop: vite + magiclink admin session + Playwright (1440 fullPage per tab + 390 + modal mid-scroll), premium-ui-critic round 1 = 6.5/10 → 13 fixes → round 2 = 8.5/10 PREMIUM + 3 residuals fixed (rose churn tone, Arabic plurals, email-tab test treatment). Backlog (critic-accepted residual): shared ImpersonateButton is still a filled gold circle per row — a global ghost-until-hover pass on that component is deferred.
- Files: src/pages/admin/AdminStudents.jsx (rebuild), src/pages/admin/AdminGroups.jsx (rebuild + FK fix), src/pages/admin/AdminTrainers.jsx (rebuild), src/pages/admin/adminPeople.css (NEW). DB: none. Edge: none.
- Status: complete — shipped direct-to-main via Trees API, deploy-verify follows.


### 2026-07-05 — ADMIN SHELL: nav reorganized + "operations rail" premium sidebar (design-loop, 2 critic passes)
- What: follow-up to the Operations Bridge dashboard — Ali asked to enhance the admin sidebar and "put everything in order". ADMIN_NAV regrouped from 4 sections (incl. a 13-item العمليات dump) into 6 purposeful ones: نظرة عامة / الطلاب والفريق / المالية والنمو / المنهج والمحتوى / التواصل والمجتمع / الجودة والنظام. ALL 28 destinations preserved (regroup, zero removals). Label/icon dedup: معاينة كطالب (Eye) + معاينة IELTS (Award), آراء المكتبة → Star, الشركاء → Handshake, المنهج التفاعلي → Layers, الموظفون → Briefcase, ⚔️ emoji dropped. Admin mobileBar slot swapped المجموعات → المالية (groups stay in the More drawer, which reads sections directly).
- Sidebar.jsx: admin-only `dense` operations-rail treatment (students' rail untouched): 40px rows / 30px icon boxes / 17px icons / 13px labels, adx gold-spark+hairline eyebrows (12px, no letter-spacing on Arabic), brand caption "غرفة العمليات" under the wordmark, gold chat-unread badge (indigo kept for students), scroll-edge fade mask + 4px scrollbar for the long list, collapsed rail keeps section structure via 24px hairlines, staff role label in the user card (إدارة الأكاديمية/مدرب/خدمة العملاء/تنسيق الحصص replaces the empty level/XP line), active-pill gradient re-anchored `to left` (RTL leading edge), hover 170ms + inset border ring, settings-gear hover state, rail height 100dvh.
- Mobile: MobileDrawer admin branch renders the same spark+hairline eyebrows; MobileBar admin chat badge is gold w/ Arabic-Indic numerals (students keep the red convention).
- Design-loop: local vite + magiclink admin session + Playwright (1440 + 390, expanded/collapsed/active/drawer states), premium-ui-critic round 1 = 7/10 → all 9 fixes applied → round 2 verified. Collapsed-brand "clipping" disproven by geometry probe (tile y=18 in 76px block).
- NOTE for parallel sessions: Sidebar/MobileBar/MobileDrawer/navigation.js/LayoutShell in the SHARED local tree were synced to main content first (they were stale — missing vocab.ts/vocabFormat.js were also materialized from main, unchanged, not part of this commit).
- Files: src/config/navigation.js (ADMIN_NAV regroup), src/components/layout/Sidebar.jsx (dense rail), src/components/layout/MobileBar.jsx (admin badge), src/components/layout/MobileDrawer.jsx (admin eyebrows). DB: none. Edge: none.
- Status: complete — shipped direct-to-main via Trees API, deploy-verify follows.


### 2026-07-04 — ADMIN DASHBOARD "Operations Bridge" premium redesign (design-loop, 2 critic passes)
- What: Ali asked to "enhance this page" (the /admin home). Full presentation-layer rebuild — ALL queries + sub-widgets preserved, zero DB/edge changes. New scoped `.adx-*` system in `src/pages/admin/adminDashboard.css`: full-bleed obsidian-steel atmosphere w/ one warm gold beam (transform-only drift, reduced-motion + pointer:coarse gated per the Android-flicker rule), greeting hero (gold-gradient name, western-numeral Arabic date, live status line) with 4 DATA-AWARE pulse chips (revenue chip hidden while the payments table is dead — chips fall back to students/groups/seats/app-installs), merged "يحتاج انتباهك" attention rail (pending payments + system errors + renewals + not-installed, count in the eyebrow; replaces the old quick-summary + إجراءات معلقة; dropped the deprecated weekly-tasks action), then editorial sections نبض الأكاديمية / المالية / الطلاب والمجموعات / النظام with gold-spark eyebrows.
- Anti-log-dump pass (critic round 1 → 5.5/10, round 2 verified): DeviceInstallStatusWidget (~1300px wall) now collapsed behind a designed summary disclosure (meter + count + "عرض التفاصيل"); the duplicate slim PWA card was removed; نشاط الطلاب اليوم shows ONLY students with signal (2-col grid, presence dot ON the avatar) + one "و N لم يدخلوا اليوم" line, with a designed هدوء empty state; finance empty state replaces the big dead "0 ر.س"; money is GOLD (renewal amounts, package bars) instead of cyan; EvaluationHealthWidget rows became inline pill badges (label+value adjacent, no 450px void) + "لم يُفحص بعد" fallback; AtelierLauncher restyled to the adx surface w/ auto-fit grid (fixed 390px clipping).
- Verified via design-loop: local vite + magiclink admin session + Playwright shots at 1440×900 and 390×844, premium-ui-critic two rounds; page height 6573px → 3766px; 0 new console errors (only a pre-existing 404 resource).
- Files: src/pages/admin/AdminDashboard.jsx (rewrite, queries verbatim), src/pages/admin/adminDashboard.css (NEW), src/components/admin/EvaluationHealthWidget.jsx (pill rows), src/pages/admin/atelier-preview/AtelierLauncher.jsx (restyle). DB: none. Edge: none.
- Status: complete — shipped direct-to-main via Trees API (shared-tree rule), deploy-verify follows.


### 2026-07-03 — PRO DESK (مركز العمليات): gated professional-English surface for 1-on-1 learners (Stage 0-1)
- What: reimagined the individual/1-on-1 account (Sara) from "a curriculum" into a professional-English TOOL — a SEPARATE gated `/desk/*` route space ("Operations Room", brass-on-obsidian, RTL) with its OWN shell + sidebar. NOT edits to the shared student pages, so سلطان + every other student are untouched.
- Gate: NEW `students.uses_pro_desk` boolean (default false → nobody sees it until flipped). Orthogonal to uses_custom_curriculum + study_mode (the two real 1-on-1 students don't share a flag). Migration `20260703120000`. Currently ON only for the IT demo نورة (`indiv-demo-it@fluentia.academy`).
- Surfaces: DeskToday cockpit (current-mission hero + live INCIDENT viewport + NOC atmosphere canvas), DeskScenarios (the 10 authored `specialization_modules` finally given a reader), DeskScenarioPlayer (brief/vocab/phrases + in-character voice roleplay + writing). Bespoke DeskShell/deskNavigation/DeskGuard/NocBackdrop.
- Data: `desk_module_progress` + 3 SECURITY-DEFINER RPCs (desk_start_scenario/complete_stage/mark_roleplay; student=auth.uid(); RLS student-own-select, writes RPC-only). Migrations `20260703120100` + hardening `20260703120200` (revoked anon exec; completion only via graded roleplay; assigned_trainer read). rls-security-auditor verdict: SAFE. arabic-copy-reviewer: brand طلاقة ✓, 7 gender fixes applied via useG.
- Roleplay engine: NO edge-fn change — main's `speaking-conversation-turn` ALREADY supports `module_id` (persona from `specialization_modules.roleplay`, incl. Bangalore accent via ai_role) and ConversationMode already had the `moduleId` prop; only added `{conversationId}` to its onComplete so DeskScenarioPlayer can call desk_mark_roleplay.
- Design-looped 2 rounds (premium-ui-critic): flat-black → immersive Operations Room (receding rack aisle + status LEDs + haze), live incident panel, brass CTA, richer cards, mobile-nav fix.
- Files: NEW `src/pages/desk/{DeskToday,DeskScenarios,DeskScenarioPlayer,useDeskModules,desk.css}`, `src/components/desk/{DeskShell,DeskGuard,NocBackdrop}`, `src/config/deskNavigation.js`, 3 migrations. Modified: `src/App.jsx` (/desk routes + RoleRedirect uses_pro_desk branch), `src/styles/design-tokens.css` (`data-track="desk"` brass), `src/components/curriculum/speaking/ConversationMode.jsx` (onComplete conversationId).
- Verify: RPCs + RLS forge-tested live as نورة; 0 Desk console errors; build green; ported onto origin/main (`0cd0a27`) via Trees API (edge fn deliberately EXCLUDED — main's is newer). Status: Stage 0-1 shipped GATED. Later stages (Say-it, Rehearsal Room) pending + need Anthropic credit top-up. Sara migration = flag flip when Ali approves.


### 2026-07-03 — IELTS HOME REDESIGN: "The Instrument" (Ali rejected the sunset Atelier design)
- What: Ali saw the live IELTS Atelier Home and rejected it outright ("didn't like it at all — design, structure, everything; delete + build completely different"). Re-hid the nav (`0428cea`), pitched 3 fresh directions as an Artifact concept board (Instrument / Ascent / Studio). He said "continue" (no pick) → I chose **① The Instrument** and, with "execute everything", built + shipped it.
- **The Instrument concept:** the whole IELTS home reframed as a precision MEASURING device, not a greeting-dashboard. Opens on the one number that matters — current band → target band on a 0–9 ruler — then four skill "instruments" (band + Δ-to-target + mini meter), a single next-action (weakest skill, with the why), a readiness strip, and the human-trainer line. Cool graphite ground + ONE signal-teal used only as measurement ink; monospace tabular readouts. The deliberate opposite of the rejected warm/soft sunset dashboard.
- **Design-loop (I can't ship UI blind):** built HTML mock → rendered via playwright chromium → premium-ui-critic 6/10 → fixed the ruler-scale bug (tick labels & markers were on different scales → the "precision instrument" misread its own numbers), backwards arrow (→ vs ← in RTL), red-alarm delta wall, and a templated glowing CTA → re-rendered = premium. Also rendered + verified the NEW empty state (what all 5 zero-usage students see first). Concept board + built-out mock published as Artifacts.
- **Files:** rewrote `src/pages/student/ielts-atelier/Home.jsx` (Instrument design wired to useAdaptivePlan/useLatestResult/useSkillProgress/useMockAttempts/useErrorBankCount, with a full empty state) + `_layout/IELTSMasterclassLayout.jsx` (graphite instrument ground replacing IELTSSunsetBackground; header fixed: «طلاقة / IELTS», NOT the old "Fluentia IELTS Masterclass" brand-violation). **Whole-Atelier re-theme in ONE move:** the layout root overrides the 8 warm `--sunset-*` tokens (+ `--ds-accent-primary/gold`) to graphite/teal — the 21 Atelier files that consume those tokens inherit the new palette via CSS-var cascade, so the labs re-theme coherently without per-page edits. Re-showed the nav entry (tamayuz-gated).
- **Verify:** full app-wide esbuild import-resolution (packages=external) on candidate tree = origin/main + 3 files → exit 0. Rendered both Home states. Deployed via worktree-off-main commit (Home/layout confirmed still the OLD sunset version on main = no parallel clobber; navigation.js ported onto fresh main).
- **Still to watch:** the token re-theme is low-risk (palette shift via inherited vars, structure unchanged) but the individual labs (Reading/Listening/Writing/Speaking/Mock) weren't each rendered — spot-check them live after deploy; any lab that hardcoded a warm treatment may need a touch-up. Directions ② Ascent / ③ Studio remain as artifacts if Ali wants a different world.

### 2026-07-03 — IELTS ATELIER: finish the track end-to-end (un-hide + trainer surface + 2 real pages + content + mock fix)
- What: Took the IELTS Atelier from "built but hidden" to fully live. (1) **Un-hid** the student nav entry — `{ id:'ielts-atelier', icon:Target, to:'/student/ielts-atelier', requiresPackage:'ielts' }` added to `navigation.js` `sections.learning` AND mobile `drawerSections.learning` (Sidebar/MobileDrawer already gate `requiresPackage:'ielts'` via `hasIELTSAccess`, which includes **tamayuz** — so the 5 IELTS-track students see it). (2) **Restored the trainer IELTS surface** — `/trainer/ielts` was parked in App.jsx's redirect-to-`/trainer` array + `IELTSOverview` orphaned; re-mounted it inside the new `TeacherLayout` block as a real route + added `/trainer/ielts/:studentId` → NEW `IELTSStudentDetail.jsx` (wraps the existing `StudentIELTSTab`: mocks/plan/errors/sessions). Fixed `useIELTSRoster` to gate on `hasIELTSAccess` (was `package==='ielts'` only → returned EMPTY for the all-tamayuz roster). Repointed `RosterRow` → `/trainer/ielts/:id`. Added gated `{ id:'ielts', visibleWhen:'ielts-students' }` to `teacherNavigation.js` (excluded from `TEACHER_MOBILE_BAR`) + a `useIELTSRoster()` filter in `TeacherSidebar` so only trainers with IELTS students see it (both real trainers د.محمد/رشيد qualify). (3) **Built the 2 placeholder pages for real** — `Readiness.jsx` (data-driven: `target_exam_date` countdown + `readyCount/4` radial-gauge showpiece + live stepper checklist from diagnostic/skill/mock/error-bank signals + day-of essentials) and `Trainer.jsx` (live dual-presence hero + human-review value + `/chat` CTA). Ran premium-ui-critic (added focal gauge/live-presence hero + layered shadows + hover + reduced-motion) + arabic-copy-reviewer (fixed 15 gendered-2nd-person leaks → rasm-neutral phrasing; brand clean). (4) **Content:** published verified-safe drafts → **listening 9→25 (all), reading 16→22, writing 9→17 (all Task 2); speaking 60**. Held 8 Task 1 (reference a chart image that isn't attached) + 21 object-shaped reading (need array conversion). (5) **Mock lab bug:** the Atelier mock is content-fed via `assembleContent()` (the `ielts_mock_tests` table is UNUSED by students), and it selected/rendered `passage_text` — a non-existent column (real column = `content`) → mock reading silently blank (never caught: 0 prior mock attempts). Fixed `passage_text→content` in `useMockSession.js` + `MockReading.jsx`. (6) **Reading renderer fix:** `Reading.jsx` (+ MockReading) question-text fallback now also reads `incomplete_sentence || question` — unblocks 2 published passages AND repairs 5 already-live passages that showed bare "السؤال N".
- Files (in-tree, dormant IELTS — edited in place): `src/pages/student/ielts-atelier/{Readiness,Trainer,Reading}.jsx`, `.../Mock/useMockSession.js`, `.../Mock/segments/MockReading.jsx`, `src/hooks/trainer/useTrainerIELTSStudents.js`, `src/components/trainer/ielts/RosterRow.jsx`, NEW `src/pages/trainer/IELTSStudentDetail.jsx`. Files (hot/shared — ported onto fresh origin/main): `src/App.jsx`, `src/config/navigation.js`, `src/config/teacherNavigation.js`, `src/components/teacher/shell/TeacherSidebar.jsx`.
- DB: `is_published=true` on 6 reading + 16 listening + 8 writing-task2 rows (live via Mgmt API). No schema/RLS/edge changes.
- Verify: full app-wide esbuild import-resolution (packages=external) on a candidate tree = origin/main + all 12 files → exit 0. Correctness code-review agent → "no bugs found". Deployed to main via worktree-off-origin/main commit (clobber-free — all 4 hot files confirmed unchanged since baseline).
- Still open (documented, not shipped): 21 object-shaped reading drafts need `questions` array conversion; 8 Task 1 need chart images; full 4-section listening in a "true" mock needs the `listening_test_id` schema (2-section-per-id today); zero real student usage → recommend a pilot walk-through.
...
- Wiring: the reading player reads **`reading_passage_audio`** (by `passage_id`), so the script upserts that row (`full_audio_url/path/duration_ms`, `voice_id=Alice`, `paragraph_audio=[] word_timestamps=[]` → audio plays, karaoke not yet) AND mirrors `curriculum_readings.passage_audio_url` (+ `audio_duration_seconds/generated_at`). Vocab → `curriculum_vocabulary.audio_url` (+ `audio_voice_name/generated_at`), played by the word-tap `pronounceWord` path.
- Storage: bucket `curriculum-audio`; paths `reading/custom/<reading_id>/full.mp3` + `vocab/custom/<vocab_id>.mp3` (upsert). The legacy `.env` service-role key IS accepted for these storage uploads (bucket is audio-mime-restricted, which is the only gate).
- Pipeline per file: ElevenLabs mp3 → `ffmpeg -i in -ac 1 -ar 44100 -c:a libmp3lame -b:a 128k -map_metadata -1` → `ffprobe` asserts channels=1 → upload → DB wire with `.select()`+rowcount. Idempotent: skip a passage/word whose `audio_url` is set AND the file returns HTTP 200 (re-run generated 0, burned 0 chars).
- Verify (all PASS): 8/8 passages have audio in BOTH tables + reachable (200) + the **CDN-served files are MONO** (ffprobe ch=1, sampled 3); 56/56 vocab audio set + reachable + mono; isolation — exactly 8 `reading_passage_audio` rows use the `reading/custom/` path (no non-Sara audio touched). Passages 35–48s each. Definitive Safari playback is Ali's device check (files are uniform mono, so the channel-drift bug cannot occur).
- Files: `scripts/audio-sara-units.cjs` (NEW). DB: 8 `reading_passage_audio` rows + 8 `curriculum_readings.passage_audio_url` + 56 `curriculum_vocabulary.audio_url` (all live). Storage: 64 mp3 objects. Edge fns: none. NO frontend/schema change → no deploy needed (data-only; the player already reads these rows, so audio is live immediately).
- Status: Complete + verified. Committed to main via Git Data API. Sara's account is now fully complete: custom IT content, her goal-framed purple space, 12 sessions, vocab, and audio. Next optional polish = passage karaoke (needs ElevenLabs with-timestamps).


### 2026-07-02 — FARDI "BUILT FOR HER": goal-framed home hero + scoped purple accent (Sara)
- What: Made a custom-curriculum (Fardi/1:1) student's space READ as built for her — a goal-framed dashboard hero + the Fardi purple accent (`#a855f7`) — everything scoped to `students.uses_custom_curriculum=true`; every other student's dashboard + palette stays byte-for-byte unchanged (verified).
- Schema (migration `20260702140000`, applied live): `students.custom_mission_ar text` (Sara's set to «هدفك: تشرحين أي مشكلة تقنية على مكالمة بطلاقة — بدون توقّف أو ترجمة.»). `private_sessions` had RLS enabled with ZERO policies (deny-all for JWT users; scheduling uses service role) → added a purely-additive `student_read_own_private_sessions` SELECT policy (`student_id = auth.uid()`) so a 1:1 student can read her own sessions on the dashboard.
- Fardi accent token (no scattered hex): `--accent-individual` (+ dim/glow/strong) added to `src/styles/design-tokens.css`, plus a `html[data-track="fardi"]` block that remaps BOTH accent systems — legacy `--accent-sky*`/`--text-link`/`--info`/`--sidebar-active-*` AND design-system `--ds-accent-primary*`/`--ds-sky` — to purple. Specificity `html[..]` (0,1,1) beats the `:root`/`[data-theme]` token defs (0,1,0), so it wins in every theme. `LayoutShell` sets/removes `data-track="fardi"` on `<html>` gated on `studentData.uses_custom_curriculum` (impersonation-safe) — reaches the sidebar, buttons, progress AND the portaled home world; no global leak.
- Goal-framed hero (`SpotlightDashboard`, the live "Living Atlas" home): when `uses_custom_curriculum && custom_mission_ar`, a new `FardiHero` branch renders (generic hero untouched for everyone else) — small greeting «مساء النور · سارة» → the MISSION as the focal headline → track progress «X من 8 وحدات» + slim purple bar → next 1:1 session (day · date · time Riyadh) from `private_sessions` → XP/streak/level demoted to a small meta row → purple CTA. The analytics drawer's «حصّتك القادمة» also swaps the group-schedule `NextClassWidget` for a `FardiNextSessions` list of her upcoming private sessions (with Meet link when set).
- `useLevelJourney` made custom-aware (3rd arg): custom student → HER units (`owner_student_id`, ordered `custom_sort`) so the whole home (hero X/8 + journey filmstrip + current-unit) reflects her 8 IT units; the generic path gained `.is('owner_student_id', null)` so custom units never leak into a normal student's journey. Fixes a Prompt-1 gap where Sara's dashboard journey showed 12 generic + 8 custom = 20 mixed units.
- New hook `src/hooks/usePrivateSessions.js` — `useUpcomingPrivateSessions(studentId)` (soonest scheduled, RLS-own-rows) + `formatSessionWhen()` (Arabic day · date · 12h مساءً/صباحاً, Riyadh wall-clock, no tz shift).
- Verify (real JWTs — all PASS): Sara reads uses_custom_curriculum=true + her mission + 8 custom units + her next session (Sat 4 Jul 23:00); another L3 student is NOT custom (generic hero + sky accent), still gets 12 generic units, and reads 0 private_sessions (no leak). esbuild parse + `--bundle --packages=external` resolution green on all edited files. React hooks all at top / after guards (FardiHero + FardiNextSessions own their hooks); `profile.id`/`studentData` throughout (impersonation-safe); no hardcoded hex in new code (token-referenced).
- Files: `src/styles/design-tokens.css`, `src/pages/student/dashboards/atlasHome.css`, `src/components/layout/LayoutShell.jsx`, `src/pages/student/dashboards/useLevelJourney.js`, `src/pages/student/dashboards/SpotlightDashboard.jsx`, `src/hooks/usePrivateSessions.js` (NEW), `supabase/migrations/20260702140000_fardi_experience.sql` (NEW). DB: 1 column + 1 RLS policy (live). Edge fns: none.
- Status: Complete + data-verified. Committed to main via Git Data API (working branch diverged + shared tree → no branch push). Frontend → Vercel redeploys; default path unchanged for non-custom students. Pixel check is the owner's step (prompt says "impersonate Sara"): a faithful live screenshot from here is blocked — Sara has must_change_password=true (login lands on the force-change screen) and admin creds are stale. Next (Prompt 4): audio for her 8 units (ElevenLabs + Safari `-ac 1` re-encode).


### 2026-07-02 — SARA'S 8-UNIT CUSTOM IT CURRICULUM ("Career English for IT")
- What: Generated Sara Al-Asmari's full 8-unit custom track on the Fardi engine (Prompt 1's mechanism). Replaced her single proof unit with the full Unit 1 and added Units 2–8 — a coherent **"Career English for IT"** curriculum built around her goal (explain IT system issues fluently on live calls without freezing). All original B1 content; no copyrighted text.
- Units (custom_sort 1–8): 1 Explaining an Outage on a Call / شرح انقطاع الخدمة على مكالمة · 2 Describing an Intermittent Issue / وصف مشكلة متقطّعة · 3 Walking Through Root-Cause Analysis / المرور خلال تحليل السبب الجذري · 4 Handling Clarifying Questions / التعامل مع أسئلة الدعم التوضيحية · 5 Escalating an Incident Clearly / تصعيد حادثة بوضوح · 6 Status Update in a Standup / تحديث الحالة في الاجتماع اليومي · 7 Writing a Clear Incident Summary (email, WRITING core) / كتابة ملخّص حادثة واضح · 8 Explaining Impact to Non-Technical Stakeholders / شرح الأثر لغير التقنيين.
- Per unit (mirrors the L3 blueprint): Unit Brief (`why_matters` + `outcomes` + `brief_questions` + `activity_ribbons`, Arabic feminine in Dr. Ali's voice), 1 reading passage (~150–190 words, original IT incident-style, target vocab marked with `*asterisks*`), 4–5 comprehension MCQs (main_idea/detail/vocabulary/inference + Arabic explanations), 7 vocabulary rows (English word + Arabic meaning + example + POS, attached to the reading), 1 grammar-in-context mini-lesson (`explanation_content.sections` with `content_ar` + HTML `content_en`) + 2 fill_blank exercises, and 1 task — **speaking** for U1–6/8, **writing** for U7. The anti-freeze scaffold (Context → Symptom → Impact → What I've checked → What I need) is woven into every task + brief. Grammar arc: past-simple↔present-perfect → adverbs of frequency → sequence connectors → question forms/clarification → necessity modals → present-perfect+continuous → passive reporting → simplifying with "which means / so".
- Pronunciation focus (listed per unit in the prompt) was folded into each speaking task's `preparation_notes` rather than into `curriculum_pronunciation` rows, because the pronunciation surface is shelved/hidden for students in this app (dead rows avoided).
- How: `scripts/data/sara-it-units.cjs` (all content) + `scripts/provision-sara-it-units.cjs` (service-role, idempotent upsert by natural key — re-running upgrades in place; `.select()`+rowcount after every write; NO deletes). Applied live.
- Verify (real JWTs — all PASS): Sara's custom branch returns EXACTLY 8 units (custom_sort 1–8) and she reads their children; another L3 student still gets the 12 generic units unchanged; RLS blocks a non-owner from listing OR directly reading Sara's units/readings/vocab (all 0 rows); the 12 generic L3 units are untouched (owner NULL). Generator idempotent (re-run → same counts, 0 errors, no dupes).
- Files: `scripts/data/sara-it-units.cjs` (NEW), `scripts/provision-sara-it-units.cjs` (NEW). DB: Sara's 8 units + all children upserted (readings / comprehension / vocabulary / grammar + exercises / speaking / writing) — live. NO schema/RLS/frontend change (Prompt 1 shipped those), so no bundle change / no deploy risk. Edge fns: none.
- Status: Complete + verified. Committed to main via the Git Data API (working branch `reading-glossary-and-bug-reports` diverged + shared tree → no branch push). Audio for passages/vocab is a separate follow-up. Next (Prompt 3): the Fardi purple theme + a goal-framed dashboard so the whole space reads as built for her.


### 2026-07-02 — CUSTOM CURRICULUM ENGINE (per-student units) + Sara proof unit
- What: Built the reusable Fardi custom-curriculum mechanism — a specific student can have their OWN units (in their field) instead of the generic level units — and proved it end-to-end with 1 IT unit for Sara. Zero change for any other student (verified with real JWTs).
- Mechanism (migration `20260702130000`, applied live via Management API): `curriculum_units.owner_student_id` (uuid→students) + `custom_sort` (int) + `idx_curriculum_units_owner`; `students.uses_custom_curriculum` (bool default false). The `(level_id, unit_number)` UNIQUE constraint became a **PARTIAL** unique index (`WHERE owner_student_id IS NULL`), so many Fardi students can each own a `unit_number=1` unit at the same level.
- RLS (the isolation guarantee): every curriculum table's `auth_read_*` policy was `auth.role()='authenticated'` (any student could read ALL curriculum). Now owner-scoped: `curriculum_units` readable iff `owner_student_id IS NULL` OR `= auth.uid()` OR caller is admin/trainer; child tables gated by their parent unit's owner via SECURITY-DEFINER helpers `cc_unit_visible(unit_id)` / `cc_reading_visible(reading_id)` / `cc_grammar_visible(grammar_id)` (rows with NULL unit_id stay public; `curriculum_listening_exercises` shared bank left public). `admin_all_*` + `service_*` policies untouched.
- Page branch (`src/pages/student/curriculum/_useCurriculumData.js`): the units query branches on `studentData.uses_custom_curriculum` → custom student loads `owner_student_id = profile.id` ordered by `custom_sort`; everyone else unchanged (`level_id` + `owner_student_id IS NULL`, ordered by `unit_number`). Impersonation-safe (`profile.id`/`studentData`, hooks at top, no new hook).
- Proof unit (`scripts/provision-sara-alasmari` sibling `scripts/provision-sara-proof-unit.cjs`, idempotent, service-role): Sara flagged `uses_custom_curriculum=true` + owns 1 custom L3 unit **"Explaining an Outage on a Call / شرح انقطاع الخدمة على مكالمة"** (owner=sara, level_id=L3 Fluency, custom_sort=1, is_published=false, `why_matters`+`outcomes`+`brief_questions`+`activity_ribbons` in Dr. Ali's feminine voice) with minimal-but-valid children: 1 reading (payment-outage incident report) + 2 comprehension + 3 vocab (server/outage/impact) + 1 speaking (explain the outage on a call).
- Verify (real JWTs — all PASS): (1) Sara's custom branch returns EXACTLY her 1 unit + she reads its children (reading/vocab/comp/speaking = 1/3/2/1); (2) another student's normal branch returns the 12 generic L3 units with NO custom leak; (3) RLS blocks a non-owner direct read of the custom unit + reading + vocab + speaking (all 0 rows); (4) normal students still read ALL generic curriculum content across every child table (RLS broke nobody); (5) proof script idempotent (re-run inserts 0). esbuild parse OK on the edited hook. No local vite build per prompt rule.
- Deliverable for Prompt 2: `docs/dev-notes/custom-curriculum-discovery.md` (unit schema + every child-table shape/JSONB + the render-path map + RLS design). Full raw L3-U1 blueprint kept locally at `docs/dev-notes/custom-curriculum-blueprint-L3U1.json` (512 KB, not committed).
- Files: `src/pages/student/curriculum/_useCurriculumData.js`, `scripts/provision-sara-proof-unit.cjs` (NEW), `supabase/migrations/20260702130000_custom_curriculum_mechanism.sql` (NEW), `docs/dev-notes/custom-curriculum-discovery.md` (NEW). DB: 3 columns + 1 index + partial unique index + 3 functions + 13 RLS policies + Sara's 1 unit & children (all live). Edge fns: none.
- Status: Complete + verified. Committed to main via the Git Data API (working branch `reading-glossary-and-bug-reports` is diverged + shared tree → no branch push). `_useCurriculumData.js` is frontend → Vercel redeploys; the default path is byte-for-byte unchanged so zero risk to non-custom students. Pending: confirm the Vercel deploy reaches Ready + impersonate Sara to watch the unit render.


### 2026-07-02 — PROVISION: Sara Al-Asmari (سارة علي الأسمري) private B1 account (July intensive)
- What: Provisioned ONE new private-intensive student end-to-end via `scripts/provision-sara-alasmari.cjs` (service-role, idempotent, no hard deletes). Sara now has a working account + B1 curriculum access + 12 July 1-on-1 sessions with د. علي + a 32-word custom IT vocabulary deck + her goal/interests. Config mirrors the proven private-student template (وجدان الحارثي) resolved LIVE, not guessed.
- Phase A (discovery, read-only via `scripts/_mgmt-query.cjs` — Supabase MCP was NOT loaded this session): confirmed `profiles.must_change_password` EXISTS (NOT on students); `students.payment_day` DOES exist but left unset (prompt payload omits it); per-student vocab store = **`vocabulary_bank`** (student_id/word/meaning_en/meaning_ar/example_sentence/source/level/mastery/next_review/review_count). Enums verified: `student_package` has `private`, `student_status` has `active`, `student_track` = foundation|development|ielts (Wejdan uses **`foundation`** → mirrored, not the `development` fallback), `private_session_status` has `scheduled`, `vocab_mastery` has `new`. Template Wejdan (`e777c9cd…`): level 3 / private / track=foundation / group_id=`aaaaaaaa-4444-4444-4444-aaaaaaaaaaaa` (المجموعة 4, level 3) / study_mode=group. Exactly ONE admin (د. علي `e5528ced…`) → Phase C unblocked; Ali ALREADY has a `trainers` row (is_active) so none created. The `.env` legacy JWT service-role key was tested and DOES work here for `auth.admin` + RLS-bypass writes (no `sb_secret_*` needed for the JS admin client).
- Sara: auth user `fc68652d-c4cb-402f-a271-f17a5a4483b1` · profiles(role=student, must_change_password=true, display_name=سارة) · students(academic_level=3, package=private, track=foundation, group_id=المجموعة 4, status=active, enrollment_date=2026-07-01, custom_price=3000, writing_limit_override=30, onboarding_completed=true, gender=female, goals set, interests=[IT, Infrastructure, Digital Transformation, Career English]). `study_mode` left at default `group` (mirrors Wejdan — the group link is content-only; her real teaching is the private sessions).
- Phase C: 12 `private_sessions` seeded (student=Sara, trainer=Ali, status=scheduled, trainer_rate=0, `google_meet_link=NULL` until Ali adds one), Jul 4–29 Asia/Riyadh, notes per the plan. Idempotent on (student_id,date,start_time). July count asserted = 12.
- Phase D: 32 IT words into `vocabulary_bank` (source='private_plan', level=3, mastery='new', next_review=now(), review_count=0). Idempotent on (student_id,word). Count asserted = 32.
- Verify (Phase F): 12/12 checks green. Re-ran the whole script → 0 duplicate inserts, counts held (12 sessions / 32 words) → idempotency proven. `.select()` + rowcount assertion after every write; no `.delete()` anywhere.
- Handoff: LOGIN https://app.fluentia.academy · EMAIL Sarahasmari6@gmail.com · TEMP PW `Fluentia2025!` (forced change on first login) · B1 (Level 3) / الفردي (private).
- Files: `scripts/provision-sara-alasmari.cjs` (NEW). DB: 1 auth user + 1 profiles + 1 students + 12 private_sessions + 32 vocabulary_bank rows (all live). Edge Functions: none.
- Status: Complete — account LIVE + verified. Committed to main via the Git Data API (working branch is `reading-glossary-and-bug-reports` on a shared tree with parallel sessions → no branch push, no working-tree touch). Pending Ali: add a recurring Google Meet link for her sessions (currently NULL) + send her the welcome email.


### 2026-06-23 — MONTHLY ENGAGEMENT REWARD (silent, admin-only) + forward XP-leak hardening
- What: New monthly reward. Qualify = (>= N active days in EVERY full Saturday-week of the month) AND (>= min honest effort XP). "Active day" = real study evidence that day (>=1 lesson section completed, OR a graded recording, OR a submission, OR >=1 vocab word mastered) — scored ONLY from server-validated work (student_daily_activity), so it's farm-proof. Reward = flat 10% discount, FLAGGED for the admin to apply MANUALLY; nothing shown to students. All thresholds live-tunable from the admin page.
- Why / leak study: live RLS on xp_transactions (migration 093) lets any student console-inject arbitrary XP (WITH CHECK only checks student_id); SRS `correct_answer` (2 XP/card, uncapped) + practice/quiz `challenge` (client-computed amount, no idempotency) were being farmed in-app (one student: 1,240 XP from SRS, 190 in one day; current #1 earned 1,005/1,390 May XP from farmable reasons). So the reward deliberately ignores xp_transactions and scores from real completions.
- Calibration: the curriculum timer UNDERCOUNTS (median active day logs 94s; engaged students show 0 days >=30 logged min), so the system does NOT gate on raw minutes — it gates on completed real work, sized to a ~30-min/day equivalent. Under current behaviour "5 active days every week" rewards 0 students (most active May student passed only 1 of 4 weeks) — intentional, forward-looking, fully tunable.
- DB (migration 20260623120000, applied live via Management API): monthly_reward_config (single tunable row), monthly_reward_status (per student/month: qualified, weeks_passed/total, active_days_total, effort_xp, detail jsonb, granted/is_final), xp_clamp_log. Functions: compute_monthly_reward (Saturday-week buckets, Riyadh tz; a week counts only if >=5 of its days fall in the month; honest-reason effort XP), refresh_monthly_rewards, finalize_previous_month_rewards. Admin RPCs (is_admin gated): admin_set_monthly_reward_config / admin_grant_monthly_reward / admin_refresh_monthly_rewards. Cron: monthly-rewards-daily (21:30 UTC recompute current month), monthly-rewards-finalize (1st of month freeze previous).
- FORWARD-ONLY XP-leak guard: BEFORE INSERT trigger `trg_xp_clamp` on xp_transactions — clamps (never raises) any new write to a per-reason ceiling (kills console amount:999999 → verified 999999→30) + a daily 60-XP per-reason cap on the farm reasons (correct_answer/challenge); staff awards exempt; clamps logged to xp_clamp_log. Existing XP / xp_total / leaderboards are NEVER touched (per Ali: don't touch old XP, only discipline new earning).
- Frontend (deployed to main e2118157, Vercel READY, chunk served): src/pages/admin/AdminMonthlyRewards.jsx (/admin/monthly-rewards, نظام → «مكافآت الشهر») — month nav, live مستحقّون/قريبون/طُبّق-الخصم stats, per-week active-day dot strip, tunable thresholds (active_days_per_week + min_effort_xp + enabled) with «حفظ الشروط» (save+recompute), per-student «طُبّق الخصم» manual toggle. App.jsx route + navigation.js nav item ported onto main's content via Trees API (parallel-session-safe; working branch is reading-glossary-and-bug-reports).
- Verify: engine recomputed May+June (14 students) → 0 qualifiers as predicted, week math confirmed on the top student (passed 1/4 weeks); leak guard clamp proven (999999→30, rolled back); npm run build green; Vercel READY + prod bundle (index-tkxv2fIt.js) references + serves AdminMonthlyRewards-B5xvXBCU.js (200).
- Status: LIVE end-to-end. Students NOT notified (per Ali). The migration .sql lives on branch reading-glossary-and-bug-reports (DB already applied live via Mgmt API); only the 3 frontend files were pushed to main.
- Notes: Ali tunes the bar from the admin page (default 5 days/week every week + 200 effort XP) and applies the 10% discount manually — the system only flags qualifiers. To ease the bar: lower active_days_per_week or min_effort_xp and hit «حفظ الشروط». To disable: untick «النظام مُفعّل».


### 2026-06-20 — FIX: MCQ answer-length leakage (reading + listening) — "the longest option is the correct one"
- What: A student found that in the reading + listening multiple-choice banks the CORRECT answer was almost always the LONGEST option, so questions could be gamed by always picking the longest. Confirmed: correct was strictly-longest in **68.3% of reading** MCQs and **59.9% of listening** (vs ~25% by chance). "Pick the conspicuously-longest" (>1.15× next) won 52.9% reading / 39.9% listening.
- Fix approach: make answer LENGTH statistically uninformative by REWRITING ONLY THE WRONG OPTIONS (distractors). Each question gets a deterministic random length-target for the correct answer (25% longest / 25% shortest / 50% middle, by djb2 hash of the question id), and distractors are regenerated (Claude sonnet-4-6, grouped per passage/transcript for context, forced tool-use JSON) to realise that target while staying plausibly-wrong per the source. Very-long answers that can't be out-lengthened are "clustered" (declump pass) so no option is conspicuously longest. Short factual answers (<25 chars: dates/numbers) left alone — length can't leak there.
- **Correct answer text/index is NEVER changed** → grading + already-completed student scores are untouched. Reading grades by text match (client shuffles order); listening grades by `correct_answer_index` (correct kept in place). Verified: 519 reading rewritten / 504 listening MCQ — **0 broken** (correct always present, no duplicates, option count preserved, index + correct-text frozen). No writes to any student table.
- Result: "pick the longest" win-rate → reading **68.3% → 32.6%** (conspicuous 52.9% → **25.2%**); listening **59.9% → 27.6%** (conspicuous 39.9% → **19.0%**) — i.e. at chance, exploit dead. Per-target landing verified ~perfect (longest→100%, shortest→97.5%, middle→99.7%).
- Reversibility: full pre-change snapshot in `public.qa_distractor_backup_20260620` (1152 reading `choices` + 72 listening `exercises`). Restore = `UPDATE … FROM qa_distractor_backup_20260620`. Safe to drop the table later once confirmed.
- Live immediately (content read from DB at runtime — no deploy). Files: `scripts/fix-answer-length-bias.cjs` (NEW, idempotent — `--reading --listening [--declump]`; skips questions already satisfying their target). DB: `curriculum_comprehension_questions.choices` (519 rows), `curriculum_listening.exercises` (distractors within), 1 backup table. Edge Functions: none.
