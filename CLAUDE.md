...
- **Verify:** `npm run build` ✓ green (1m20s; no new errors; the >600kB chunk warning is pre-existing — eruda/charts). High-impact diffs (dashboard switch, listening play()/watchdog) code-reviewed. PENDING: Ali device check — Mac Safari (hard refresh Cmd+Shift+R) for listening + word audio; visual review of dashboard + spelling card; iPhone Safari.
...
- Status: Complete on branch `reading-glossary-and-bug-reports`, build-green, UNCOMMITTED — awaiting Ali's device verification before commit/merge.
