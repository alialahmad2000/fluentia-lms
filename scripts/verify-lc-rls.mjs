#!/usr/bin/env node
/**
 * PHASE B11 — prove the coach's own session can do exactly what it should and
 * nothing more.
 *
 * Runs as the REAL coach through the ANON key — not the service role —
 ...
 *
 *   node scripts/verify-lc-rls.mjs [--email=someone@example.com]
 *
 * Assertions 2, 3 and 4 are the ones that matter: if the coach can read
...
async function mintCoachSession(email, secret, anon) {
...
async function main() {
  ...
  const email = arg('email') || await (async () => {
    const { data } = await admin.from('profiles').select('email').eq('role', 'coach').limit(1)
    if (!data?.length) throw new Error('no coach account exists — run scripts/create-coach.mjs first')
    return data[0].email
  })()

  console.log(`COACH RLS VERIFICATION\nproject ${REF}\nacting as ${email} (anon key, real session)\n`)

  const { client: sb, user } = await mintCoachSession(email, secret, anon)
  console.log(`session minted for ${user.id}\n`)

  // 1 — the radar works
  {
    const { data, error } = await sb.rpc('lc_get_radar')
    record('1. lc_get_radar() returns rows', !error && Array.isArray(data) && data.length > 0,
      error ? error.message : `${data?.length ?? 0} active students`)
  }
  ...
  // 5 — he can read the Arabic library but never rewrite it
  {
    const { data: readable, error: readErr } = await sb
      .from('lc_message_templates').select('code').limit(1)
    if (readErr || !readable?.length) {
      record('5a. the coach can READ the message library', false, readErr?.message || 'no rows')
    } else {
      record('5a. the coach can READ the message library', true, `${readable.length} row visible`)
    }
    const { data, error } = await sb
      .from('lc_message_templates')
      .update({ situation_en: 'RLS probe — must never persist' })
      .eq('code', 'silent_7d_open')
      .select()
    const blocked = !!error || !data || data.length === 0
    record('5. UPDATE lc_message_templates fails (admin-only)', blocked,
      error ? `blocked (${error.code || error.message})` : `${data?.length ?? 0} rows updated`)
    const { data: after } = await admin
      .from('lc_message_templates').select('situation_en').eq('code', 'silent_7d_open').single()
    if (String(after?.situation_en || '').includes('RLS probe')) {
      record('5b. the template really is untouched', false, 'the probe text persisted')
    }
  }

  // 6 — he cannot file work under another coach's name
  {
    const { data: other } = await admin
      .from('profiles').select('id').eq('role', 'admin').limit(1).maybeSingle()
    const { data: anyStudent } = await admin
      .from('students').select('id').is('deleted_at', null).limit(1).maybeSingle()
    const { data, error } = await sb
      .from('lc_touchpoints')
      .insert({
        coach_id: other?.id ?? '00000000-0000-0000-0000-000000000000',
        student_id: anyStudent?.id,
        action: 'observation',
        blocker_type: 'unknown',
        note_en: 'RLS probe — should never persist',
      })
      .select()
    const blocked = !!error || !data || data.length === 0
    record('6. inserting a touchpoint as another coach fails', blocked,
      error ? `blocked (${error.code || error.message})` : 'row was accepted')
    if (!blocked) await admin.from('lc_touchpoints').delete().eq('note_en', 'RLS probe — should never persist')
  }

  // 7 — the blocker is not optional, even when calling the RPC directly
  {
    const { data: anyStudent } = await admin
      .from('students').select('id').is('deleted_at', null).eq('status', 'active').limit(1).maybeSingle()
    const { error } = await sb.rpc('lc_send_message', {
      p_student_id: anyStudent?.id,
      p_template_code: 'silent_7d_open',
      p_blocker: null,
      p_note: null,
    })
    record('7. lc_send_message with a null blocker fails', !!error,
      error ? `rejected (${error.message.slice(0, 60)})` : 'IT SENT A MESSAGE — investigate immediately')
  }

  // 8 — his OWN log must still work, or the console is unusable
  {
    const { data, error } = await sb
      .from('lc_daily_log')
      .upsert({
      ...
      .select()
    record('8. inserting his OWN daily log succeeds', !error && data?.length === 1,
      error ? error.message : 'ok')
    await admin.from('lc_daily_log').delete().eq('log_date', '1999-01-02')
  }

  await sb.auth.signOut()

  const failed = results.filter((r) => !r.pass)
  const moneyFailed = results.filter((r) => !r.pass && /payments|payroll|payouts|lc_message_templates/.test(r.name))
  console.log(`\n${'─'.repeat(60)}`)
  console.log(`${results.length - failed.length}/${results.length} passed`)
  if (moneyFailed.length) {
    console.log('\n⛔ A FINANCIAL TABLE IS READABLE BY THE COACH. Do not commit. Do not deploy.')
    process.exit(2)
  }
