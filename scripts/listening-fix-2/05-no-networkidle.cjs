// Reproduce WITHOUT waitForLoadState('networkidle'). If the storm doesn't fire,
// the trigger is something in the long networkidle wait. Otherwise the bug is
// inherent to the page.
...
;(async () => {
  ...
  await page.goto(`${PROD_URL}/student/curriculum/unit/${UNIT_ID}?activity=listening`, {
    waitUntil: 'domcontentloaded',  // do NOT wait for networkidle
  })

  // Sample every 1s for 15s
  for (let i = 1; i <= 15; i++) {
