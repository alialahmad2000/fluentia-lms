...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN COUNT(*) = 0 THEN 0
         ELSE ROUND(100.0 * COUNT(*) FILTER (WHERE a.status = 'present') / COUNT(*))::int
    END AS rate_pct
  INTO v_attend FROM attendance a
  WHERE a.student_id = p_student_id AND a.date BETWEEN p_period_start AND p_period_end;
...
`;
  ...
  try {
    await client.query(aggregatorSQL);
    console.log('Aggregator RPC created successfully');
  } catch(e) {
    console.error('Aggregator RPC error:', e.message);
  }
...
AS $fn$
  SELECT
    pr.data AS report,
    pr.data->'student'->>'name' AS student_name,
    pr.data->'trainer'->>'name' AS trainer_name,
    pr.period_end,
    pr.ai_narrative_ar,
    pr.trainer_notes AS trainer_note,
    pr.next_goals,
    pr.pdf_url
  FROM public.progress_reports pr
  WHERE pr.share_token = p_token AND pr.status = 'published'
  LIMIT 1;
$fn$;
...
`;
  ...
  try {
    await client.query(shareSQL);
    console.log('Share RPC created successfully');
  } catch(e) {
    console.error('Share RPC error:', e.message);
  }

  // 3. Test aggregator with a real student
  try {
