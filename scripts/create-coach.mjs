#!/usr/bin/env node
/**
 * PHASE D — provision a Learning Coach account.
 *
 *   node scripts/create-coach.mjs \
...
// 3. the coach row — this is what the console reads the timezone from
await sql(`
  insert into public.lc_coaches (id, timezone, is_active)
  values (${q(uid)}, ${q(timezone)}, true)
  on conflict (id) do update set timezone = excluded.timezone, is_active = true;
`)

// 4. read both back — an UPDATE that matched nothing also "succeeds"
const [profile] = await sql(`
  select p.id, p.full_name, p.role::text as role, p.ui_language, p.must_change_password,
         p.timezone, p.email, c.timezone as coach_timezone, c.is_active
  from public.profiles p
  left join public.lc_coaches c on c.id = p.id
  where p.id = ${q(uid)};
`)
if (!profile) throw new Error('the profile row is not there after the write')
const problems = []
if (profile.role !== 'coach') problems.push(`role is ${profile.role}, not coach`)
if (profile.coach_timezone !== timezone) problems.push(`lc_coaches.timezone is ${profile.coach_timezone}, not ${timezone}`)
if (profile.is_active !== true) problems.push('lc_coaches.is_active did not stick')
...
──────────────────────────────────────────────────────────────
  ...
  Their screens
    ${APP_URL}/coach       the radar — every active student, worst first
    ${APP_URL}/coach/log   the daily log

  They read no Arabic. Every message they send comes from the approved
  library and cannot be edited. When nothing in it fits, their only route is
  Escalate — which lands in /admin/coach-escalations.
──────────────────────────────────────────────────────────────`)
