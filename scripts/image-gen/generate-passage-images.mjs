/**
 * Generate passage illustration images for all 144 curriculum readings
 * Uses fal.ai Flux Schnell — watercolor style on dark background
 *
 * Each reading gets 1 passage image stored as JSONB array in passage_image_urls
 * These illustrate the main scene/setting of the passage content
 *
 * Run: node scripts/image-gen/generate-passage-images.mjs [--level N] [--resume] [--dry-run]
 */
...
function buildPrompt(title, theme, firstParagraph) {
  // Use first paragraph excerpt for more context about the passage scene
  const excerpt = firstParagraph
    ? firstParagraph.replace(/\*[^*]+\*/g, '').slice(0, 120)
    : '';
  const context = excerpt ? ` — scene: ${excerpt}` : '';
  return `A detailed illustration of the main setting and subject of "${title}" (topic: ${theme})${context}, ${STYLE}`;
}
...
async function main() {
  ...
  console.log("🖼️  Passage Image Generator (passage_image_urls)");
  ...
  let query = supabase
    ...
    .select('id, unit_id, reading_label, title_en, passage_content, passage_image_urls');

  if (resume) {
    // Resume: only process readings with empty or null passage_image_urls
    query = query.or('passage_image_urls.is.null,passage_image_urls.eq.[]');
  }
  ...
  let enriched = readings.map(r => {
    ...
    const firstParagraph = r.passage_content?.paragraphs?.[0] || '';
    return {
      ...r,
  ...
  if (dryRun) {
    ...
    console.log(`\n✅ Dry run: ${enriched.length} passage images would be generated.`);
    return;
  }

  // LIVE generation
  const outDir = "scripts/image-gen/output/passage-images";
  ...
  for (let i = 0; i < enriched.length; i++) {
    ...
    try {
      const prompt = buildPrompt(r.title_en, r.theme, r.firstParagraph);
      ...
      const filename = `passage-${label}.png`;
      fs.writeFileSync(path.join(outDir, filename), buf);

      // Upload to Supabase Storage
      const storagePath = `passages/${filename}`;
      ...
      // Update DB — store as JSON array with single URL
      const { error: dbErr } = await supabase
        .from("curriculum_readings")
        .update({ passage_image_urls: [publicUrl] })
        .eq("id", r.id);
  ...
  console.log("\n" + "=".repeat(60));
  console.log("📊 PASSAGE IMAGE RESULTS:");
