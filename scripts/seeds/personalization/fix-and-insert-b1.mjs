...
const realUUIDs = JSON.parse(fs.readFileSync(`${BASE}/b1-uuids.json`,'utf8'));
const prefixMap = {};
...
for (const b of buckets) {
  const path = `${BASE}/b1-${b}.js`;
  if (!fs.existsSync(path)) { console.log(`MISSING: b1-${b}.js`); continue; }
  let c = fs.readFileSync(path,'utf8'), o = c;
  // Fix apostrophes in title lines
  ...
  if(c!==o){fs.writeFileSync(path,c,'utf8');console.log(`Fixed b1-${b}.js`);}
  try{const mod=await import(`file:///C:/Users/Dr.%20Ali/Desktop/fluentia-lms/scripts/seeds/personalization/b1-${b}.js`);all.push(...(mod.variants||[]));console.log(`Loaded b1-${b}.js: ${mod.variants?.length}`);}
  catch(e){console.error(`ERROR b1-${b}:`,e.message.slice(0,120));}
}
...
const {rows:dist}=await client.query(`SELECT interest_bucket,COUNT(*) FROM public.personalized_readings WHERE cefr_level='B1' GROUP BY interest_bucket ORDER BY interest_bucket`);
const {rows:[qa]}=await client.query(`SELECT AVG(qa_word_count_ratio)::NUMERIC(4,2) r,AVG(qa_vocab_coverage)::NUMERIC(4,2) c,COUNT(*) FILTER(WHERE qa_passed=FALSE) f FROM public.personalized_readings WHERE cefr_level='B1'`);
console.log(`OK:${ok} FAIL:${fail} | Total in DB: ${total}`);
console.log('B1:',dist.map(r=>`${r.interest_bucket}:${r.count}`).join(', '));
