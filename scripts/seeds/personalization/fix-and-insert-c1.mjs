...
const realUUIDs = JSON.parse(fs.readFileSync(`${BASE}/c1-uuids.json`,'utf8'));
const prefixMap = {};
...
for (const b of buckets) {
  const path = `${BASE}/c1-${b}.js`;
  if (!fs.existsSync(path)) { console.log(`MISSING: c1-${b}.js`); continue; }
  let c = fs.readFileSync(path,'utf8'), o = c;
  c = c.replace(/^(\s+title:\s*)'(.+)'(,\s*)$/gm,(match,prefix,inner,suffix)=>{if(inner.includes("'"))return`${prefix}"${inner.replace(/"/g,'\\"')}"${suffix}`;return match;});
  ...
  if(c!==o){fs.writeFileSync(path,c,'utf8');console.log(`Fixed c1-${b}.js`);}
  try{const mod=await import(`file:///C:/Users/Dr.%20Ali/Desktop/fluentia-lms/scripts/seeds/personalization/c1-${b}.js`);all.push(...(mod.variants||[]));console.log(`Loaded c1-${b}.js: ${mod.variants?.length}`);}
  catch(e){console.error(`ERROR c1-${b}:`,e.message.slice(0,120));}
}
console.log(`UUID fixes: ${fixCount} | Total: ${all.length}`);
const {Client}=pg;
...
const {rows:dist}=await client.query(`SELECT interest_bucket,COUNT(*) FROM public.personalized_readings WHERE cefr_level='C1' GROUP BY interest_bucket ORDER BY interest_bucket`);
const {rows:[qa]}=await client.query(`SELECT AVG(qa_word_count_ratio)::NUMERIC(4,2) r,AVG(qa_vocab_coverage)::NUMERIC(4,2) c,COUNT(*) FILTER(WHERE qa_passed=FALSE) f FROM public.personalized_readings WHERE cefr_level='C1'`);
console.log(`OK:${ok} FAIL:${fail} | Total in DB: ${total}`);
console.log('C1:',dist.map(r=>`${r.interest_bucket}:${r.count}`).join(', '));
