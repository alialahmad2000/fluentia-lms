// Inserts the authored Business Track content (10 stages / 30 lessons) into the DB. Idempotent (upsert on slug).
// Content lives IN-REPO at scripts/biz-track-content/stage-<N>.json (authored per SPEC.md there).
...
const DIR = path.join(__dirname, 'biz-track-content');
...
(async () => {
  ...
  for (let n = 1; n <= 10; n++) {
    ...
    const { data: st, error: se } = await sb.from('biz_track_stages')
      .upsert({
        ...
        accent: s.accent || '#f59e0b', icon: s.icon || 'Briefcase',
      }, { onConflict: 'slug' })
    ...
    for (const l of j.lessons) {
      ...
      const { error: le } = await sb.from('biz_track_lessons')
        .upsert({
  ...
  const { count: sc } = await sb.from('biz_track_stages').select('id', { count: 'exact', head: true });
  const { count: lc } = await sb.from('biz_track_lessons').select('id', { count: 'exact', head: true });
