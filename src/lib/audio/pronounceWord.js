...
export async function pronounceWord(rawWord, { studentId = null } = {}) {
  ...
  try {
    ...
    // Network blip → fall through to generated TTS / Web Speech.
  }

  // ── Tier 1.5: generated word audio — a REAL MP3 for ANY word, cached ──
  // The DB lookup above only covers curriculum VOCABULARY (the highlighted gold
  // words). Every OTHER word — the ones students reported they "can't hear" —
  // now gets a real MP3 from the word-tts edge function: same George voice as the
  // vocab audio, generated once and cached forever in storage, played from an
  // in-memory blob (Safari-safe). This is what makes EVERY tapped word in the
  // reading article pronounce, highlighted or not. (WORD-AUDIO-EVERY-WORD 2026-06)
  try {
    const ttsUrl = supabase.storage.from('curriculum-audio').getPublicUrl(`word-tts/${word}.mp3`).data?.publicUrl
    // Already generated by anyone before? → play the cached file from a blob (fast).
    let blobUrl = ttsUrl ? await primeBytes(word, ttsUrl) : null
    if (!blobUrl) {
      // First time this word is tapped → synthesize + cache, then play it.
      const { data: gen } = await supabase.functions.invoke('word-tts', { body: { word } })
      if (gen?.url) blobUrl = await primeBytes(word, gen.url)
    }
    if (blobUrl) {
      if (ttsUrl) wordAudioCache.set(word, ttsUrl)
      return playUrl(blobUrl, word, studentId, 'tts')
    }
  } catch {
    // fall through to Web Speech
  }

  // ── Tier 2: Web Speech (last resort — unreliable on iOS) ──────────
  if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
