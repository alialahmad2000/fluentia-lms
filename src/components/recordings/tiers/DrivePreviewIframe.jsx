...
export default function DrivePreviewIframe({ recording, onError, onLoad }) {
  const timerRef = useRef(null)
  const fileId = recording?.google_drive_file_id || extractFileId(recording?.google_drive_url)
  const url = fileId ? `https://drive.google.com/file/d/${fileId}/preview` : null

  useEffect(() => {
    if (!url) { onError?.('no_file_id'); return }
    timerRef.current = setTimeout(() => onError?.('preview_timeout'), 15000)
  ...
  return (
    <div className="w-full relative bg-black rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9' }}>
      <iframe
        ...
        onError={() => onError?.('preview_iframe_error')}
      />
    </div>
  )
}
