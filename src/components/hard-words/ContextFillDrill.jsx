...
export default function ContextFillDrill({ batch, studentId, onComplete }) {
  ...
  const options = useMemo(() => {
    if (!current) return []
    const pool = distractorPool.filter((d) => d.vocabularyId !== current.vocabularyId)
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              >
                اقرأ الجملة واختر الكلمة المناسبة:
              </span>
              {current.meaningAr && (
                <span
                  ...
                  style={{ color: 'var(--text-tertiary)' }}
                >
                  المعنى: <span style={{ color: 'var(--text-secondary)' }}>{current.meaningAr}</span>
                </span>
              )}
            </div>
            <p
            ...
            {options.map((opt) => {
              ...
              return (
                <motion.button
                  ...
                  className="p-4 rounded-xl text-start font-semibold"
                  style={{
          ...
          {revealed && current.audioUrl && (
            ...
            >
              <button
                ...
                className="px-4 py-2 rounded-full text-sm font-bold font-['Tajawal'] inline-flex items-center gap-2"
                style={{
