...
export default function ShareAchievementCard({ type, studentName, levelName, unitName, studentText, feedback, scores, leaderboard, currentStudentId }) {
  ...
  const handleShare = useCallback(async () => {
    ...
    // Wait for DOM render + font loading
  ...
  return (
    <>
      ...
      {/* Card — only rendered when capturing, shown behind a backdrop so browser renders it */}
      {isCapturing && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9998 }}>
          {/* Backdrop with loading message */}
          <div style={{
            position: 'fixed',
            inset: 0,
            ...
            <div style={{ color: 'white', fontSize: '18px', fontFamily: "'Tajawal', sans-serif" }}>
              جاري تجهيز الصورة... ⏳
            </div>
          </div>

          {/* The actual card — positioned in viewport so browser renders it fully */}
          <div
            ref={cardRef}
            style={{
              position: 'fixed',
          ...
          >
            <div style={{ background: 'linear-gradient(165deg, #060e1c 0%, #0a1930 40%, #0d1f3c 100%)', padding: '48px', fontFamily: "'Tajawal', sans-serif", direction: 'rtl', color: '#e2e8f0', borderRadius: '20px', overflow: 'hidden' }}>
              {/* Embedded fonts for image capture */}
                  ...
                  return (
                    <div key={key}>
                      ...
                      <div style={{ height: '10px', borderRadius: '5px', background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: '5px', width: `${(s / 10) * 100}%`, background: scoreColor(s) }} />
                      </div>
                    </div>
                  )
                })}
