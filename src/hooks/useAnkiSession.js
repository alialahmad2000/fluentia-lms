...
export function useAnkiSession(studentId, settings) {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    const { data: newRows, error: nErr } = await supabase
      .from('anki_cards')
      .select(
        'id, state, stability, difficulty, elapsed_days, scheduled_days, reps, lapses, due_at, last_review_at, vocabulary:curriculum_vocabulary!vocabulary_id(id, word, definition_en, definition_ar, example_sentence, part_of_speech, audio_url, synonyms, antonyms, word_family, pronunciation_alert)'
      )
      .eq('student_id', studentId)
      .eq('state', 'new')
