...
export default function DeskReadingPassage() {
  ...
  return (
    <div className="space-y-10 max-w-[720px] mx-auto">
      <Link to="/desk/reading" className="inline-flex items-center gap-1.5 font-['Hanken_Grotesk'] text-[13px] desk-rise" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.5)' }}><ArrowLeft size={15} /> Reading</Link>

      {/* masthead */}
      <div className="desk-glass desk-station-head p-6 lg:p-7 desk-rise">
        <div className="flex items-center gap-2 mb-2.5 flex-wrap font-['Hanken_Grotesk'] text-[11.5px]" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.5)' }}>
        ...
        <p className="font-['Tajawal'] text-[14px] mt-1" style={{ color: 'rgba(42, 33, 64,0.5)' }}>{r.title_ar}</p>
        <p className="font-['Hanken_Grotesk'] text-[13.5px] mt-3 leading-relaxed" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.62)' }}>{r.intro}</p>
      </div>

      {/* the passage */}
      <section className="desk-rise">
        <div className="flex items-center gap-2.5 mb-4"><span className="desk-lesson-sec-mark"><BookOpen size={16} /></span><div><p className="font-['Hanken_Grotesk'] text-[12px] tracking-[0.18em]" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.46)' }}>THE PASSAGE</p><h2 className="font-['Hanken_Grotesk'] font-extrabold text-[20px] leading-tight mt-0.5" dir="ltr" style={{ color: 'var(--cream)' }}>Read</h2></div></div>
        <div className="desk-glass p-6 lg:p-7">
          ...
          <div className="space-y-4" dir="ltr">
            {r.paragraphs.map((p, k) => (
              <p key={k} className="font-['Hanken_Grotesk'] text-[15.5px] leading-[1.95]" style={{ color: 'rgba(42, 33, 64,0.86)' }}>{p}</p>
            ))}
          </div>
        </div>
      </section>
            ...
            {r.key_terms.map((t, k) => (
              <div key={k} className="desk-glass p-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="font-['Hanken_Grotesk'] text-[12.5px] mt-1 leading-snug" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.55)' }}>{t.def_en}</p>
                </div>
                <PlayBtn text={t.term} />
              </div>
            ))}
      ...
      <div className="pt-1 desk-rise">
        ...
        <div className="flex items-center justify-between gap-3 mt-4">
          <Link to="/desk/reading" className="inline-flex items-center gap-1.5 font-['Hanken_Grotesk'] text-[13px]" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.55)' }}><ArrowLeft size={15} /> Library</Link>
...
function ReadingQ({ q, onResolve }) {
  ...
  return (
    <div className="desk-glass p-6">
      <p className="font-['Hanken_Grotesk'] text-[14.5px] font-semibold mb-3.5 leading-relaxed" dir="ltr" style={{ color: 'rgba(42, 33, 64,0.9)' }}>{q.q}</p>
      ...
      {picked != null && q.why && (
        <p className="font-['Hanken_Grotesk'] text-[12.5px] mt-3 pt-3 desk-hair leading-relaxed" dir="ltr" style={{ color: chosen?.correct ? 'rgba(110,231,183,0.9)' : 'rgba(255,180,164,0.85)' }}>{q.why}</p>
      )}
