...
const band = (pct) =>
  ...
  : pct >= 65 ? { en: 'Strong', c: '#7dd3fc' }
  : pct >= 40 ? { en: 'Progressing', c: '#38bdf8' }
  : { en: 'Getting started', c: 'rgba(238, 243, 251,0.6)' }
    ...
    <div className="relative" style={{ width: 168, height: 168 }}>
      <svg width="168" height="168" viewBox="0 0 168 168" style={{ transform: 'rotate(-90deg)' }}>
        ...
        <motion.circle
          ...
          transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] }} style={{ filter: 'drop-shadow(0 0 8px rgba(56, 189, 248,0.5))' }} />
        <defs>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-['Inter'] font-black text-[38px] leading-none tabular-nums" style={{ color: 'var(--cream)' }}>{pct}<span className="text-[18px]" style={{ color: 'rgba(238, 243, 251,0.5)' }}>%</span></span>
        <span className="font-['Inter'] text-[12px] font-bold mt-1" dir="ltr" style={{ color: b.c }}>{b.en}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="desk-glass p-4">
      <div className="flex items-center justify-between mb-2.5">
        <span className="font-['Inter'] font-bold text-[13px]" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.8)' }}>{label}</span>
        <span className="font-['Inter'] text-[12px] tabular-nums font-semibold" style={{ color: pct === null ? 'rgba(238, 243, 251,0.35)' : 'var(--brass-hi)' }}>{pct === null ? '—' : `${(score).toFixed(1)}/10`}</span>
      </div>
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="desk-rise">
        ...
        <h1 className="font-['Inter'] font-extrabold text-2xl lg:text-[30px]" dir="ltr" style={{ color: 'var(--cream)' }}>Your readiness</h1>
        <p className="font-['Tajawal'] text-[13px] mt-1" style={{ color: 'rgba(238, 243, 251,0.5)' }}>تقدّمي</p>
        <p className="font-['Inter'] text-[14px] mt-1.5" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.55)' }}>How ready you are for English work calls — built from every call you practice.</p>
      </div>
      ...
      {attempts === 0 ? (
        <div className="desk-glass p-9 text-center desk-rise">
          ...
          <p className="font-['Inter'] font-bold text-lg" dir="ltr" style={{ color: 'var(--cream)' }}>You haven&apos;t taken your first call yet</p>
          <p className="font-['Inter'] text-[13px] max-w-sm mx-auto mt-2 leading-relaxed" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.6)' }}>
        ...
        <>
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-5 items-center">
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ ease: [0.16, 1, 0.3, 1] }}
              className="desk-glass p-6 flex flex-col items-center" style={{ borderColor: 'rgba(56, 189, 248,0.24)' }}>
              <p className="font-['Inter'] font-bold text-[13px] mb-3" dir="ltr" style={{ color: 'var(--brass)' }}>Call readiness</p>
              <ReadinessRing pct={readinessPct} />
              <p className="font-['Inter'] text-[11px] mt-3" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.45)' }}>Averaged from {attempts} graded {attempts === 1 ? 'call' : 'calls'}</p>
            </motion.div>
          ...
          {ins?.errors?.length > 0 && (
            <div className="desk-rise">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-['Inter'] font-bold text-[16px]" dir="ltr" style={{ color: 'var(--cream)' }}>Worth working on</h2>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                {ins.errors.slice(0, 4).map((e, i) => (
                  <div key={i} className="desk-glass p-4">
                    ...
                    {e.rule && <p className="font-['Tajawal'] text-[12px] mt-2 leading-relaxed" style={{ color: 'rgba(238, 243, 251,0.62)' }}>{e.rule}</p>}
                  </div>
                ))}
              </div>
              {ins?.tip && (
                <div className="desk-glass p-4 mt-3 flex items-start gap-2.5" style={{ borderColor: 'rgba(56, 189, 248,0.24)' }}>
                  <span className="mt-0.5 flex-shrink-0" style={{ color: 'var(--brass-hi)' }}>◆</span>
                  <p className="font-['Tajawal'] text-[13px] leading-relaxed" style={{ color: 'rgba(238, 243, 251,0.78)' }}>{ins.tip}</p>
                </div>
              )}
            </div>
          )}
          ...
          {/* scenario ledger */}
          <div className="desk-rise">
            <h2 className="font-['Inter'] font-bold text-[16px] mb-3" dir="ltr" style={{ color: 'var(--cream)' }}>Your call log</h2>
            <div className="desk-glass divide-y" style={{ borderColor: 'rgba(56, 189, 248,0.16)' }}>
              {modules.map((m) => {
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 px-4 py-3" style={{ borderColor: 'rgba(56, 189, 248,0.10)' }}>
                    <span className="font-['Inter'] font-black text-[11px] w-7 h-7 grid place-items-center rounded-lg flex-shrink-0" style={{ color: '#052033', background: 'linear-gradient(135deg,#7dd3fc,#38bdf8)' }}>{String(m.module_number).padStart(2, '0')}</span>
                    <div className="min-w-0 flex-1">
                      <p className="font-['Inter'] font-bold text-[13px] truncate" dir="ltr" style={{ color: 'var(--cream)' }}>{m.title_en || m.title_ar}</p>
                      {pct !== null ? (
                        <div dir="ltr" className="h-1.5 rounded-full overflow-hidden mt-1.5 max-w-[220px]" style={{ background: 'rgba(255,255,255,0.06)' }}>
                          <div className="h-full rounded-full" style={{ width: `${pct}%`, background: 'linear-gradient(90deg,#38bdf8,#7dd3fc)' }} />
                        </div>
                      ) : (
                        <p className="font-['Inter'] text-[11px] mt-0.5" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.4)' }}>Not graded yet</p>
                      )}
                    </div>
                    {done
                      ? <CheckCircle2 size={16} className="flex-shrink-0" style={{ color: '#6ee7b7' }} />
                      : <span className="font-['Inter'] text-[12px] tabular-nums font-semibold flex-shrink-0" style={{ color: pct === null ? 'rgba(238, 243, 251,0.3)' : 'var(--brass-hi)' }}>{pct === null ? '' : `${pct}%`}</span>}
                  </div>
                )
              })}
            </div>
