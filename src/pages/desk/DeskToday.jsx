  ...
  return (
    <div className="space-y-7">
      ...
      <div className="desk-rise">
        ...
        <h1 className="font-['Inter'] font-extrabold text-2xl lg:text-[30px]" dir="ltr" style={{ color: 'var(--cream)' }}>
          {firstName ? `Welcome back, ${firstName}` : 'Welcome back'}
        </h1>
        <p className="font-['Inter'] text-[14px] mt-1.5" dir="ltr" style={{ color: 'rgba(243,238,226,0.7)' }}>
          {current ? 'Your next mission is ready — run it before your real call.' : "You've cleared every scenario available. Great work."}
        </p>
        <p className="font-['Tajawal'] text-[12px] mt-0.5" style={{ color: 'rgba(243,238,226,0.45)' }}>
          {current ? 'جهّزنا مهمتك القادمة — جرّبيها قبل مكالمتك الحقيقية.' : 'أنجزتِ كل السيناريوهات المتاحة.'}
        </p>
      </div>
        ...
        <motion.div initial={rm ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.16, 1, 0.3, 1], delay: 0.02 }}>
          <Link to={`/desk/classes/${latestClass.id}`} className="group desk-glass flex items-center gap-4 p-5" style={{ borderColor: isClassDone(latestClass.id) ? undefined : 'rgba(201,162,92,0.24)' }}>
            ...
            <div className="min-w-0 flex-1">
              <p className="font-['Inter'] text-[11px] font-bold mb-0.5 tracking-[0.14em]" dir="ltr" style={{ color: 'var(--brass)' }}>
                {isClassDone(latestClass.id) ? 'REVIEWED · OPEN AGAIN' : 'REVIEW YOUR LAST CLASS'}
              </p>
              <h3 className="font-['Inter'] font-extrabold text-[15px] leading-tight truncate" dir="ltr" style={{ color: 'var(--cream)' }}>
                Class {latestClass.number} · {latestClass.title_en || latestClass.title_ar}
              </h3>
              <p className="font-['Tajawal'] text-[12px] mt-1 truncate" style={{ color: 'rgba(243,238,226,0.5)' }}>
                {latestClass.title_ar}
              </p>
            </div>
      ...
      <motion.div initial={rm ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.16, 1, 0.3, 1], delay: 0.04 }}>
        <Link to={currentLesson ? `/desk/track/${currentLesson.id}` : '/desk/track'} className="group desk-glass flex items-center gap-4 p-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-['Inter'] text-[11px] font-bold mb-0.5 tracking-[0.14em]" dir="ltr" style={{ color: 'var(--brass)' }}>
              {track.done === 0 ? 'START YOUR TRACK' : currentLesson ? 'CONTINUE YOUR TRACK' : 'TRACK COMPLETE'}
            </p>
            <h3 className="font-['Inter'] font-extrabold text-[15px] leading-tight truncate" dir="ltr" style={{ color: 'var(--cream)' }}>
              {currentLesson ? (currentLesson.en || currentLesson.ar) : 'All lessons complete — great work'}
            </h3>
            {currentLesson && (
              <p className="font-['Tajawal'] text-[12px] mt-0.5 truncate" style={{ color: 'rgba(243,238,226,0.5)' }}>
                {currentLesson.ar}
              </p>
            )}
            <div className="flex items-center gap-2.5 mt-1.5">
              ...
              <span className="font-['Inter'] text-[11px] font-bold tabular-nums" dir="ltr" style={{ color: 'var(--brass-hi)' }}>{track.done}/{track.total}</span>
            </div>
          </div>
          <ArrowLeft size={18} className="flex-shrink-0" style={{ color: 'var(--brass)' }} />
        </Link>
      ...
      {current ? (
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.16, 1, 0.3, 1] }}
          ...
          <div className="grid lg:grid-cols-[1fr_300px]">
            ...
            <div className="p-6 lg:p-8">
              <p className="font-['Inter'] text-[12px] font-bold mb-3 tracking-[0.14em]" dir="ltr" style={{ color: 'var(--brass)' }}>TODAY'S MISSION</p>
              <div className="flex items-center gap-2.5 mb-2">
                ...
                {current.progress?.status === 'in_progress' && (
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-bold font-['Inter'] tracking-wide" dir="ltr" style={{ color: '#efd299', background: 'rgba(201,162,92,0.12)', border: '1px solid rgba(201,162,92,0.3)' }}>IN PROGRESS</span>
                )}
              </div>
              <h2 className="font-['Inter'] font-extrabold text-2xl leading-tight mb-1.5" dir="ltr" style={{ color: 'var(--cream)' }}>{current.title_en || current.title_ar}</h2>
              <p className="font-['Tajawal'] text-[13px] mb-4" style={{ color: 'rgba(201,162,92,0.72)' }}>{current.title_ar}</p>
              ...
              <Link to={`/desk/scenarios/${current.id}`} className="desk-cta inline-flex items-center gap-2 px-6 h-12 rounded-2xl font-['Inter'] font-bold text-[14px]">
                <Headset size={17} /> Start scenario
              </Link>
            </div>
        ...
        <div className="desk-glass p-8 text-center desk-rise">
          ...
          <p className="font-['Inter'] font-bold text-lg" dir="ltr" style={{ color: 'var(--cream)' }}>You've cleared every scenario — great work</p>
          <p className="font-['Tajawal'] text-[13px] mt-1" style={{ color: 'rgba(243,238,226,0.5)' }}>أنجزتِ كل السيناريوهات</p>
          <Link to="/desk/scenarios" className="inline-flex items-center gap-1.5 mt-3 font-['Inter'] text-sm" dir="ltr" style={{ color: 'var(--brass)' }}>Review them again <ArrowLeft size={14} /></Link>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-[1fr_auto] gap-4 items-center desk-glass p-5">
        <div>
          <p className="font-['Inter'] text-[13px] font-bold mb-2" dir="ltr" style={{ color: 'rgba(243,238,226,0.7)' }}>Your track progress</p>
        ...
        <Link to="/desk/scenarios" className="inline-flex items-center justify-center gap-1.5 px-4 h-10 rounded-xl font-['Inter'] font-bold text-[13px]" dir="ltr" style={{ color: 'var(--brass-hi)', background: 'rgba(201,162,92,0.10)', border: '1px solid rgba(201,162,92,0.24)' }}>
          All scenarios <ArrowLeft size={15} />
        </Link>
      </div>
    </div>
  )
