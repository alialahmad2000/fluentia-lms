...
const STATUS = {
  ...
  in_progress: { label: 'In progress', color: '#7dd3fc', bg: 'rgba(56, 189, 248,0.12)',  bd: 'rgba(56, 189, 248,0.30)' },
  new:         { label: 'New',         color: 'rgba(238, 243, 251,0.6)', bg: 'rgba(255,255,255,0.04)', bd: 'rgba(255,255,255,0.10)' },
}
  ...
  if (!data?.modules?.length) {
    return (
      <div className="desk-glass p-8 text-center desk-rise">
        ...
        <p className="font-['Inter'] text-sm mt-1" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.55)' }}>Your career track will be assigned soon.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="desk-rise">
        ...
        {spec?.title_ar && (
          <p className="font-['Tajawal'] text-sm mt-1" style={{ color: 'rgba(238, 243, 251,0.55)' }}>{spec.title_ar}</p>
        )}
        {spec?.tagline_ar && (
          <p className="font-['Tajawal'] text-sm mt-2 max-w-2xl leading-relaxed" style={{ color: 'rgba(238, 243, 251,0.6)' }}>{spec.tagline_ar}</p>
        )}
        {/* progress */}
          ...
          return (
            <motion.div key={m.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.05, 0.4), ease: [0.16, 1, 0.3, 1] }}>
              <Link to={`/desk/scenarios/${m.id}`} className="desk-glass block p-5 h-full transition-transform hover:-translate-y-0.5 group"
                style={m.id === currentId ? { borderColor: 'rgba(56, 189, 248,0.42)', boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.06), 0 0 0 1px rgba(56, 189, 248,0.18), 0 20px 44px -22px rgba(56, 189, 248,0.4)' } : undefined}>
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2.5">
                    <span className="font-['Inter'] font-black text-[13px] w-8 h-8 grid place-items-center rounded-lg" style={{ color: '#052033', background: 'linear-gradient(135deg,#7dd3fc,#38bdf8)' }}>{String(m.module_number).padStart(2, '0')}</span>
                ...
                <p className="font-['Tajawal'] text-[12px] mb-2" style={{ color: 'rgba(238, 243, 251,0.5)' }}>{m.title_ar}</p>
                <p className="font-['Tajawal'] text-[13px] leading-relaxed line-clamp-2" style={{ color: 'rgba(238, 243, 251,0.58)' }}>{m.scenario_ar}</p>
                <div className="flex items-center gap-4 mt-3 pt-3 desk-hair">
                  <span className="flex items-center gap-1.5 font-['Inter'] text-[11px]" style={{ color: 'rgba(238, 243, 251,0.45)' }}><Clock size={12} /> {m.estimated_minutes || 25} min</span>
                  <span className="flex items-center gap-1.5 font-['Inter'] text-[11px]" style={{ color: 'rgba(238, 243, 251,0.45)' }}><BookOpen size={12} /> {m.vocab_count || (Array.isArray(m.vocabulary) ? m.vocabulary.length : 0)} terms</span>
                </div>
              </Link>
            </motion.div>
          )
