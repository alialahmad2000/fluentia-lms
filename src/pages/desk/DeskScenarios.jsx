...
const STATUS = {
  completed:   { label: 'مكتمل',       color: '#6ee7b7', bg: 'rgba(110,231,183,0.12)', bd: 'rgba(110,231,183,0.28)' },
  in_progress: { label: 'قيد التنفيذ', color: '#efd299', bg: 'rgba(201,162,92,0.12)',  bd: 'rgba(201,162,92,0.30)' },
  new:         { label: 'جديد',        color: 'rgba(243,238,226,0.6)', bg: 'rgba(255,255,255,0.04)', bd: 'rgba(255,255,255,0.10)' },
}
  ...
  if (!data?.modules?.length) {
    return (
      <div className="desk-glass p-8 text-center desk-rise">
        ...
        <p className="font-['Tajawal'] font-bold text-lg">لا توجد سيناريوهات بعد</p>
        <p className="font-['Tajawal'] text-sm mt-1" style={{ color: 'rgba(243,238,226,0.55)' }}>سيتم تعيين مسارك المهني قريباً.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="desk-rise">
        <p className="font-['Tajawal'] text-[12px] tracking-[0.2em] mb-1" style={{ color: 'var(--brass)' }}>السيناريوهات</p>
        <h1 className="font-['Tajawal'] font-extrabold text-2xl lg:text-3xl" style={{ color: 'var(--cream)' }}>
          {spec?.title_ar || 'مسارك المهني'}
        </h1>
        {spec?.tagline_ar && (
              ...
              <Link to={`/desk/scenarios/${m.id}`} className="desk-glass block p-5 h-full transition-transform hover:-translate-y-0.5 group"
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="px-2.5 py-1 rounded-full text-[10px] font-bold font-['Tajawal'] flex items-center gap-1" style={{ color: st.color, background: st.bg, border: `1px solid ${st.bd}` }}>
                      <StatusIcon size={11} /> {st.label}
                    </span>
                    {m.id === currentId && <span className="font-['Tajawal'] text-[10px] font-bold" style={{ color: 'var(--brass)' }}>· الحالية</span>}
                  </div>
                  <span className="desk-ghost-btn"><ArrowLeft size={15} /></span>
                </div>
                <h3 className="font-['Tajawal'] font-bold text-[16px] leading-snug mb-1.5" style={{ color: 'var(--cream)' }}>{m.title_ar}</h3>
                <p className="font-['Inter'] text-[11px] mb-2" dir="ltr" style={{ color: 'rgba(201,162,92,0.7)' }}>{m.title_en}</p>
                ...
                <div className="flex items-center gap-4 mt-3 pt-3 desk-hair">
                  <span className="flex items-center gap-1.5 font-['Tajawal'] text-[11px]" style={{ color: 'rgba(243,238,226,0.45)' }}><Clock size={12} /> {m.estimated_minutes || 25} دقيقة</span>
                  <span className="flex items-center gap-1.5 font-['Tajawal'] text-[11px]" style={{ color: 'rgba(243,238,226,0.45)' }}><BookOpen size={12} /> {m.vocab_count || (Array.isArray(m.vocabulary) ? m.vocabulary.length : 0)} مصطلح</span>
                </div>
              </Link>
            </motion.div>
          )
