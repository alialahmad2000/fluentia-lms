  ...
  return (
    <div className="relative" style={{ width: 92, height: 92 }}>
      <svg width="92" height="92" viewBox="0 0 92 92">
        ...
        <defs><linearGradient id="deskClassGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#ef6a43" /><stop offset="1" stopColor="#cf4a1c" /></linearGradient></defs>
      </svg>
  ...
  return (
    <div className="space-y-12 max-w-[720px] mx-auto">
      ...
      <motion.div initial={rm ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.16, 1, 0.3, 1] }}
        className="desk-glass p-6 flex items-center gap-5" style={{ borderColor: 'rgba(239, 106, 67,0.2)' }}>
      ...
      {/* continue button */}
      {prog.current && (
        <Link to={`/desk/classes/${cls.id}/${prog.current.id}`} className="desk-cta -mt-3 w-full inline-flex items-center justify-center gap-2 h-12 rounded-2xl font-['Hanken_Grotesk'] font-bold text-[14px] desk-rise" dir="ltr">
      ...
      {/* station journey */}
      <div>
        <p className="font-['Hanken_Grotesk'] text-[12px] tracking-[0.2em] mb-4 desk-rise" dir="ltr" style={{ color: 'rgba(239, 106, 67,0.6)' }}>STATIONS · {cls.chapters.length}</p>
                ...
                <div className={`desk-lesson-card ${isCurrent ? 'is-current' : ''}`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-['Hanken_Grotesk'] font-bold text-[15px] leading-snug" dir="ltr" style={{ color: 'var(--cream)' }}>
                        <span className="font-['Hanken_Grotesk'] font-black text-[12px] me-1" style={{ color: 'var(--brass)' }}>{i + 1}</span> {ch.en}
                      </h3>
      ...
      {cls.takeaways_en?.length > 0 && (
        <section className="desk-rise">
          ...
          <div className="desk-glass p-6 space-y-2.5" style={{ borderColor: 'rgba(239, 106, 67,0.22)' }}>
            {cls.takeaways_en.map((t, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="desk-gold-dot">{i + 1}</span>
                <p className="font-['Hanken_Grotesk'] text-[14px] font-bold leading-relaxed" dir="ltr" style={{ color: 'var(--cream)' }}>{t}</p>
              </div>
            ))}
          </div>
        </section>
