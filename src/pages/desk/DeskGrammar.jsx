  ...
  return (
    <div className="space-y-7 max-w-[680px] mx-auto">
      ...
      <div className="desk-rise">
        ...
        <p className="font-['Inter'] text-[13.5px] mt-1.5 leading-relaxed" dir="ltr" style={{ color: 'rgba(243,238,226,0.55)' }}>The grammar a working professional actually needs — one rule a day, the rest here whenever you want it.</p>
      ...
      {grammarToday && (
        <motion.div initial={rm ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="desk-rise">
          <Link to={`/desk/daily/grammar/${grammarToday.id}`} className="group desk-glass flex items-center gap-4 p-5" style={{ borderColor: 'rgba(201,162,92,0.26)' }}>
      ...
      {/* the bank */}
      <div className="desk-rise">
        <p className="font-['Inter'] text-[12px] tracking-[0.2em] mb-3" dir="ltr" style={{ color: 'rgba(201,162,92,0.6)' }}>ALL RULES · {DESK_GRAMMAR.length}</p>
