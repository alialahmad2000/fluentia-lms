...
function Session({ rm }) {
  ...
  if (done) {
    return (
      <div className="desk-glass p-8 text-center desk-rise" style={{ borderColor: 'rgba(201,162,92,0.24)' }}>
        ...
        {ids.length > 0 && (
          <div className="inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-full" style={{ background: 'rgba(201,162,92,0.1)', border: '1px solid rgba(201,162,92,0.24)' }}>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div dir="ltr" className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <motion.div className="h-full rounded-full" initial={false} animate={{ width: `${(idx / ids.length) * 100}%` }} style={{ background: 'linear-gradient(90deg,#c9a25c,#efd299)' }} />
        </div>
        ...
        <AnimatePresence mode="wait">
          {!flipped ? (
            <motion.div key="front" initial={rm ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }} className="flex flex-col items-center justify-center gap-2 py-4">
              ...
              <p className="font-['Inter'] text-[13px]" dir="ltr" style={{ color: 'rgba(201,162,92,0.7)' }}>/{w.ipa}/ · {w.pos}</p>
              <PlayBtn text={w.word} size={16} />
              <p className="font-['Inter'] text-[12px] mt-2" dir="ltr" style={{ color: 'rgba(243,238,226,0.4)' }}>Tap to see the meaning</p>
            </motion.div>
          ) : (
            <motion.div key="back" initial={rm ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }} className="flex flex-col items-center gap-2.5 py-2 text-center">
              ...
              <div className="desk-hair w-full pt-3 mt-1">
                ...
                <p className="font-['Tajawal'] text-[12.5px] mt-1.5" style={{ color: 'rgba(243,238,226,0.55)' }}>{w.example_ar}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
...
function Vault() {
  ...
  const badge = (w) => {
    ...
    if (!st?.seen) return { t: 'New', c: 'rgba(243,238,226,0.45)', b: 'rgba(255,255,255,0.06)' }
    if ((st.box || 0) >= 4) return { t: 'Mastered', c: 'var(--brass-hi)', b: 'rgba(201,162,92,0.14)' }
        ...
        return (
          <div key={w.id} className="desk-glass p-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="font-['Inter'] text-[12.5px] mt-1.5" dir="ltr" style={{ color: 'rgba(243,238,226,0.5)' }}>{w.example}</p>
            </div>
            <PlayBtn text={w.word} />
          </div>
        )
  ...
  return (
    <div className="space-y-7 max-w-[640px] mx-auto">
      ...
      <div className="desk-rise">
        ...
        <p className="font-['Inter'] text-[13.5px] mt-1.5 leading-relaxed" dir="ltr" style={{ color: 'rgba(243,238,226,0.55)' }}>Review with spaced repetition — each word comes back at just the right time so it sticks.</p>
        <div className="flex items-center gap-4 mt-3 font-['Inter'] text-[12.5px]" dir="ltr" style={{ color: 'rgba(243,238,226,0.5)' }}>
