        ...
        return (
          <button key={i} disabled={picked != null && opt.correct}
            ...
            {picked != null && (state === 'correct' || state === 'wrong') && (
              <p className="font-['Inter'] text-[12px] mt-2 pt-2 desk-hair" dir="ltr" style={{ color: state === 'correct' ? 'rgba(110,231,183,0.9)' : 'rgba(255,180,164,0.85)' }}>{opt.why}</p>
            )}
          </button>
        )
      })}
      {picked != null && !chosen?.correct && (
        <button onClick={() => setPicked(null)} className="inline-flex items-center gap-1.5 font-['Inter'] text-[12px] mt-1" dir="ltr" style={{ color: 'var(--brass)' }}>
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 pt-1">
        {!checked && picked.length === practice.steps.length && (
          <button onClick={check} className="desk-cta inline-flex items-center gap-2 px-5 h-10 rounded-xl font-['Inter'] font-bold text-[13px]" dir="ltr">
            <Check size={15} /> Check
          </button>
        )}
        {checked && (
          <>
            <span className="font-['Inter'] text-[13px] font-bold" dir="ltr" style={{ color: correct ? '#6ee7b7' : 'rgba(255,180,164,0.9)' }}>
              {correct ? 'Correct order ✓' : 'Check the order'}
            </span>
            <button onClick={reset} className="inline-flex items-center gap-1.5 font-['Inter'] text-[12px]" dir="ltr" style={{ color: 'var(--brass)' }}>
