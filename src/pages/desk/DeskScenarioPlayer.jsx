  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between desk-rise">
        <Link to="/desk/scenarios" className="inline-flex items-center gap-1.5 font-['Inter'] text-[13px]" style={{ color: 'rgba(238, 243, 251,0.55)' }}>
      ...
      <div className="desk-glass p-6 lg:p-7 desk-rise" style={{ borderColor: 'rgba(56, 189, 248,0.22)' }}>
        ...
        <p className="font-['Tajawal'] text-[13px] mb-4" style={{ color: 'rgba(56, 189, 248,0.7)' }}>{module.title_ar}</p>
        <p className="font-['Tajawal'] text-[14px] leading-relaxed" style={{ color: 'rgba(238, 243, 251,0.72)' }}>{module.scenario_ar}</p>
      </div>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={active
                ? { color: '#052033', background: 'linear-gradient(135deg,#7dd3fc,#38bdf8)' }
                : { color: 'rgba(238, 243, 251,0.6)', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(56, 189, 248,0.14)' }}>
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
            ...
            <div className="space-y-4">
              {rp?.ai_role && (
                <div className="desk-glass p-5">
                  ...
                  <p className="font-['Inter'] text-[13px] leading-relaxed" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.75)' }}>{rp.ai_role}</p>
                  {rp?.student_role && <p className="font-['Inter'] text-[13px] mt-3 pt-3 desk-hair" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.6)' }}>Your role: <span className="font-['Inter']">{rp.student_role}</span></p>}
                </div>
              )}
              {objectives.length > 0 && (
                <div className="desk-glass p-5">
                  ...
                  <ul className="space-y-2.5">
                    {objectives.map((o, i) => (
                      <li key={i} className="flex items-start gap-2.5">
                        ...
                        <span className="font-['Inter'] text-[14px] leading-relaxed" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.78)' }}>{o.en || o.ar || o}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}
              <button onClick={() => setTab('roleplay')} className="w-full desk-glass p-4 flex items-center justify-center gap-2 font-['Inter'] font-bold text-[14px] transition-transform hover:-translate-y-0.5" style={{ color: '#052033', background: 'linear-gradient(135deg,#7dd3fc,#38bdf8)', border: 'none' }}>
              ...
              {vocab.map((v, i) => (
                <div key={i} className="desk-glass p-4">
                  <div className="flex items-baseline gap-2 mb-1">
                    ...
                    {v.pos && <span className="font-['Inter'] text-[10px] italic" style={{ color: 'rgba(238, 243, 251,0.4)' }}>{v.pos}</span>}
                  </div>
                  <p className="font-['Tajawal'] text-[14px]" style={{ color: 'rgba(238, 243, 251,0.8)' }}>{v.ar}</p>
                  {v.example && <p className="font-['Inter'] text-[12px] mt-2 leading-relaxed" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.5)' }}>“{v.example}”</p>}
                </div>
              ))}
            </div>
          )}
              ...
              {phrases.map((p, i) => (
                <div key={i} className="desk-glass p-4">
                  ...
                  <div className="flex items-center gap-2 mt-2 pt-2 desk-hair">
                    <p className="font-['Tajawal'] text-[13px]" style={{ color: 'rgba(238, 243, 251,0.6)' }}>{p.ar}</p>
                  </div>
                  {p.context_ar && <p className="font-['Tajawal'] text-[11px] mt-1.5" style={{ color: 'rgba(56, 189, 248,0.65)' }}><span className="font-['Inter']" dir="ltr">When?</span> {p.context_ar}</p>}
                </div>
              ))}
            </div>
          )}
          ...
          {tab === 'writing' && (
            <div className="desk-glass p-6">
              ...
              {writing?.prompt_ar && <p className="font-['Tajawal'] text-[14px] leading-relaxed mb-4" style={{ color: 'rgba(238, 243, 251,0.72)' }}>{writing.prompt_ar}</p>}
              {Array.isArray(writing?.hints) && writing.hints.length > 0 && (
                <div className="pt-3 desk-hair">
                  <p className="font-['Inter'] text-[12px] font-bold mb-2" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.6)' }}>Hints</p>
                  <ul className="space-y-1.5">
                    {writing.hints.map((h, i) => (
                      <li key={i} className="font-['Tajawal'] text-[13px] flex items-start gap-2" style={{ color: 'rgba(238, 243, 251,0.62)' }}>
              ...
              <div className="mt-5 pt-4 desk-hair">
                <div className="flex items-center justify-between mb-2">
                  <label className="font-['Inter'] text-[12px] font-bold" dir="ltr" style={{ color: 'rgba(238, 243, 251,0.7)' }}>Your report</label>
                  <span className="font-['Inter'] text-[11px] tabular-nums" style={{ color: wordCount ? 'var(--brass-hi)' : 'rgba(238, 243, 251,0.35)' }}>{wordCount} {wordCount === 1 ? 'word' : 'words'}</span>
                </div>
                <textarea
                  value={report}
                  ...
                  style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(56, 189, 248,0.16)', color: 'var(--cream)' }}
                  onFocus={(e) => { e.target.style.borderColor = 'rgba(56, 189, 248,0.4)' }}
                  onBlur={(e) => { e.target.style.borderColor = 'rgba(56, 189, 248,0.16)' }}
                />
