...
import { ProgressRing, StatusChip, getUnitStatus, LoadingSkeleton, EmptyState, CINEMATIC_TOKENS as V1 } from './_premiumPrimitives'
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '1100px', margin: '0 auto', padding: '0 24px' }}>
        ...
        <section style={{ padding: '80px 0 60px', position: 'relative' }}>
          <div style={{
            ...
            background: `linear-gradient(135deg, ${V1.accentCyan}, ${V1.accentGold})`,
          ...
          <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
            <motion.button
              ...
              style={{ fontSize: '14px', color: `${V1.accentGold}99` }}
            >
              <ArrowRight size={16} />
              العودة للمستويات
            </motion.button>
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
              <div className="flex-1">
                ...
                <motion.div variants={fadeUp} className="flex items-center gap-3 mb-4">
                  ...
                  <span style={{ fontFamily: "'Inter Tight', sans-serif", fontSize: '20px', color: V1.accentGold, fontStyle: 'italic' }} dir="ltr">
                    {level.name_en}
                  </span>
                  <span style={{
                    fontSize: '12px', fontWeight: 600, padding: '3px 10px', borderRadius: '999px',
                    background: `${V1.accentCyan}15`, color: V1.accentCyan, border: `1px solid ${V1.accentCyan}30`,
                  }}>
                    {level.cefr}
                  </span>
                </motion.div>
                ...
                {nextUnit && (
                  <motion.button
                    ...
                    style={{
                      ...
                      border: `2px solid ${V1.accentGold}60`, color: V1.accentGold, background: 'transparent',
                    }}
                    whileHover={{ background: `${V1.accentGold}15`, boxShadow: `0 0 20px ${V1.accentCyan}20` }}
                  >
                    ابدأ من حيث توقفت
                    <ArrowLeft size={18} />
                  </motion.button>
              ...
              <motion.div variants={fadeUp} className="flex flex-col items-center shrink-0">
                <div style={{ position: 'relative' }}>
                  <ProgressRing percent={levelProgress.overallPercent} size={140} strokeWidth={8} color={V1.accentCyan} bgColor="rgba(255,255,255,0.06)" />
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: '32px', fontWeight: 800, fontFamily: "'Inter Tight', sans-serif", color: V1.textPrimary }}>
        ...
        {units.length === 0 ? <EmptyState /> : chapters.map((chapter, ci) => (
          <section key={ci} style={{ marginBottom: '64px' }}>
            ...
            >
              <div style={{ flex: 1, height: '1px', background: `${V1.accentGold}30` }} />
              <span style={{ color: V1.accentGold, fontSize: '14px' }}>◆</span>
              <span style={{ fontFamily: "'Amiri', serif", fontSize: '28px', fontWeight: 700, color: V1.textPrimary }}>
                {chapter.name}
              </span>
              {chapter.theme && (
                <span style={{ fontSize: '16px', color: V1.textDim }}>— {chapter.theme}</span>
              )}
              <span style={{ color: V1.accentGold, fontSize: '14px' }}>◆</span>
              <div style={{ flex: 1, height: '1px', background: `${V1.accentGold}30` }} />
            </motion.div>
...
function Pill({ label, value, color }) {
  return (
    <span style={{
      ...
      background: `${color}10`, border: `1px solid ${color}25`, color: V1.textPrimary,
    }}>
...
function FeaturedCard({ unit, progress, levelColor, navigate, levelNum }) {
  return (
    <motion.div
      ...
      whileHover={{ borderColor: `${V1.accentGold}40`, boxShadow: `0 0 40px ${V1.accentGold}10`, scale: 1.01 }}
    ...
    >
      ...
      <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <span style={{ fontSize: '80px', fontWeight: 800, fontFamily: "'Inter Tight', sans-serif", color: V1.accentGold, opacity: 0.15, lineHeight: 1 }}>
          {unit.unit_number}
        </span>
        <h3 style={{ fontSize: '32px', fontWeight: 700, color: V1.textPrimary, marginTop: '-20px', marginBottom: '8px' }}>
          {unit.theme_ar}
        </h3>
        {unit.theme_en && (
          <p style={{ fontSize: '16px', color: V1.accentCyan, fontStyle: 'italic', marginBottom: '16px' }} dir="ltr">
            {unit.theme_en}
          </p>
        )}
        <div className="flex items-center gap-3">
          ...
          {unit.estimated_minutes && (
            <span className="flex items-center gap-1" style={{ fontSize: '13px', color: V1.textDim }}>
        ...
        {progress.percent > 0 && (
          <div style={{ marginTop: '12px', height: '3px', borderRadius: '2px', background: 'rgba(255,255,255,0.08)' }}>
            <div style={{ height: '100%', borderRadius: '2px', width: `${progress.percent}%`, background: progress.status === 'completed' ? '#4ade80' : V1.accentCyan, transition: 'width 0.5s' }} />
          </div>
        )}
      </div>
    </motion.div>
...
function StandardCard({ unit, progress, levelColor, navigate, levelNum }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6, borderColor: `${V1.accentGold}40`, boxShadow: `0 12px 40px ${V1.accentGold}08` }}
      transition={{ type: 'spring', stiffness: 300, damping: 25 }}
    >
      ...
      <div style={{ padding: '16px 20px 20px' }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 style={{ fontSize: '17px', fontWeight: 700, color: V1.textPrimary }}>{unit.theme_ar}</h3>
        </div>
        {unit.theme_en && (
          <p style={{ fontSize: '13px', color: V1.textDim, fontStyle: 'italic' }} dir="ltr">{unit.theme_en}</p>
        )}
        {progress.percent > 0 && (
          <div style={{ marginTop: '10px', height: '2px', borderRadius: '1px', background: 'rgba(255,255,255,0.08)' }}>
            <div style={{ height: '100%', borderRadius: '1px', width: `${progress.percent}%`, background: progress.status === 'completed' ? '#4ade80' : V1.accentCyan, transition: 'width 0.5s' }} />
          </div>
        )}
      </div>
    </motion.div>
