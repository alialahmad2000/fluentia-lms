// A single Business Track lesson: intro → vocabulary (audio) → reading (tap-to-translate)
// → key phrases → comprehension quiz → speaking/writing task → mark complete.
// Mirrors TechLessonPage; 2nd-person Arabic is gender-aware via useG (ظافر = male).
import { useState } from 'react'
import { useParams, Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { ArrowRight, Volume2, Check, Loader2, Mic, PenLine } from 'lucide-react'
import { useG } from '../../../i18n/gender'
import { useBizLesson, useCompleteBizLesson } from './useBizTrack'
import BizPassage, { speak } from './BizPassage'
import BizQuiz from './BizQuiz'
import './bizTrack.css'
...
export default function BizLessonPage() {
  const g = useG()
  const { lessonSlug } = useParams()
  const { lesson, stage, done, score, isLoading } = useBizLesson(lessonSlug)
  const complete = useCompleteBizLesson()
  ...
  if (!lesson) {
    return (
      <div className="tt-lp" dir="rtl">
        <Link to="/biz" className="tt-back"><ArrowRight size={16} /> العودة إلى المسار</Link>
    ...
    <div className="tt-lp" dir="rtl">
      ...
      {c.reading && Array.isArray(c.reading.paragraphs) && c.reading.paragraphs.length > 0 && (
        <div className="tt-section">
          ...
          <div className="tt-card">
            <BizPassage title_en={c.reading.title_en} paragraphs={c.reading.paragraphs} vocab={c.vocab} accent={accent} />
          </div>
        </div>
      )}
      ...
      {Array.isArray(c.comprehension) && c.comprehension.length > 0 && (
        <div className="tt-section">
          ...
          <div className="tt-card">
            <BizQuiz questions={c.comprehension} accent={accent} onScore={setQuizScore} />
          </div>
        </div>
      )}
      ...
      <div className="tt-section">
        {isDone ? (
          <Link to="/biz" className="tt-btn" style={{ background: accent, textDecoration: 'none', boxShadow: `0 12px 30px -10px ${accent}99` }}>
