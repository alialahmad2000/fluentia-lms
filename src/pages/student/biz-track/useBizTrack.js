// Data hooks for the Business Track «مسار الأعمال». Mirrors useTechTrack.js against biz_track_*.
...
export function useBizRoadmap() {
  const profileId = useProfileId()

  const { data: stages = [], isLoading: sLoading } = useQuery({
    queryKey: ['biz-track', 'stages'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('biz_track_stages')
  ...
  const { data: lessons = [], isLoading: lLoading } = useQuery({
    queryKey: ['biz-track', 'lessons-light'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('biz_track_lessons')
  ...
  const { data: progress = [], isLoading: pLoading } = useQuery({
    queryKey: ['biz-track', 'progress', profileId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('biz_track_progress')
...
export function useBizLesson(slug) {
  const profileId = useProfileId()

  const { data, isLoading } = useQuery({
    queryKey: ['biz-track', 'lesson', slug],
    queryFn: async () => {
  ...
  const { data: prog } = useQuery({
    queryKey: ['biz-track', 'lesson-progress', profileId, lessonId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('biz_track_progress')
        .select('status, score')
...
export function useCompleteBizLesson() {
  const qc = useQueryClient()
  const profileId = useProfileId()
  return useMutation({
    mutationFn: async ({ lessonId, score }) => {
      const { data, error } = await supabase.rpc('biz_track_complete_lesson', {
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['biz-track', 'progress', profileId] })
      qc.invalidateQueries({ queryKey: ['biz-track', 'lesson-progress', profileId] })
    },
  })
}
