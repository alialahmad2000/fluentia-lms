// A single Environment Track lesson: intro → vocabulary (audio) → reading (tap-to-translate)
// → key phrases → comprehension quiz → speaking/writing task → mark complete.
// Mirrors BizLessonPage; 2nd-person Arabic is gender-aware via useG (نورة = female).
import { useState } from 'react'
import { useParams, Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { ArrowRight, Volume2, Check, Loader2, Mic, PenLine } from 'lucide-react'
import { useG } from '../../../i18n/gender'
import { useEnvLesson, useCompleteEnvLesson } from './useEnvTrack'
import EnvPassage, { speak } from './EnvPassage'
import EnvQuiz from './EnvQuiz'
import './envTrack.css'
...
export default function EnvLessonPage() {
  const g = useG()
  const { lessonSlug } = useParams()
  const { lesson, stage, done, score, isLoading } = useEnvLesson(lessonSlug)
  const complete = useCompleteEnvLesson()
  ...
  if (!lesson) {
    return (
      <div className="tt-lp" dir="rtl">
        <Link to="/env" className="tt-back"><ArrowRight size={16} /> العودة إلى المسار</Link>
        <p style={{ color: 'var(--ds-text-secondary)' }}>ما وجدنا هذا الدرس — {g('ارجع', 'ارجعي')} إلى المسار {g('واختر', 'واختاري')} درسًا آخر.</p>
      </div>
    )
  }

  const accent = stage?.accent || '#22c55e'
  ...
  return (
    <div className="tt-lp" dir="rtl">
      ...
      <div className="tt-lp-head">
        ...
        {isDone && <div className="tt-done-banner" style={{ marginTop: 12 }}><Check size={18} /> {g('أتممتَ', 'أتممتِ')} هذا الدرس{finalScore != null ? ` · ${finalScore}%` : ''}</div>}
      </div>
      ...
      {c.reading && Array.isArray(c.reading.paragraphs) && c.reading.paragraphs.length > 0 && (
        <div className="tt-section">
          <SectionH accent={accent}>القراءة · {g('اضغط', 'اضغطي')} أي كلمة مُظلَّلة لمعناها ونطقها</SectionH>
          <div className="tt-card">
            <EnvPassage title_en={c.reading.title_en} paragraphs={c.reading.paragraphs} vocab={c.vocab} accent={accent} />
          </div>
        </div>
      )}
      ...
      {Array.isArray(c.comprehension) && c.comprehension.length > 0 && (
        <div className="tt-section">
          <SectionH accent={accent}>{g('اختبر فهمك', 'اختبري فهمكِ')}</SectionH>
          <div className="tt-card">
            <EnvQuiz questions={c.comprehension} accent={accent} onScore={setQuizScore} />
          </div>
        </div>
      )}
      ...
      <div className="tt-section">
        {isDone ? (
          <Link to="/env" className="tt-btn" style={{ background: accent, textDecoration: 'none', boxShadow: `0 12px 30px -10px ${accent}99` }}>
        ...
        {complete.isError && <p style={{ color: '#f87171', fontSize: 13, textAlign: 'center', marginTop: 8 }}>تعذّر حفظ الإتمام، {g('حاول', 'حاولي')} مرة أخرى.</p>}
      </div>
    </div>
  )
}
