...
export default function TechQuiz({ questions = [], accent = '#38bdf8', onScore }) {
    ...
    <div className="tt-quiz">
      ...
      {total > 0 && (
        <div style={{ fontSize: 14, color: 'var(--ds-text-tertiary)', textAlign: 'center', marginTop: 6 }}>
          {answeredCount === total
            ? <span style={{ color: accent, fontWeight: 700 }}>نتيجتكِ: {Math.round((correctCount / total) * 100)}% ({correctCount}/{total})</span>
            : `${answeredCount}/${total}`}
        </div>
      )}
    </div>
