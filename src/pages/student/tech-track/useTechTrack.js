// Data hooks for the Tech Track «مسار التقنية».
...
export function useTechRoadmap() {
  ...
  const { data: stages = [], isLoading: sLoading } = useQuery({
    queryKey: ['tech-track', 'stages'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tech_track_stages')
  ...
  const { data: lessons = [], isLoading: lLoading } = useQuery({
    queryKey: ['tech-track', 'lessons-light'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tech_track_lessons')
  ...
  const { data: progress = [], isLoading: pLoading } = useQuery({
    queryKey: ['tech-track', 'progress', profileId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tech_track_progress')
...
export function useTechLesson(slug) {
  const profileId = useProfileId()

  const { data, isLoading } = useQuery({
    queryKey: ['tech-track', 'lesson', slug],
    queryFn: async () => {
  ...
  const { data: prog } = useQuery({
    queryKey: ['tech-track', 'lesson-progress', profileId, lessonId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tech_track_progress')
        .select('status, score')
...
export function useCompleteTechLesson() {
  const qc = useQueryClient()
  const profileId = useProfileId()
  return useMutation({
    mutationFn: async ({ lessonId, score }) => {
      const { data, error } = await supabase.rpc('tech_track_complete_lesson', {
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tech-track', 'progress', profileId] })
      qc.invalidateQueries({ queryKey: ['tech-track', 'lesson-progress', profileId] })
    },
  })
}
