    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'center', padding: '4px 0 12px' }}>
        <button type="button" onClick={openDigest}
          ...
          style={{ fontFamily: 'Tajawal, sans-serif', fontSize: 11.5, color: 'var(--ds-accent-gold)', padding: '5px 14px', borderRadius: 999, border: '1px solid color-mix(in oklab, var(--ds-accent-gold) 22%, transparent)', background: 'color-mix(in oklab, var(--ds-accent-gold) 6%, transparent)' }}>
      ...
      {open && createPortal(
        <div onClick={() => setOpen(false)}
          ...
          <div onClick={(e) => e.stopPropagation()} dir="rtl"
            style={{ width: '100%', maxWidth: 468, background: 'linear-gradient(180deg, var(--ds-bg-elevated), var(--ds-bg-base))', borderTop: '1px solid color-mix(in oklab, var(--ds-accent-gold) 14%, var(--ds-border-subtle))', borderRadius: '24px 24px 0 0', padding: '14px 18px max(env(safe-area-inset-bottom), 20px)', boxShadow: '0 -24px 60px -20px rgba(0,0,0,0.85)', maxHeight: '78vh', overflowY: 'auto' }}>
